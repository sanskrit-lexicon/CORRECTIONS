"""hwchk_iast1.py  ejf  for burnouf
  May 24, 2016
  Apr 21, 2016  Modified to make use of hwnorm1c
  Apr 20, 2016
  python hwchk_iast1.py ../../../orig/bur.txt ../../burhw2.txt hwchk_iast1.txt hwchk_iast1.org  <path to hwnorm1c.txt>
...
def main(inlines,hwrecs,fileout,fileout1):
 ...
 for hwrec in hwrecs:
  ...
  # For Burfey, skip this normalize_key logic, as hw0 already has done it
  ...
  if re.search(r'[()*\[]',hw1norm) or re.search(r'[()*\[]',hwnorm):
   ...
   print 'bur:%s,%d:%s:%s:t:' %(hwrec.hwslp,hwrec.linenum1,'X','Y')
   print
   nprob = nprob+1
   continue
...
if __name__ == "__main__":
 ...
 # slurp bur.txt file into list of lines
 with codecs.open(filein,encoding='utf-8',mode='r') as f:
  inlines = [x.rstrip('\r\n') for x in f]
