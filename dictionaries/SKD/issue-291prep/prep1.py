...
"""prep1.py for skd
  Apr 20, 2016
 python prep1.py skd prep1keys.txt ../../../orig/skd.txt ../../skdhw2.txt prep1.txt prep1.org
"""
import re
import sys,codecs
import transcoder
...
def disp_org(dictcode,icase,L,hw0,url,page0,datalines,maxout=10):
 ...
 out = "%s:%s,%s:%s:t:" %(dictcode,hw0,L,hw0)
 outarr.append(out)
 datalines = adjust_datalines(dictcode,datalines)
 # output up to maxout lines of datalines
 outlines = datalines[0:maxout]
 for x in outlines:
  # Remove '|', which is a line-separator 
  #x = re.sub(r'[|]','',x)
  y = transcoder.transcoder_processString(x,'as','roman')
  outarr.append(';  %s' % y)
 if len(datalines)>maxout:
  ndiff = len(datalines) - maxout
  outarr.append(';   [and %s more lines]' % ndiff)
 outarr.append('')
 return outarr

def disp_md(dictcode,icase,L,hw0,url,page0,datalines,maxout):
...
def main(dictcode,headwords,inlines,hwrecs,fileout1,fileout2):
 ...
 for hw in headwords:
  ...
  # org mode
  maxout=5
  outarr = disp_org(dictcode,icase,L,hw0,url,page0,datalines,maxout)
  f1.write('\n'.join(outarr))
  # markdown
  outarr = disp_md(dictcode,icase,L,hw0,url,page0,datalines,maxout)
  f2.write('\n'.join(outarr))
