_Created: 03-05-2016 · Last updated: 05-09-2026_

 Case 0001: aMsraM  [page 1-156](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-156) 
```
skd:aMsraM,3070:aMsraM:n:

 <HI>aMsraM, klI, (asyate kzipyate yat . as + ra .) raktaM .
<>asru . ityamaraH .. (raktArte yaTA, --
<>“pipAsAdAhapittAsrayuktaM pittajvaraM jayet” . <>iti SArNgaDaraH ..
<>“kzIRe'sre maDurAkANkzA mUrcCA ca tvaci rukzatA .
<>SETilyaYca sirARAM syAdvAtAdunmArgagAmitA” .
  [and 5 more lines]
```
------------------------------------------
 Case 0002: akArpyaRyaM  [page 1-003](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-003) 
```
skd:akArpyaRyaM,61:akArpyaRyaM:n:

 <HI>akArpyaRyaM, klI, (na kArpaRyam . naYsamAsaH .)
<>kfpaRatArAhityaM . yaTA, --
<>“stokAdapi ca dAtavyamadInenEva cAtmanA .
<>ahanyahani yatkiYcidakArpyaRyaMhitat smftam” .. <>ityekAdaSItattvam ..
```
------------------------------------------
 Case 0003: akzOriBaM  [page 1-006](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-006) 
```
skd:akzOriBaM,168:akzOriBaM:n:

 <HI>akzOriBaM klI, kzurikarmmAvihitanakzatraM . yaTA, --
<>“uttarAtritayayAmyarohiRI- <>rOdrasarpapitfBezu cAgniBe .
<>amaSrukarmma sakalaM vivarjjayet <>pretakAryyamapi budDimAn naraH ..
<>candraSudDiryadA nAsti tArAyASca viSezataH .
<>akzOriBe'pi karttavyaM candracandrajayordine” ..
  [and 1 more lines]
```
------------------------------------------
 Case 0004: atwaNa  [page 1-031](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-031) 
```
skd:atwaNa,706:atwaNa:n:

 <HI>atwaNa, atikrame . baDe . iti kavikalpadrumaH .. dantya-
<>vargAdyopaDaH . Na atiwwizate . iti durgAdAsaH ..
```
------------------------------------------
 Case 0005: atyantonaH  [page 1-031](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-031) 
```
skd:atyantonaH,717:atyantonaH:n:

 <HI>atyantonaH, tri, (antasyAtyayaH avyayIBAvasamAsaH .
<>atyantaM gAmI dvitIyAtatpuruzaH . lakzmoM parampa-
<>rIRAM tvamatyantInatvamunnaye iti BawwiH .) BfSa-
<>NgAmI . atiSayagamanaSIlaH . ityamaraH ..
```
------------------------------------------
 Case 0006: adwaNa  [page 1-033](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-033) 
```
skd:adwaNa,774:adwaNa:n:

 <HI>adwaNa Na atikrame baDe . iti kavikalpadrumaH ..
<>dantyavargatftIyopaDaH . (awwituM atikramitu-
<>micCati -- adw + san + law te .) Na awwi- <>wizate . iti durgAdAsaH ..
```
------------------------------------------
 Case 0007: adqa  [page 1-033](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-033) 
```
skd:adqa,775:adqa:n:

 <HI>adqa, aBiyoge . iti kavikalpadrumaH .. dantyavarga-
<>tftIyopaDaH . (aqqituM aBiyoktumicCati
<>adq + san + law tip .) aqqiqizati . kvipi
<>saMyogAntalope ad . aBiyogaH samADAnaM .
<>aqqati pakzaM vidDAn . aBi samantAt yogo-
  [and 1 more lines]
```
------------------------------------------
 Case 0008: adyaSvonA  [page 1-035](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-035) 
```
skd:adyaSvonA,787:adyaSvonA:n:

 <HI>adyaSvonA, strI, (adyaSvas + “adyaSvInAvazwabDe”
<>iti sUtreRa Ka tasya Ina wilopaSca striyAM
<>wAp .) AsannaprasavA adyaSvo vA sUte adyaSvInA- [Pagē-035-c+ 52]
<>gOH iti saMkziptasArapariSizwaM .
```
------------------------------------------
 Case 0009: anaDyaMkzaH  [page 1-041](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-041) 
```
skd:anaDyaMkzaH,946:anaDyaMkzaH:n:

 <HI>anaDyaMkzaH, tri, (akzamindriyamaDigataH prAdi-
<>samAsaH, pratyakzaH tadBinnaH naYsamAsaH .) apra-
<>tyakzaH . ityamarawIkAyAM ramAnATaH .. aDyakzaBinnaH ..
```
------------------------------------------
 Case 0010: antaHsatvA  [page 1-053](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-053) 
```
skd:antaHsatvA,1226:antaHsatvA:n:

 <HI>antaHsatvA, strI, (antarmaDye sattvaM prARI sAro vA
<>yasyAH sA, striyAM wAp .) BallAtakaH . iti
<>SabdacandrikA .. garbBavatI . antarudaramaDye satvaM
<>prARI yasyAH sA ..
```
------------------------------------------
 Case 0011: apatTaM  [page 1-063](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-063) 
```
skd:apatTaM,1394:apatTaM:n:

 <HI>apatTaM, klI, (na patati vaMSo yasmAt, pata + bAhu-
<>lyAt yat, tato naYsamAsaH .) puttraH . kanyA .
<>tatparyyAyaH . 2 santAnaH . 3 tokaM . 4 santatiH .
<>5 prasUtiH . iti jawADaraH .
<>(“asmiMstu nirguRaM gotre nApatyamupajAyate”) . <>iti hitopadeSe .
  [and 2 more lines]
```
------------------------------------------
 Case 0012: abjaMH  [page 1-071](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-071) 
```
skd:abjaMH,1610:abjaMH:n:

 <HI>abjaMH, puM, (adByaH jAyate, ap + jan + qaH .)
<>candraH . DanvantariH . niculavfkzaH . iti medinI ..
```
------------------------------------------
 Case 0013: aByaNkzaH  [page 1-078](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-078) 
```
skd:aByaNkzaH,1764:aByaNkzaH:n:

 <HI>aByaNkzaH, puM, (aBi + akza + ac, pfzodarAditvAt
<>sADa .) tilakalkaH . iti fgvediSrAdDaprayogaH ..
```
------------------------------------------
 Case 0014: aruntadaH  [page 1-097](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-097) 
```
skd:aruntadaH,2134:aruntadaH:n:

 <HI>aruntadaH, tri, (arUMzi marmmARi tudati, tud +
<>KaS + mum ca .) marmmapIqakaH . tatparyyAyaH .
<>marmmaspfk 2 . ityamaraH .. (paruzaH . kaWoraH .
<>SravaRakawuH . “mAkandaM makarandatundilamamuM gA-
<>hasva kAkaH svayaM karRAruntudamantareRa ruRitaM
  [and 5 more lines]
```
------------------------------------------
 Case 0015: alikulasaqkulaH  [page 1-120](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-120) 
```
skd:alikulasaqkulaH,2347:alikulasaqkulaH:n:

 <HI>alikulasaqkulaH, puM, (alikulena saNkalaH vyAptaH,
<>tftIyAtatpuruzaH .) kubjakavfkzaH . iti rAja- <>nirGaRwaH ..
```
------------------------------------------
 Case 0016: allaH  [page 1-121](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-121) 
```
skd:allaH,2388:allaH:n:

 <HI>allaH, puM, (al + ka .) parameSvaraH .
<>etannAmnA yavanA upAsante . yaTA . “o~ asmallAM
<>ille mitrAvaruRo divyAni Datte . ilalle varuRo
<>rAjA punardaduH . hayAmi mitro illAM illalleti
<>illAllAM varuRo mitro tejakAmAH . hovAramindro
  [and 13 more lines]
```
------------------------------------------
 Case 0017: avakIrRo  [page 1-122](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-122) 
```
skd:avakIrRo,2398:avakIrRo:n:

 <HI>avakIrRo, [n] tri, (avakIrRamanena ava + ka
<>+ kta + ini, avakIrRaM DvastaM vratamiti SezaH,
<>asyAstIti .) kzatavrataH . strIsaMsargAdinA tyakta-
<>niyamavyaktiH . ityamarajawADarO .
<>(“kuSIlavo'vakIrRI ca vfzalIpatireva ca .
  [and 1 more lines]
```
------------------------------------------
 Case 0018: avamUrdDaSAyI  [page 1-127](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-127) 
```
skd:avamUrdDaSAyI,2500:avamUrdDaSAyI:n:

 <HI>avamUrdDaSAyI, [n] tri, (avamUrdDa + SI + Rini .)
<>aDomuKaSAyI . avamUrdDaM yaTA syAt taTA
<>SayituM SIlamasya ityarTe Rin pratyayena nizpannaH ..
```
------------------------------------------
 Case 0019: avaSyayaH  [page 1-128](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-128) 
```
skd:avaSyayaH,2546:avaSyayaH:n:

 <HI>avaSyayaH, strI, (ava + SyE + Ra . pfzodarAditvAt
<>hrasvaH .) kujJawikA . iti SabdaratnAvalI ..
<>(kujJawikASabde'sya viSezo jYeyaH .)
```
------------------------------------------
 Case 0020: avoDaH  [page 1-134](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-134) 
```
skd:avoDaH,2695:avoDaH:n:

 <HI>avoDaH, tri, (nAsti voDo jYAnaM yasya .) nirvvoDaH . <>ajYAnaH .
```
------------------------------------------
 Case 0021: aScaKurI  [page 1-144](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-144) 
```
skd:aScaKurI,2814:aScaKurI:n:

 <HI>aScaKurI, (strIM) aparAjitAlatA . iti Sabda- <>mAlA ..
```
------------------------------------------
 Case 0022: aSrudDaM  [page 1-136](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-136) 
```
skd:aSrudDaM,2751:aSrudDaM:n:

 <HI>aSrudDaM, tri, (na SudDam . naYtatpuruzaH .) apavitra .
<>akftaSoDanaM . aSOcayuktaM . yaTA . SaNKaH, -- [Pagē-137-a+ 52]
<>“tataH SrAdDamaSudDO tu kuryyAdekAdaSe taTA .
<>karttustAtkAlikI SudDiraSudDaH punareva saH” .. <>iti .
<>“SudDA BarttuScaturTe'hni aSudDA dEvapEtrayIH .
  [and 2 more lines]
```
------------------------------------------
 Case 0023: aSruBaM  [page 1-137](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-137) 
```
skd:aSruBaM,2752:aSruBaM:n:

 <HI>aSruBaM, klI, (na SuBam . naYtatpuruzaH . nAsti-
<>SuBaM yasyeti samAse vAcyaliNga eva .) pApaM .
<>iti hemacandraH .. amaNgalaM .
<>(“na ca kiYciduvAcEnaM SuBaM vA yadi vASuBam” .
<>mA ca vo'stvaSuBaM kiYcitsarvvaTA paRqunandanAH” ..
  [and 6 more lines]
```
------------------------------------------
 Case 0024: aSvinIkumArO  [page 1-147](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-147) 
```
skd:aSvinIkumArO,2865:aSvinIkumArO:n:

 <HI>aSvinIkumArO, puM, (aSvinyA aSvIBUtasaMjYAnAma-
<>sUryyapatnyAH yamajO kumArO .) aSvinIsutO . [Pagē-147-c+ 52]
<>aSvIBUtasaMjYAnAmasUryyapatnyAH yamajaputtrO . tO
<>devacikitsakO . (yaTA harivaMSe,
<>“vivasvAn kaSyApAjjajYe dAkzAyaRyAmarindama ! .
  [and 3 more lines]
```
------------------------------------------
 Case 0025: aSvinO  [page 1-147](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-147) 
```
skd:aSvinO,2868:aSvinO:n:

 <HI>aSvinO, puM, (praSastA aSvAH santi yayoH . iniH .
<>yadvA, aSvinyAm jAtO . sanDiveletyaRo nakza-
<>tremyo bahulamiti luki luktadvitalukIti NIpo
<>luk ..) aSvinIkumArO . ityamaraH ..
<>(“tvAzwrI tu saviturBAryyA vaqavArUpaDAriRI .
  [and 26 more lines]
```
------------------------------------------
 Case 0026: ahinakulatA  [page 1-159](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-159) 
```
skd:ahinakulatA,3132:ahinakulatA:n:

 <HI>ahinakulatA strI, ahinakulayorvEraM . nitya-
<>viroDiteti yAvat . ahiSca <>nakulaSca ahinakulO tayorBAva iti BAvArTe
<>ta-zRikapratyayayoH striyAmApA nizpannA ..
```
------------------------------------------
 Case 0027: ahibraDradevatA  [page 1-160](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-160) 
```
skd:ahibraDradevatA,3138:ahibraDradevatA:n:

 <HI>ahibraDradevatA, strI, (ahibraDno rudro'DizWAtrI
<>devatA yasyAH .) uttaraBAdrapadanakzatraM . iti jyo-
<>tiHSAstraM .. [Pagē-160-b+ 52]
```
------------------------------------------
 Case 0028: ahimuk  [page 1-160](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-160) 
```
skd:ahimuk,3141:ahimuk:n:

 <HI>ahimuk, [j] puM, (ahiM BuNkte . ahi + Buj +
<>kvip .) garuqaH . mayUraH .. ityamaraH .. nAkulI .
<>ganDanAkulI . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0029: AyurdruvyaM  [page 1-186](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-186) 
```
skd:AyurdruvyaM,3665:AyurdruvyaM:n:

 <HI>AyurdruvyaM, klI, (Ayaso dravyam .) OzaDaM . iti ratnamAlA ..
```
------------------------------------------
 Case 0030: AravI  [page 1-188](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-188) 
```
skd:AravI,3696:AravI:n:

 <HI>AravI, strI, AravanAmakamlecCadeSIyaBAzA . yaTA,
<>“jyezWASlezA maGA pUrbbA revatI BaraRIdvaye” .
<>viSAKArdrottarAzAQaSataBe pApavAsare ..
<>lagre sTire sacandre ca pArasImAravIM paWet” .
<>iti gaRapatimuhUrttaH ..
```
------------------------------------------
 Case 0031: AvoDanaM  [page 1-194](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-194) 
```
skd:AvoDanaM,3852:AvoDanaM:n:

 <HI>AvoDanaM, klI, (AN + vaD + lyuw .) jYAnam . <>budDiH ..
```
------------------------------------------
 Case 0032: ASvineyO  [page 1-195](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-195) 
```
skd:ASvineyO,3903:ASvineyO:n:

 <HI>ASvineyO, puM, (aSvinyA apatye . aSvinI + Qak .)
<>aSvinIkumArO . ityamaraH .. (pARquputtrO nakula-
<>sahadevanAmAnO, aSvinIkumArajAtatvAt tayo- <>staTAtvam .)
```
------------------------------------------
 Case 0033: iyaM  [page 1-213](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-213) 
```
skd:iyaM,4208:iyaM:n:

 <HI>iyaM, strI, idam Sabdasya strIliNge praTamAyAM rUpaM .
<>ei strI iti BAzA . iti vyAkaraRam ..
```
------------------------------------------
 Case 0034: izWArTodyuktaH  [page 1-215](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-215) 
```
skd:izWArTodyuktaH,4260:izWArTodyuktaH:n:

 <HI>izWArTodyuktaH, tri, (izwo'rTastasmin udyuktaH .)
<>utsukaH . utsAhayuktaH . ityamaraH ..
```
------------------------------------------
 Case 0035: IrikA  [page 1-216](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-216) 
```
skd:IrikA,4296:IrikA:n:

 <HI>IrikA, strI, vfkzaviSezaH . yaTA . IrikAvaRam .
<>iti RatvaprakaraRe durgAdAsaH ..
```
------------------------------------------
 Case 0036: uttamaPalino  [page 1-224](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-224) 
```
skd:uttamaPalino,4503:uttamaPalino:n:

 <HI>uttamaPalino, strI, (uttamaPalaM vidyate yasyAH .
<>uttamaPala + nin + NIp .) dugDikAvfkzaH . iti [Pagē-224-b+ 52]
<>ratnamAlA . kzIrAi iti BAzA ..
```
------------------------------------------
 Case 0037: udvanDanaM  [page 1-242](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-242) 
```
skd:udvanDanaM,4783:udvanDanaM:n:

 <HI>udvanDanaM, klI, (ut + vanDa + lyuw) UrdDabanDanam .
<>iti smftiH .. galAya daqi deona PA~sI deona <>wANgAna ityAdi BAzA ..
```
------------------------------------------
 Case 0038: undUraH  [page 1-243](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-243) 
```
skd:undUraH,4816:undUraH:n:

 <HI>undUraH, puM, (unda + Ura .) jantuviSezaH . idu~ra iti
<>BAzA . tatparyyAyaH . mUzikaH 2 AKuH 3 . kzadra- [Pagē-244-a+ 52]
<>Scet . girikA 4 bAlamUzikA 5 . neNwe idu~ra
<>iti BAzA . ityamaraH .. mUzakaH 6 mUzaH 7 mUzIkaH
<>8 undUruH 9 KanakaH 10 vaBruH 11 vfzaH 12 A-
  [and 6 more lines]
```
------------------------------------------
 Case 0039: upayozaM  [page 1-259](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-259) 
```
skd:upayozaM,4965:upayozaM:n:

 <HI>upayozaM, vya, upajozam . AnandaH . ityamarawIkAyAM <>BarataH ..
```
------------------------------------------
 Case 0040: uSIk  [page 1-275](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-275) 
```
skd:uSIk,5204:uSIk:n:

 <HI>uSIk, [j] puM, (vazwi uSyate vA . vaSa kAntO +
<>“vaSaH kit” . 2 . 71 . iti uRAdisUtreRa ijiH .
<>samprasAraRam .) agniH . GftaM . iti sidDAntakO-
<>mudyAmuRAdivfttiH ..
```
------------------------------------------
 Case 0041: UruparbbA  [page 1-279](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-279) 
```
skd:UruparbbA,5287:UruparbbA:n:

 <HI>UruparbbA, [n] puM, klI, (UroH parbbeva .) jAnu .
<>ityamaraH .. hA~wu iti BAzA .
```
------------------------------------------
 Case 0042: Urjjasvi  [page 1-280](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-280) 
```
skd:Urjjasvi,5295:Urjjasvi:n:

 <HI>Urjjasvi, [n] klI, (Urjas + vini .) alaNkAra-
<>viSezaH . tasya lakzaRam . sAhaNkAravastvaBiDA-
<>nam . taTA coktam . Urjasvi rUQAhaNkAram .
<>tasyodAharaRaM yaTA . “pracapalamaguruM BarAsa-
<>hizRuM janamasamAnamanUrjitam vivarjya . kftavasa-
  [and 2 more lines]
```
------------------------------------------
 Case 0043: fYjAsanaH  [page 1-284](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-284) 
```
skd:fYjAsanaH,5373:fYjAsanaH:n:

 <HI>fYjAsanaH, puM, (fji + “fjivfGisandisahiByaH
<>kit” . 2 . 87 . ityuRAdisUtreRa asAnac .
<>kicca .) meGaH . iti sidDAntakOmudI .. uRAdi- <>kozaSca ..)
```
------------------------------------------
 Case 0044: fRamatkaRaH  [page 1-284](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-284) 
```
skd:fRamatkaRaH,5376:fRamatkaRaH:n:

<HI>fRamatkaRaH, puM, (fRe matkuREva .) lagnakaH . iti [Pagē-284-b+ 52]
<>SabdaratnAvalI .. jAmina iti BAzA ..
```
------------------------------------------
 Case 0045: fRodgrAhaRaM  [page 1-284](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-284) 
```
skd:fRodgrAhaRaM,5384:fRodgrAhaRaM:n:

 <HI>fRodgrAhaRaM, klI, (fRasya udgrAhaRaM .) aDamarRa-
<>gfhItarRagrahaRaM . yatra fRaM prArTanakAle prArTito-
<>'pi fRI na dadAti tatra Danikena yat karttavyaM
<>tadAha manuH . 8 . 48 -- 49 .
<>“yEryErupAyErarTaM svaM prApnuyAduttamarRikaH .
  [and 68 more lines]
```
------------------------------------------
 Case 0046: FH  [page 1-290](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-290) 
```
skd:FH,5435:FH:n:

 <HI>FH, puM, (FRAti prApnoti viSvamiti . F + kvip .)
<>BEravaH . (yaTA, “FnandadAtri ! pramaTeSasaNgeH ! .”
<>ityudBawaH ..) danujaH . iti medinI .. <H>x
```
------------------------------------------
 Case 0047: XH  [page 1-290](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-290) 
```
skd:XH,5441:XH:n:

 <HI>XH, puM, sarvvaH . mahAdevaH . iti kaScidekAkzarakozaH .. <H>e
```
------------------------------------------
 Case 0048: ekAdgaviMSatiH  [page 1-296](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-296) 
```
skd:ekAdgaviMSatiH,5509:ekAdgaviMSatiH:n:

 <HI>ekAdgaviMSatiH, tri, (ekena na viMSatiH . ekAdi-
<>ScEkasya cAduk” . 6 . 3 . 76 . ityanena sidDam .)
<>ekonaviMSatiH 19 . iti vyAkaraRam ..
```
------------------------------------------
 Case 0049: La  [page 5-555](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-555) 
```
skd:La,42199:La:n:

 <HI>La, SezalakAraH . gOqadeSe asyAkAroccAraRayo-
<>rBedo nAsti . kintvasyADa ekabindumAtracihnaM
<>kriyate . vede asya uccAraRasTARaM jihvAmUlam .
<>devanAgaravarRamAlAyAM asyAkAraH evaM prakAraH
<>La . asya prayagaH . agnimIle . he indra
  [and 19 more lines]
```
------------------------------------------
 Case 0050: ESI  [page 1-301](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-301) 
```
skd:ESI,5619:ESI:n:

 <HI>ESI tri, (ISasya ISvarasya ca iyam .
<>ISa + ISvara + aR + NIp .) ISvara- <>sambanDI . ISvarera iti BAzA ..
```
------------------------------------------
 Case 0051: ESvaryyakarmmA  [page 1-301](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-301) 
```
skd:ESvaryyakarmmA,5622:ESvaryyakarmmA:n:

 <HI>ESvaryyakarmmA, [n] puM, (ESvaryyaM karmma yasya .) ISvara-
<>sambanDikarmmayuktaH . tatparyyAyaH . irajyati 1 pa-
<>tyate 2 kzayati 3 rAjati 4 . iti catvAra
<>ESvaryyakarmmARaH . iti vedanirGaRwO 2 aDyAyaH ..
```
------------------------------------------
 Case 0052: oH  [page 1-302](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-302) 
```
skd:oH,5630:oH:n:

 <HI>oH, puM, vrahmA . ityekAkzarakozaH ..
```
------------------------------------------
 Case 0053: o~  [page 1-302](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-302) 
```
skd:o~,5629:o~:n:

 <HI>o~, vya, om . praRavaH . iti vedAgamO ..
```
------------------------------------------
 Case 0054: O~  [page 1-304](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=1-304) 
```
skd:O~,5678:O~:n:

 <HI>O~, vya, SUdrARAM praRavaH . yaTA, --
<>“caturddaSasvaro yo'sO seturOkArasaMjYitaH .
<>sa cAnusvAranAdAByAM SudrARAM seturucyate” ..
<>iti tantrasAraDftakAlikApurARam ..
```
------------------------------------------
 Case 0055: kaNkamuKuH  [page 2-005](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-005) 
```
skd:kaNkamuKuH,5803:kaNkamuKuH:n:

 <HI>kaNkamuKuH, puM, (kaNkasya muKamiva muKaMyasya .) sandaMSaH .
<>iti hemacandraH .. sA~qASi iti BAzA . (klI,
<>nAlyodDaraRArTaH yantraBedaH . sa tu asTivinazwa-
<>SalyodDaraRArTamupadiSyate ..) asya praSaMsA yaTA,
<>“nirvvarttate sADvavagAhane ca <>SalyaM pragfhyodDarate ca yasmAt .
  [and 4 more lines]
```
------------------------------------------
 Case 0056: kacCAM  [page 2-007](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-007) 
```
skd:kacCAM,5854:kacCAM:n:

 <HI>kacCAM, strI, (kacaM paScAtpradeSaM CAdayatIti . Cad +
<>Ric + qaH pfzodarAditvAt hrasvaH . wAp ca .
<>kacaSCAdyate'nayA vA .) pariDAnAparAYcalam .
<>iti hemacandraH .. cIrikA . Ji~JI pokA iti
<>BAzA . vArAhI . iti medinI ..
```
------------------------------------------
 Case 0057: kaRqakinI  [page 2-014](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-014) 
```
skd:kaRqakinI,6056:kaRqakinI:n:

 <HI>kaRqakinI, strI, (kaRqakAH santyasyAH . iniH . Ip
<>ca .) vArttAkI . SoRaJiRwI . maDuKarjjurI . <>iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0058: kanqarpamuzalaH  [page 2-020](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-020) 
```
skd:kanqarpamuzalaH,6225:kanqarpamuzalaH:n:

 <HI>kanqarpamuzalaH puM, (kandarpasya muzala iva .) liNgaH . iti
<>trikARqaSezaH ..
```
------------------------------------------
 Case 0059: kandarI  [page 2-020](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-020) 
```
skd:kandarI,6220:kandarI:n:

 <HI>kandarI, strI, guhA . ityamaraH . 2 . 3 . 6 ..
```
------------------------------------------
 Case 0060: karRat  [page 2-037](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-037) 
```
skd:karRat,6606:karRat:n:

 <HI>karRat ka Bidi . iti kavikalpadrumaH .. (adanta-
<>cUrAM--paraM--sakaM--sew .) karRayati karRApayati .
<>ANpUrbbo'yaM SravaRe . devAkarRaya yena yena sahasA [Pagẹ-037-b+ 52]
<>yatyat samAsAditamiti . upasargAttattadarTAnAM
<>dyotakA eva na tu vAcakAH . kevalaDAtoranekA-
  [and 10 more lines]
```
------------------------------------------
 Case 0061: karRapUH  [page 2-038](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-038) 
```
skd:karRapUH,6624:karRapUH:n:

 <HI>karRapUH, [r] strI, (karRasya sUryyOrasajAtasya kuntyAH
<>praTamaputtrasya pUH puram .) karRarAjapuram . tatpa-
<>ryyAyaH . campA 2 mAlinI 3 lomapAdapUH 4 . <>iti hemacandraH ..
```
------------------------------------------
 Case 0062: kardramI  [page 2-044](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-044) 
```
skd:kardramI,6681:kardramI:n:

 <HI>kardramI, strI, mudgaravfkzaH . iti vEdyakam ..
```
------------------------------------------
 Case 0063: karmmoraH  [page 2-054](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-054) 
```
skd:karmmoraH,6747:karmmoraH:n:

 <HI>karmmoraH, puM, (karmma + Iran .) kirmmIravarRaH . itya-
<>marawIkA ..
```
------------------------------------------
 Case 0064: karvvUraH  [page 2-055](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-055) 
```
skd:karvvUraH,6756:karvvUraH:n:

 <HI>karvvUraH, puM, (karvva + KarjjUrAditvAt UraH . uRAM 1 .
<>82 .) rAkzasaH . SawI . ityamaraH .. 2 . 9 . 94 ..
```
------------------------------------------
 Case 0065: kiriwI  [page 2-128](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-128) 
```
skd:kiriwI,7736:kiriwI:n:

 <HI>kiriwI, [n] puM, (kirIwo'styasya . kirIwa +
<>iniH .) arjjunaH . iti trikARqaSezaH .
<>(yaTA, mahABArate . 4 . 42 . 17 ..
<>“purA SakreRa me dattaM yuDyato dAnavarzaBEH .
<>kirIwaM murdDni sUryyAmaM tenAhurmAM kirIwinam” ..)
  [and 4 more lines]
```
------------------------------------------
 Case 0066: kIrttimAR  [page 2-132](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-132) 
```
skd:kIrttimAR,7809:kIrttimAR:n:

 <HI>kIrttimAR [t] puM, (kIttirasyAstIti matup .)
<>vasudevajyezWaputtraH . (yaTA BAgavate 9 . 24 . 53 .
<>“vasudevastu devakyAM azwaputtrAnajIjanat .
<>kIrttimantaM suzeRaYca BadrasenamudAraDIH .
<>fjuM saMmardanaM BadraM saNkarzaRamahISvaram” ..)
  [and 3 more lines]
```
------------------------------------------
 Case 0067: kuqahuYcI  [page 2-138](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-138) 
```
skd:kuqahuYcI,7977:kuqahuYcI:n:

 <HI>kuqahuYcI, strI, (kuqI kzudrA huYcI kAravellI .) kzudra-
<>kAravellI . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0068: kuRalaH  [page 2-139](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-139) 
```
skd:kuRalaH,7992:kuRalaH:n:

 <HI>kuRalaH, puM, (kvaRa + “pIyukvaRiByAmiti” uRAM 3 .
<>76 . iti . kAlan . samprasAraRaYca .) deSaBedaH .
<>iti sidDAntakOmudyAmuRAdivfttiH .. (aSoka-
<>rAjaputtrezvekaH . sa ca bOdDaH ..)
```
------------------------------------------
 Case 0069: kokavanDuH  [page 2-200](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-200) 
```
skd:kokavanDuH,8941:kokavanDuH:n:

 <HI>kokavanDuH, puM, (kokayoH cakravAkayugalayordivasa-
<>praBayA saMyogakAritayA banDuriva .) sUryyaH . iti <>SabdacandrikA ..
```
------------------------------------------
 Case 0070: koqIkftiH  [page 2-220](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-220) 
```
skd:koqIkftiH,9306:koqIkftiH:n:

 <HI>koqIkftiH, strI, (kroqa + kf + BAve + ktin . aBUta-
<>tadBAve ciH .) AliNganam . iti hemacandraH ..
```
------------------------------------------
 Case 0071: kozeyaM  [page 2-211](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-211) 
```
skd:kozeyaM,9152:kozeyaM:n:

 <HI>kozeyaM, tri, (kOSeya + pfzodarAt zakArAdeSaH .)
<>kfmikozotTavastrAdi . resamI kApaqa iti
<>BAzA . ityamarawIkAyAM BarataH .. (yaTA, mArkaRqeya-
<>purARe . 15 . 27 . <>“kozakAraSca kOzeye hfte vastre'BijAyate” ..)
```
------------------------------------------
 Case 0072: kOmodo  [page 2-209](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-209) 
```
skd:kOmodo,9108:kOmodo:n:

 <HI>kOmodo, strI, (kuM pfTvIM modayati nandayatIti .
<>kumodaH vizRuH tasyeyaM iti aR tato NIp .)
<>vizRugadA . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0073: kzAradaSajaM  [page 2-232](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-232) 
```
skd:kzAradaSajaM,9479:kzAradaSajaM:n:

 <HI>kzAradaSajaM, klI, (daSaviDAH kzArAH atra . kap
<>kzArARAM daSakamityeke .) daSaviDakzAraH . yaTA, -- [Pagẹ-232-b+ 52]
<>“SigrumUlakapalASacukrikA- <>citrakArdrakasaMnimbasamBavEH .
<>ikzuSEKarikamocikodBatEH <>kzArapUrbbadaSakaM prakIrttitam” ..
<>iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0074: kzudrahiNgalikA  [page 2-247](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-247) 
```
skd:kzudrahiNgalikA,9691:kzudrahiNgalikA:n:

<HI>kzudrahiNgalikA, strI, (kzudrA hiNgulilA nityakarmma- [Pagẹ-247-b+ 52]
<>DArayaH .) kaRwakArI . iti SabdacandrikA ..
<>(guRAdayo'sya kaRwakArISabde bodDavyAH ..)
```
------------------------------------------
 Case 0075: kzuro  [page 2-250](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-250) 
```
skd:kzuro,9752:kzuro:n:

 <HI>kzuro, strI, (kzudraH kzuraH kzura iva vA . kzura + svalpArTe
<>NIp .) CurikA . iti hemacandraH .. kzurIti <>ca pAWaH ..
```
------------------------------------------
 Case 0076: kzvelI  [page 2-266](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-266) 
```
skd:kzvelI,9895:kzvelI:n:

 <HI>kzvelI, strI, (kzvela + gOrAditvAt NIz .)
<>krIqA . yaTA, SrIBAgavate . 10 . 29 . 46 .
<>“bAhuprasArapariramBakarAlakoru- <>nIvistanAlaBananarmmanaKAgrapAtEH .
<>kzvelyAvalokahasitErvrajasundarIRA-
<>muttamBayan ratipatiM ramayAYcakAra ..”
  [and 6 more lines]
```
------------------------------------------
 Case 0077: KajyotiH  [page 2-271](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-271) 
```
skd:KajyotiH,9932:KajyotiH:n:

 <HI>KajyotiH, [s] puM, (Ke AkASe jyotiryasya .)
<>KadyotaH . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0078: KadarikA  [page 2-275](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-275) 
```
skd:KadarikA,10023:KadarikA:n:

 <HI>KadarikA, strI, (KadireRa Kadirarasena samo raso- [Pagẹ-275-c+ 52]
<>'styasyAH . astyarTe + Wan tataH wAp .) lAkzA . <>iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0079: KaravallalikA  [page 2-277](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-277) 
```
skd:KaravallalikA,10076:KaravallalikA:n:

 <HI>KaravallalikA, strI, (KarA vallarI tataH svArTe
<>kan wAp ca IkArasya hrasvatvaM rasya latvaYca .)
<>nAgabalA . iti ratnamAlA .. gorakzacAuliyA <>iti BAzA ..
```
------------------------------------------
 Case 0080: KavAspaH  [page 2-279](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-279) 
```
skd:KavAspaH,10147:KavAspaH:n:

 <HI>KavAspaH, puM, (Ke AkASe SUnye sTito vAspaH .)
<>himam . iti hArAvalI ..
```
------------------------------------------
 Case 0081: KAnI  [page 2-282](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-282) 
```
skd:KAnI,10191:KAnI:n:

 <HI>KAnI, strI, (KAni + vA NIz .) KaniH . iti <>SabdaratnAvalI ..
```
------------------------------------------
 Case 0082: KemaRiH  [page 2-276](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-276) 
```
skd:KemaRiH,10041:KemaRiH:n:

 <HI>KemaRiH, puM, (Ke AkASe maRiriva jyotirmaya-
<>tvAt prakASakatvAdvA .) sUryyaH . iti trikARqa- <>SezaH ..
```
------------------------------------------
 Case 0083: gajitaH  [page 2-311](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-311) 
```
skd:gajitaH,10667:gajitaH:n:

 <HI>gajitaH, puM, (garjjo garjjanaM jAto'sya . jAtArTe
<>itac .) mattahastI . ityamaraH . 2 . 8 . 36 ..
```
------------------------------------------
 Case 0084: gaRezaBUzaRaM  [page 2-295](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-295) 
```
skd:gaRezaBUzaRaM,10412:gaRezaBUzaRaM:n:

 <HI>gaRezaBUzaRaM, klI, (gaReSasya BUzaRamiva .) sindU-
<>ram . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0085: garutmAn  [page 2-310](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-310) 
```
skd:garutmAn,10652:garutmAn:n:

 <HI>garutmAn, [t] puM, (garutaH pakzAH santyasya .
<>garut + matup .) garuqaH . ityamaraH . 1 . 1 . 31 .
<>(yaTA, BAgavate . 3 . 19 . 11 .
<>“jagrAha lIlayA prAptAM garutmAniva pannagIm ..”)
<>pakzimAtram . iti medinI . te . 196 ..
```
------------------------------------------
 Case 0086: garddaBarSAKI  [page 2-311](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-311) 
```
skd:garddaBarSAKI,10676:garddaBarSAKI:n:

 <HI>garddaBarSAKI, strI, (garddaBAKyA SAKA yasyAH .
<>yadvA garddaBapriyA SAKA asyAH jAtO saMjYAyAM
<>vA NIz .) BArgI . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0087: garBapAtanI  [page 2-314](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-314) 
```
skd:garBapAtanI,10700:garBapAtanI:n:

 <HI>garBapAtanI, strI, (garBapAtana + saMjYAyAM NIz .
<>gOrAditvAdityeke .) kalikArIvfkzaH . iti <>rAjanirGERwaH ..
```
------------------------------------------
 Case 0088: galakamvalaH  [page 2-318](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-318) 
```
skd:galakamvalaH,10732:galakamvalaH:n:

 <HI>galakamvalaH, puM, (gale kambala iva .) gavAM galasTita-
<>kambalAkftimAMsama . tatparyyAyaH . sAsnA 2 .
<>ityamaraH . 2 . 9 . 63 .. (“sAsnA
<>gogalakambalaH .” ityujjvaladattaH ..)
```
------------------------------------------
 Case 0089: gAtraBaNgAH  [page 2-322](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-322) 
```
skd:gAtraBaNgAH,10817:gAtraBaNgAH:n:

 <HI>gAtraBaNgAH, strI, (gAtrasya BaNgo'vasAdo yasyAH .)
<>SUkaSimbI . iti SabdacandrikA ..
```
------------------------------------------
 Case 0090: gArmBIryyaM  [page 2-324](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-324) 
```
skd:gArmBIryyaM,10847:gArmBIryyaM:n:

 <HI>gArmBIryyaM, klI, (gamBIrasya BAvo gamBIre BavaM
<>vA . “gamBIrAYYyaH .” 4 . 3 . 58 . iti <>YyaH .) gamBIratA . yaTA, --
<>“meGanirGozagArmBIryyaM pratinAdaviDAyitA .”
<>iti jinayAlguRe hemacandraH . [Pagẹ-324-c+ 52]
<>(idaM hi nAyakasya sAttvikaguRaviSezaH . yaTA,
  [and 7 more lines]
```
------------------------------------------
 Case 0091: gAloqanaM  [page 2-328](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-328) 
```
skd:gAloqanaM,10872:gAloqanaM:n:

 <HI>gAloqanaM, klI, (gAH indriyARi AlAqyante pramA-
<>dyante'nen . go + A + loqi + karaRe lyuw .
<>nipAtanAt sADuH .) unmAdaH . rogaH . mUrKa-
<>tvam . iti vakzyamARaSabdadarSanAt ..
```
------------------------------------------
 Case 0092: gAvO  [page 2-328](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-328) 
```
skd:gAvO,10875:gAvO:n:

 <HI>gAvO, puM, (goSabdasya dvivacanena nizpattiriti
<>voDyam .) grAmaH . yaTA--
<>“ekEkena taTA dbAByAM vacanEstriBireva ca .
<>sAkANkzatvaprayuktantu nAma paYcaviDaM smftam ..
<>ekavacanasAkANkzatvamuktaM ekaviMSatyAdiSabde .
  [and 5 more lines]
```
------------------------------------------
 Case 0093: gudANkaraH  [page 2-338](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-338) 
```
skd:gudANkaraH,11044:gudANkaraH:n:

 <HI>gudANkaraH, puM, (gude aNkuraH praroha iva gudamaDi-
<>kftya prarohavadutpadyate valirityarTaH .) arSo-
<>rogaH . iti hemacandraH .. 3 . 132 .
<>(“gudANkurA bahvanilAH .” iti vABawe nidAna-
<>sTAne saptame'DyAye ..) [Pagẹ-339-a+ 52]
```
------------------------------------------
 Case 0094: gftasaH  [page 2-348](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-348) 
```
skd:gftasaH,11168:gftasaH:n:

 <HI>gftasaH, puM, (gfDyati lipsatyaneneti . gfD aBi-
<>kANkzAyAm + “gfDipaRyordakO ca .” uRAM . [Pagẹ-348-b+ 52]
<>3 . 69 . iti saH sa ca kit dakArAntAdeSaSca .)
<>kandarpaH . ityuRAdikozaH .. (tri, stutyaH .
<>yaTA, fgvede . 7 . 87 . 5 .
  [and 2 more lines]
```
------------------------------------------
 Case 0095: gfDrARI  [page 2-349](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-349) 
```
skd:gfDrARI,11179:gfDrARI:n:

 <HI>gfDrARI, strI, (gfDra iva DUmravarRena aniti prARi-
<>tIti . an + pacAdyac . gOrAditvAt NIz .
<>saMjYAyAM Ratvam .) DUmrapatrAvfkzaH . iti rAja- <>nirGaRwaH ..
```
------------------------------------------
 Case 0096: gfhAH  [page 2-351](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-351) 
```
skd:gfhAH,11212:gfhAH:n:

 <HI>gfhAH, puM, (gfhRanti DyAnAdikaM iti . grah + “gehe
<>kaH .” 3 . 1 . 144 . iti kaH .) gfham . (yaTA,
<>manuH . 4 . 250 . [Pagẹ-351-c+ 52]
<>“SayyAM gfhAn kuSAn ganDAnapaH puzpaM maRIn daDi .
<>DAnA matsyAn payo mAMsaM SAkaYcEva na nirRudet ..”
  [and 2 more lines]
```
------------------------------------------
 Case 0097: gocarmma  [page 2-354](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-354) 
```
skd:gocarmma,11277:gocarmma:n:

 <HI>gocarmma, [n] klI, (goScarmma .) gavAM tvak .
<>karmmaviSeze tadAsanaviDiryaTA, --
<>“mezAsanantu vaSyArTamAkfzwO vyAGracarmma ca .
<>SAntO mfgAjinaM SastaM mokzArTaM vyAGracarmma ca ..”
<>gocarmmastamBane devi . samBave vAjicarmma ca ..”
  [and 8 more lines]
```
------------------------------------------
 Case 0098: godruvaH  [page 2-356](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-356) 
```
skd:godruvaH,11308:godruvaH:n:

 <HI>godruvaH, puM, (dravati sravatIti . dru + ac dravaH .
<>gavAM dravaH .) gomUtram . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0099: goDuk  [page 2-356](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-356) 
```
skd:goDuk,11319:goDuk:n:

 <HI>goDuk, [h] tri, (gAM dogDIti . go + duha + “satsu
<>dbizadruhaduhayujEtyAdi .” 3 . 2 . 61 . iti
<>kvip .) godogDA . iti vyAkaraRam .. (yaTA, <>fgvede . 1 . 164 . 26 .
<>“upa hvaye suduzAM DenumetAM <>suhasto goDuguta dohadenAm ..”)
```
------------------------------------------
 Case 0100: gomAn  [page 2-361](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-361) 
```
skd:gomAn,11394:gomAn:n:

 <HI>gomAn, [t] tri, (bahavo gAvo'syAsmin vA
<>santIti . “tadasyAstIti .” 5 . 2 . 94 . matup .)
<>bahUnAM gavAM svAmI . tatparyyAyaH . gavISvaraH 2
<>gomI 3 . ityamaraH . 2 . 9 . 58 .. (yaTA, <>aTarvvavede . 6 . 68 . 3 .
<>“yenAvapat savitA kzureRa <>somasya rAjYo varuRasya vidvAn .
  [and 1 more lines]
```
------------------------------------------
 Case 0101: gorARwikA  [page 2-363](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-363) 
```
skd:gorARwikA,11422:gorARwikA:n:

 <HI>gorARwikA, strI, (gorAwI + svArTe kan . wAp
<>pUrbbahrasvaSca . tato nipAtanAt sADuH . gorA-
<>wikA ityapi dfSyate .) SArikApakzI . iti <>rAjanirGaRwaH ..
```
------------------------------------------
 Case 0102: gORI  [page 2-370](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-370) 
```
skd:gORI,11503:gORI:n:

 <HI>gORI, strI, (guRaM sAdfSyamaDikftya pravfttA .
<>guRa + aRa + striyAM NIp .) aSItiprakAra-
<>lakzaRAntagetalakzaRAviSezaH . yaTA, --
<>“sAdfSyetarasambanDAH SudDAstAH sakalA api .
<>sAdfSyAttu matA gORyastena zoqaSa BeditAH ..”
  [and 44 more lines]
```
------------------------------------------
 Case 0103: gOtamasamBravA  [page 2-371](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-371) 
```
skd:gOtamasamBravA,11505:gOtamasamBravA:n:

 <HI>gOtamasamBravA, strI, (gOtamaH samBavo yasyAH .
<>yadvA, gOtamAt samBavatIti . saB + BU +
<>ac .) godAvarI nadI . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0104: grAmaRIH  [page 2-384](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-384) 
```
skd:grAmaRIH,11598:grAmaRIH:n:

 <HI>grAmaRIH, strI, (grAmeRa janasamUhenetyarTaH nIyate
<>gfhyate yadvA grAmeRa grAmyaDarmmeRa kevalaM mETunA-
<>dinA nayati kAlaM yApayatIti . nI + kvip .)
<>veSyA . grAmeyA . nIlikA . iti hemacandraH ..
```
------------------------------------------
 Case 0105: GawAlAyuH  [page 2-388](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-388) 
```
skd:GawAlAyuH,11682:GawAlAyuH:n:

 <HI>GawAlAyuH, strI, (GawAkArA alAvuH .) kumBa-
<>tumbI . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0106: Garvva  [page 2-392](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-392) 
```
skd:Garvva,11759:Garvva:n:

 <HI>Garvva, gatO . iti kavikalpadrumaH .. (BvAM-paraM-
<>sakaM-sew .) rePopaDaH . Darvvati . iti durgA- <>dAsaH ..
```
------------------------------------------
 Case 0107: GftatElAdikalpaH  [page 2-396](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-396) 
```
skd:GftatElAdikalpaH,11847:GftatElAdikalpaH:n:

 <HI>GftatElAdikalpaH, puM, (GftatElAdInAM rogavinA-
<>SakapakvaGftatElAdInAM kalpo biDiH .) roga-
<>nASakanAnOzaDipakvaGftatElAdivargaH . yaTA . <>DanvantariruvAca .
<>“GftatElAdi vakzyAmi SfRu suSruta ! rAganut .
<>SaNKapuzpI vaca somA brAhmI brahmasuvarccalA ..
  [and 50 more lines]
```
------------------------------------------
 Case 0108: GrARatapaRaH  [page 2-408](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-408) 
```
skd:GrARatapaRaH,11894:GrARatapaRaH:n:

 <HI>GrARatapaRaH, puM, (GrARamindriyaviSezaM tarpayatIti .
<>tfp + lyuH .) suganDiH . ityamaraH . 2 . 5 . 11 ..
<>(yaTA, rAmAyaRe . 2 . 94 . 14 .
<>GrARatapaRamaByetya kaM naraM na praharzayet ..”)
```
------------------------------------------
 Case 0109: catuHpalA  [page 2-419](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-419) 
```
skd:catuHpalA,12059:catuHpalA:n:

 <HI>catuHpalA, strI, (catvAri palAni yasyAM yasyA
<>vA .) nAgabalA . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0110: catustriMSajjAtajYaH  [page 2-423](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-423) 
```
skd:catustriMSajjAtajYaH,12106:catustriMSajjAtajYaH:n:

 <HI>catustriMSajjAtajYaH, puM, (catustriMSajjAtaM jAnAtIti .
<>jYA + “Ato'nupasarga kaH .” 3 . 2 . 3 . iti kaH .)
<>budDaBedaH . iti hemacandraH . 2 . 147 ..
```
------------------------------------------
 Case 0111: canca  [page 2-423](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-423) 
```
skd:canca,12118:canca:n:

 <HI>canca, u itO . iti kavikalpadrumaH .. (BvAM-paraM-
<>sakaM-sew . udittvAt ktrAvew .) u, caYcitvA
<>caktvA . itirgatiH . iti durgAdAsaH ..
```
------------------------------------------
 Case 0112: candragolasTAH  [page 2-428](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-428) 
```
skd:candragolasTAH,12147:candragolasTAH:n:

 <HI>candragolasTAH, puM, (candra eva golaH candragolaka-
<>sTAnaM candraloka ityarTaH . tatra tizWanti ye .)
<>divyapitaraH . iti trikARqaSezaH .. bahuvaca- <>nAnto'yaM SabdaH ..
```
------------------------------------------
 Case 0113: camyAluH  [page 2-434](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-434) 
```
skd:camyAluH,12229:camyAluH:n:

 <HI>camyAluH, puM, (campaScampakastadvat koSavarRaM AlAti
<>A + lA + quH .) panasaH . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0114: cikkaRI  [page 2-446](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-446) 
```
skd:cikkaRI,12482:cikkaRI:n:

 <HI>cikkaRI, strI, (cikkaRa + gOrAditvAt NIz .)
<>pUgaPalam . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0115: ciNgawI  [page 2-447](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-447) 
```
skd:ciNgawI,12486:ciNgawI:n:

 <HI>ciNgawI, strI, (ciNgawa + jAtO NIz .) matsya-
<>viSezaH . GuzAciNgaqI iti BAzA .. asyA
<>guRAH . maDuratvam . hfdyatvam . vAtanASitvam .
<>SlezmakAritvam . gurutvaYca . iti rAjavallaBaH ..
```
------------------------------------------
 Case 0116: cudqa  [page 2-456](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-456) 
```
skd:cudqa,12725:cudqa:n:

 <HI>cudqa, kftO . hAve . iti kavikalpadrumaH .. (BvAM-
<>paraM-sakaM-akaM ca-sew .) tavargatftIyopaDaH .
<>kvipi saMyogAntalope cud . kAtantrAdO hAva-
<>karaRa ityeko'rTo dfSyate . amI hAvAH kriyAH
<>SfNgAraBAvajAH . ityamaraH . BAva evAlpa-
  [and 2 more lines]
```
------------------------------------------
 Case 0117: cuMbraM  [page 2-456](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-456) 
```
skd:cuMbraM,12729:cuMbraM:n:

 <HI>cuMbraM, klI, (cumbyate'nenAtra vA . cubi cumbane
<>+ “fjrendrAgreti .” uRAM . 2 . 28 . iti
<>rapratyayo nakAralopaSca .) vadanam . ityu-
<>RAdikozaH .. [Pagẹ-456-c+ 52]
```
------------------------------------------
 Case 0118: cuva  [page 2-456](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-456) 
```
skd:cuva,12728:cuva:n:

 <HI>cuva, i ki cumbane . iti kavikalpadrumaH .. (curAM-
<>pakze BvAM-paraM-sakaM-sew .) cumbanaM muKasaMyogaH .
<>i, cumbyate . ki, cumbayati cumbati kAntADaraM
<>kAmI . priyAmuKaM kimpuruzaScucumbe .. iti
<>vyatIhAre Atmanepadam . iti durgAdAsaH ..
```
------------------------------------------
 Case 0119: CAH  [page 2-488](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-488) 
```
skd:CAH,12946:CAH:n:

 <HI>CAH, puM, (CAdyate mAtfpakzAByAmasO . Cad + “sarvva-
<>DAtuByo'sun .” uRAM . 4 . 188 . iti asun tataH
<>pfzodarAt sADuH .) SAvakaH . ityekAkzarakozaH ..
```
------------------------------------------
 Case 0120: jagaccakzuH  [page 2-497](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-497) 
```
skd:jagaccakzuH,13054:jagaccakzuH:n:

 <HI>jagaccakzuH, [s] puM, (jagatAM BuvanAnAM cakzuriva
<>prakASakatvAt .) sUryyaH . iti hemacandraH .
<>2 . 12 .. (yaTA, kASIKaRqe . 46 . 44 .
<>“iti kASIpraBAvajYo jagaccakzustamonudaH .
<>kftvA dbAdaSaDAtmAnaM kASIpuryyAM vyavasTitaH ..”)
```
------------------------------------------
 Case 0121: jaGanakUpakO  [page 2-502](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-502) 
```
skd:jaGanakUpakO,13078:jaGanakUpakO:n:

 <HI>jaGanakUpakO, puM, (jaGanakUpe iva kAyataH iti .
<>kE + kaH .) kukundarO . iti halAyuDaH .. <>dvivacanAnto'yaM SabdaH ..
```
------------------------------------------
 Case 0122: jattuH  [page 2-519](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-519) 
```
skd:jattuH,13314:jattuH:n:

 <HI>jattuH, puM, (jAyate yasmAditi . jana + “janestu
<>raH .” uRAM 5 . 46 . iti tuH rePaScAntA-
<>deSaH .) yoniH . hastI . ityuRAdikozaH ..
```
------------------------------------------
 Case 0123: jamajO  [page 2-512](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-512) 
```
skd:jamajO,13226:jamajO:n:

 <HI>jamajO, tri, (yamO ekatra sahacarO santO jAyete
<>iti . jana + qa . pfzodarAditvAt yasya jaH .)
<>yamajO . iti dbirUpakozaH ..
```
------------------------------------------
 Case 0124: jahilaH  [page 2-519](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-519) 
```
skd:jahilaH,13316:jahilaH:n:

 <HI>jahilaH, puM, araRyatilaH . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0125: jahnaH  [page 2-528](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-528) 
```
skd:jahnaH,13507:jahnaH:n:

 <HI>jahnaH, puM, (jahAtIti . hA + “jahAterdve anta-
<>lopaSca .” uRAM . 3 . 36 . iti nuH dvitva-
<>mantalopaSca .) vizRuH . rAjarziBedaH . sa
<>candravaMSodBavakururAjaputtraH . gaNgAgamanakAle yena
<>gaNgA pItA punarBagIraTaprArTanena UruM BittvA
  [and 1 more lines]
```
------------------------------------------
 Case 0126: jAdo  [page 2-531](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-531) 
```
skd:jAdo,13568:jAdo:n:

 <HI>jAdo, tri, (jAta ityasya jAdo itetyatprayogastu
<>prAkftalaNkeSvarasammata iti boDyam .) jAtaH .
<>iti prAkftalaNkeSvaraH ..
```
------------------------------------------
 Case 0127: jihvAmUlIvaH  [page 2-536](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-536) 
```
skd:jihvAmUlIvaH,13679:jihvAmUlIvaH:n:

 <HI>jihvAmUlIvaH, puM, (jihvAmUle BavaH . “jihvA-
<>mUlANguleSCaH .” 4 . 3 . 62 . iti CaH .)
<>vajrAkftivarRaH . vopadevenAsya mU iti saMjYA
<>kftA . sa tu kaKapare visargasTAne Bavati . yaTA
<>hari × kAmyaH . asyoccAraRaM visargavat . iti <>vyAkaraRam ..
  [and 3 more lines]
```
------------------------------------------
 Case 0128: jIvaYjIyaH  [page 2-539](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-539) 
```
skd:jIvaYjIyaH,13716:jIvaYjIyaH:n:

 <HI>jIvaYjIyaH, puM, (jIvaM jIvayati vizadozaM nASaya-
<>tIti . kftyalyuwo bahulamiti bAhulakAt
<>Kac .) cakorapakzI . ityamaraH . 2 . 5 . 35 ..
<>(aparaH pakziviSezaH . vizAdivikftasyAnnAdeH
<>parIkzArTamasyAvaSyakatvaM Bavati . yaTA, vABawe
  [and 4 more lines]
```
------------------------------------------
 Case 0129: jyezWI  [page 2-549](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-549) 
```
skd:jyezWI,13890:jyezWI:n:

 <HI>jyezWI, strI, gfhagoDA . jeWI iti wikwikI
<>iti ca BAzA . tatparyyAyaH . muzalI 2 musalI 3
<>kuqyamatsyA 4 gfhagoDikA 5 mUlI 6 wuk-
<>wukI 7 SakunajYA 8 gfhApikA 9 . iti
<>SabdaratnAvalI .. tasyAH patanaPalaM yaTA, --
  [and 11 more lines]
```
------------------------------------------
 Case 0130: jyotizkAH  [page 2-550](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-550) 
```
skd:jyotizkAH,13906:jyotizkAH:n:

 <HI>jyotizkAH, puM, (jyotirBiH kAyatIti . kE + kaH .)
<>candrArkagrahanakzatratArakAH . iti hemacandraH ..
<>bahuvacanAnto'yaM SabdaH ..
```
------------------------------------------
 Case 0131: Jatti  [page 2-568](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-568) 
```
skd:Jatti,13956:Jatti:n:

 <HI>Jatti, vya, (Jawiti iti Sabdasya prAkftaBAzAyAM
<>Jatti iti sidDam .) Jawiti . iti prAkfta-
<>BAzAyAM kAvyaprakASaH .. [Pagẹ-569-a+ 52]
```
------------------------------------------
 Case 0132: YiH  [page 2-571](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-571) 
```
skd:YiH,14028:YiH:n:

 <HI>YiH, puM, pratyayaviSezaH . sa preraRArTe Bavati .
<>tasya ikArastizWati . DAtoranubanDaviSezaH .
<>sa varttamAnaktapratyayaboDakaH . iti vopadeva- <>svAmI ..
```
------------------------------------------
 Case 0133: YyantaH  [page 2-571](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-571) 
```
skd:YyantaH,14029:YyantaH:n:

 <HI>YyantaH, puM, (YiH pratyayaviSezo'nte yasya .) Yi-
<>pratyayAntaH . yaTA, “YiYyantapibaTe” ityAdi
<>AtmanepadaprakaraRIyasUtram .. sa ca pratyayaH
<>GAtoH SabdAcca Bavati . yaTA, YiH preraRe
<>iti leH kftyAKyAne Yiriti ca mugDaboDa-
  [and 2 more lines]
```
------------------------------------------
 Case 0134: waNgiRI  [page 2-571](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-571) 
```
skd:waNgiRI,14050:waNgiRI:n:

 <HI>waNgiRI, strI, (waki banDe + RiniH . tataH pfzo-
<>darAt Ratve NIzi ca sADuH .) vfkzaviSezaH .
<>iti SabdacandrikA .. AkanAdi iti BAzA .. [Pagẹ-572-a+ 52]
```
------------------------------------------
 Case 0135: waWaWaM  [page 2-572](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-572) 
```
skd:waWaWaM,14079:waWaWaM:n:

 <HI>waWaWaM vya, anukaraRaSabdaH . Wan Wan iti
<>BAzA . yaTA, mahAnAwake . 96 . <>“rAmABizeke madavihvalAyAH
<>kakzAccyuto hemaGawastaruRyAH . <>sopAnamAruhya cakAra SabdaM
<>WaWaM WaWaM WaM WaWaWaM WaWaM CaH ..”
```
------------------------------------------
 Case 0136: WeraH  [page 2-572](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-572) 
```
skd:WeraH,14080:WeraH:n:

 <HI>WeraH, tri, vfdDaH . yaTA, jA WeraM vva hasantI .
<>iti kAvyaprakASe deSIBAzA .. <H>qa
```
------------------------------------------
 Case 0137: RATa  [page 2-576](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-576) 
```
skd:RATa,14172:RATa:n:

 <HI>RATa, f Na ASizi . davESye'rTe . iti kavi-
<>kalpadrumaH .. (BvAM-AtmaM-sakaM-akaM ca-sew .) [Pagẹ-576-c+ 52]
<>RopadeSaviDO varjjane'pyAdyasya pAWaH kasya
<>cidanuroDAt . f, ananATat . Na, praRATate .
<>dava upatApaH . ASIrizwArTasyASaMsanam .
  [and 9 more lines]
```
------------------------------------------
 Case 0138: RADa  [page 2-576](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-576) 
```
skd:RADa,14173:RADa:n:

 <HI>RADa, f Na nATe . iti kavikalpadrumaH .. (BvAM-
<>AtmaM-sakaM-sew .) RopadeSaviDAvasya varjjane-
<>'pi iha pAWaH kasyacidanuroDAt . f, ana-
<>nADat . Na, nADate nATo davASIrESvaryyArTa- <>nAni . iti durgAdAsaH ..
```
------------------------------------------
 Case 0139: tagdozAriH  [page 2-667](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-667) 
```
skd:tagdozAriH,15730:tagdozAriH:n:

 <HI>tagdozAriH, puM, (tagdozasya rogaviSezasya ariH
<>SatruH . tannASakatvAt taTAtvam .) hastikandaH .
<>iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0140: tanUhnadaH  [page 2-583](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-583) 
```
skd:tanUhnadaH,14328:tanUhnadaH:n:

 <HI>tanUhnadaH, puM, (tanvAH tanvAM vA hrada iva .) pAtyuH .
<>iti trikARqaSezaH ..
```
------------------------------------------
 Case 0141: tantrIH  [page 2-586](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-586) 
```
skd:tantrIH,14358:tantrIH:n:

 <HI>tantrIH, strI, (tantrayati mohayati lokAniti .
<>tantra + “avitFstftantriBya IH .” uRAM . 3 .
<>158 . iti IH .) vIRAguRaH . (yaTA, rAmA- <>yaRe . 2 . 39 . 29 .
<>“nAtantrIrvidyate vIRA nAcakro vidyate raTaH ..”
<>bIRA . yaTA, tatrEva . 1 . 2 . 18 .
  [and 7 more lines]
```
------------------------------------------
 Case 0142: taraRqI  [page 2-592](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-592) 
```
skd:taraRqI,14474:taraRqI:n:

 <HI>taraRqI, strI, (taratyanayeti . ta + aRqan . gOrA-
<>ditvAt NIz .) nOkA . iti SabdaratnAvalI ..
<>(taraRqA . iti hArAvalIDftapAWaH . 55 ..)
```
------------------------------------------
 Case 0143: tarIH  [page 2-598](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-598) 
```
skd:tarIH,14499:tarIH:n:

 <HI>tarIH, strI, (taratyanayA iti . tF + “avitFstf-
<>tantriBya IH .” uRAM . 3 . 158 . iti IH .)
<>nOkA . (yaTA, mAGe . 3 . 76 . <>“tarIzu tatratyamaPalguBARqam ..”)
<>gadA . vastrAdipewakaH . ityuRAdikozaH ..
<>DUmaH . iti trikARqaSezaH .. droRI . iti
  [and 1 more lines]
```
------------------------------------------
 Case 0144: tarIzI  [page 2-598](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-598) 
```
skd:tarIzI,14501:tarIzI:n:

 <HI>tarIzI, strI, (tarIza + saMjYAyAM NIz .) indra-
<>kanyA . iti medinI . ze, 38 ..
```
------------------------------------------
 Case 0145: tarmma  [page 2-600](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-600) 
```
skd:tarmma,14558:tarmma:n:

 <HI>tarmma, [n] klI, (taratIti . tF + “sarvva-
<>DAtuByo manin .” uRAM 4 . 144 . iti
<>manin .) yUpAgram . ityamaraH . 2 . 7 . 19 ..
```
------------------------------------------
 Case 0146: tAtajanayitryO  [page 2-603](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-603) 
```
skd:tAtajanayitryO,14625:tAtajanayitryO:n:

 <HI>tAtajanayitryO, strI, (tAtaSca janayitrI ca te .)
<>mAtApitarO . ityamaraH .. dbivacanAnto'yam ..
```
------------------------------------------
 Case 0147: tiTipraRIH  [page 2-617](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-617) 
```
skd:tiTipraRIH,14869:tiTipraRIH:n:

 <HI>tiTipraRIH, puM, (tiTiM praRayati hrAsavfdDirUpa-
<>kalAkriyayeti . pra + nI + kvip RatvaYca .)
<>candraH . iti hemacandraH . 3 . 18 ..
```
------------------------------------------
 Case 0148: timIH  [page 2-621](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-621) 
```
skd:timIH,14902:timIH:n:

 <HI>timIH, puM, (timiH pfzodarAditvAt sADuH .)
<>timimatsyaH . iti dvirUpakozaH .. (timi +
<>vA NIz . timijAtistrI . iti kecit ..)
```
------------------------------------------
 Case 0149: tiryyaYcI  [page 2-622](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-622) 
```
skd:tiryyaYcI,14921:tiryyaYcI:n:

 <HI>tiryyaYcI, strI, (tiryyaYca + striyAM NIp .)
<>tiraScI . strIpaSupakzyAdiH . iti mugDaboDam ..
```
------------------------------------------
 Case 0150: tITarAjI  [page 2-630](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-630) 
```
skd:tITarAjI,15004:tITarAjI:n:

 <HI>tITarAjI, strI, (tIrTAnAM rAjI SreRiriva .
<>yadvA, tIrTezu rAjate SoBate iti . rAja + in .
<>tato vAM NIz .) kASI . iti trikARqaSezaH ..
```
------------------------------------------
 Case 0151: tulI  [page 2-639](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-639) 
```
skd:tulI,15155:tulI:n:

 <HI>tulI, strI, (turI + rasya laH .) turI . iti <>SabdaratnAvalI ..
```
------------------------------------------
 Case 0152: tuvaraTAvanAlaH  [page 2-639](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-639) 
```
skd:tuvaraTAvanAlaH,15159:tuvaraTAvanAlaH:n:

 <HI>tuvaraTAvanAlaH, puM, (tuvaraH kazAyo yAvanAlaH .)
<>GAnyaviSezaH . tatparyyAyaH . tuvaraH 2 kazAya- [Pagẹ-640-a+ 52]
<>yAvanAlaH 3 raktayAvanAlaH 4 lohitakustumburu-
<>DAnyam 5 . asya guRAH . kazAyatvam . uzRa-
<>tvam . virecakatvam . saMgrAhitvam . vAtaSamana-
  [and 1 more lines]
```
------------------------------------------
 Case 0153: tfROkaH  [page 2-643](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-643) 
```
skd:tfROkaH,15275:tfROkaH:n:

 <HI>tfROkaH, [s] klI, (tfRanirmmitamoko vasati-
<>sTAnam .) kAyamAnam . tfRanirmmitagfham .
<>iti hemacandraH . 4 . 62 .. KaquyA Gara iti <>BAzA ..
```
------------------------------------------
 Case 0154: trapuzI  [page 2-653](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-653) 
```
skd:trapuzI,15448:trapuzI:n:

 <HI>trapuzI, strI, (trapuza + gOrAditvAt NIz .)
<>karkkawI . iti hemacandraH . 4 . 255 .. (yaTA,
<>“pawolakErvvArutrapuzyalAvUH ..”
<>iti hArIte praTamesTAne daSame'DyAye ..) <>dantyasAnto'pi pAWaH ..
```
------------------------------------------
 Case 0155: triKawwI  [page 2-655](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-655) 
```
skd:triKawwI,15508:triKawwI:n:

 <HI>triKawwI, strI, (tisfRAM KawwAnAM samAhAraH .
<>“dvigoH .” 4 . 1 . 21 . iti NIp .) triKa-
<>wwam . ityamaraH . 3 . 5 . 41 .. [Pagẹ-656-a+ 52]
```
------------------------------------------
 Case 0156: trowI  [page 2-665](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-665) 
```
skd:trowI,15685:trowI:n:

 <HI>trowI, strI, (trowi + kfdikArAditi vA NIz .)
<>trowiH . iti SabdaratnAvalI .. [Pagẹ-665-b+ 52]
```
------------------------------------------
 Case 0157: tryahapsarSaH  [page 2-665](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-665) 
```
skd:tryahapsarSaH,15697:tryahapsarSaH:n:

 <HI>tryahapsarSaH, puM, (tryahaM cAndradinatrayaM spfSatIti .
<>spfSa + pacAdyac .) tridinaspfk . tiTitraya-
<>sparSayuktamahaH . iti jyotizam ..
```
------------------------------------------
 Case 0158: tryArzeyAH  [page 2-666](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-666) 
```
skd:tryArzeyAH,15700:tryArzeyAH:n:

 <HI>tryArzeyAH, puM, (fzerayaM ArzeyaH . fzi + Qak .
<>traya ArzeyAH fziDarmmA netrAdIndriyakriyA-
<>BAvarUpA yezAm .) anDavaDiramUkAH . yaTA .
<>“atroktAni vacanAni sarvvARi tiryyagaDikaraRa-
<>nyAyamUlAni . taTA hi tiryyagaDikaraRe .
  [and 7 more lines]
```
------------------------------------------
 Case 0159: TurvvaRaM  [page 2-669](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-669) 
```
skd:TurvvaRaM,15767:TurvvaRaM:n:

 <HI>TurvvaRaM, klI, hananam . TurvvaDAtorBAve anawi
<>(lyuwi) nizpannam ..
```
------------------------------------------
 Case 0160: dakziRAruH  [page 2-672](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-672) 
```
skd:dakziRAruH,15809:dakziRAruH:n:

 <HI>dakziRAruH, [s] puM, (dakziRe dakziRaBAge arurvraRa-
<>Basya .) vyADakarttfkadakziRANgavraRitamfgaH .
<>ityamaraH . 2 . 10 . 24 ..
```
------------------------------------------
 Case 0161: daDisaktavaH  [page 2-681](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-681) 
```
skd:daDisaktavaH,15903:daDisaktavaH:n:

 <HI>daDisa(Sa)ktavaH, puM, daDyupasiktAH saktavaH . karamBaH .
<>ityamaraH . 2 . 9 . 48 .. dECAtu iti BAzA ..
<>nityabahuvacanAnto'yam .. (yaTA, mahABArate . <>13 . 104 . 91 .
<>“na pARO lavaRaM vidbAn prASIyAnna ca rAtrizu .
<>daDisaktUn na BuYjIta vfTAmAMsaYca varjayet ..”)
```
------------------------------------------
 Case 0162: damanakAropaRotsavaH  [page 2-685](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-685) 
```
skd:damanakAropaRotsavaH,15985:damanakAropaRotsavaH:n:

 <HI>damanakAropaRotsavaH, puM, (damanakasya Aropa-
<>RArTaM ya utsavaH .) SrIkfzRasya damanakArpaRArTa-
<>mahApUjArUpotsavaH . yaTA, --
<>“cEtrasya SukladbAdaSyAM damanAropaRotsavam .
<>vidaDyAttadbiDirbODAyanAdyukto'tra liKyate ..
  [and 54 more lines]
```
------------------------------------------
 Case 0163: dayAkUrcyaH  [page 2-686](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-686) 
```
skd:dayAkUrcyaH,15999:dayAkUrcyaH:n:

 <HI>dayAkUrcyaH, puM, (dayAyAM kUrcyaH SIrzamiva praDAnam .)
<>budDaH . iti hemacandraH . 2 . 148 ..
```
------------------------------------------
 Case 0164: dasrO  [page 2-697](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-697) 
```
skd:dasrO,16146:dasrO:n:

 <HI>dasrO, puM, (dasyataH kzipato rogAniti . dasa +
<>“sPAyitaYcIti .” uRAM . 2 . 13 . iti
<>rak .) aSvinO . ityamaraH . 1 . 1 . 54 ..
<>dvivacanAnto'yaM SabdaH .. (yaTA, BAvaprakASasya
<>pUrbbaKaRqe praTame BAge .
  [and 6 more lines]
```
------------------------------------------
 Case 0165: dAtrI  [page 2-699](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-699) 
```
skd:dAtrI,16193:dAtrI:n:

 <HI>dAtrI, strI, (dadAtIti . dA + tfc + NIp .)
<>dAnakartrI . iti vyAkaraRam .. (gaNgA . yaTA,
<>kASIKaRqe . 29 . 89 . tasyAH sahasranAmakIrttane .
<>“dInasantApaSamanI dAtrI davaTuvEriRI ..”)
```
------------------------------------------
 Case 0166: dAyAdI  [page 2-704](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-704) 
```
skd:dAyAdI,16230:dAyAdI:n:

 <HI>dAyAdI strI, (dAyaM attIti . ad + aR .
<>striyAM NIz .) kanyA . iti SabdArTakalpataruH ..
```
------------------------------------------
 Case 0167: dArAH  [page 2-705](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-705) 
```
skd:dArAH,16238:dArAH:n:

 <HI>dArAH, puM, (dArayanti BrAtfbanDUniti . dF + “dAra-
<>jArO karttari Riluk ca .” 3 . 3 . 20 .
<>ityasya vArttikoktyA GaY Riluk ca .) BAryyA .
<>bahuvacanAnto'yam . ityamaraH . 2 . 6 . 6 ..
<>(yaTA, mahABArate . 1 . 159 . 27 .
  [and 2 more lines]
```
------------------------------------------
 Case 0168: dikvaH  [page 2-709](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-709) 
```
skd:dikvaH,16325:dikvaH:n:

 <HI>dikvaH, puM, (diSi kAyate SabdAyate iti . kE +
<>kaH .) karaBaH . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0169: dikvaraH  [page 2-709](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-709) 
```
skd:dikvaraH,16326:dikvaraH:n:

 <HI>dikvaraH, puM, (diSaM AdeSaM karoti utsAhAnvi-
<>tatvAt . kf + waH .) yuvA . iti mAGawIkA-
<>DftakozaH .. (mahAdevaH . yaTA, kalikApurARe . <>82 aDvAye .
<>“dikkarastaruRaH proktastaTA SamBuSca dikkaraH ..”)
```
------------------------------------------
 Case 0170: dinapraRIH  [page 2-710](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-710) 
```
skd:dinapraRIH,16358:dinapraRIH:n:

 <HI>dinapraRIH, puM, (dinaM praRayatIti . pra + nI +
<>kvip .) sUryyaH . iti trikARqaSezaH .. (arka-
<>vfkzaH . sUryyaparyyAyatvAt ..)
```
------------------------------------------
 Case 0171: dizWaH  [page 2-713](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-713) 
```
skd:dizWaH,16441:dizWaH:n:

 <HI>dizWaH, puM, (diSatIti . diSa + saMjYAyAM ktaH .)
<>kAlaH . ityamaraH . 1 . 4 . 1 .. (vEvasvatamanoH
<>puttraviSezaH . yaTA, BAgavate . 8 . 13 . 2 .
<>“narizyanto'Ta nABAgaH saptamo dizwOcyate ..”)
<>dAruharidrA . iti SabdamAlA ..
```
------------------------------------------
 Case 0172: dIGatimizA  [page 2-721](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-721) 
```
skd:dIGatimizA,16530:dIGatimizA:n:

 <HI>dIGatimizA, strI, (dIrGastimizaH . aBiDAnAt
<>strItvam .) karkawI . iti SabdamAlA ..
```
------------------------------------------
 Case 0173: dIyamAnaH  [page 2-720](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-720) 
```
skd:dIyamAnaH,16505:dIyamAnaH:n:

 <HI>dIyamAnaH, tri, (dIyate iti . dA + karmmaRi
<>SAnac .) varttamAnadAnasambanDivastu . iti
<>vyAkaraRam . yaTA, kftyacintAmaRO .
<>“vivAhotsavayajYezu antarA mftasUtake .
<>pUrbbasaNkalpitaM dravyaM dIyamAnaM na duzyati ..”
  [and 1 more lines]
```
------------------------------------------
 Case 0174: duHkUlaH  [page 2-723](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-723) 
```
skd:duHkUlaH,16606:duHkUlaH:n:

 <HI>duHkUlaH, puM, (durduHKaM tajjanakaM kUlaM roDo yasmAt .
<>etadutpattyA tawasya viDnatvAt asya taTAtvam .)
<>coranAmaganDadravyam . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0175: duHKadohyAM  [page 2-723](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-723) 
```
skd:duHKadohyAM,16609:duHKadohyAM:n:

 <HI>duHKadohyAM, strI, (duHKena duhyate iti . duha +
<>“fhalorRyat .” 3 . 4 . 124 . iti Ryat .) [Pagẹ-723-c+ 52]
<>durdohagavI . tatparyyAyaH . karawA 2 . iti <>hemacandraH . 4 . 335 ..
```
------------------------------------------
 Case 0176: durnAma  [page 2-732](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-732) 
```
skd:durnAma,16704:durnAma:n:

 <HI>durnAma, [n] klI, (durduzwaM nAma yasya .) arSo-
<>rogaH . iti rAjanirGaRwaH .. (yaTA, suSrute . <>1 . 45 .
<>“daDyAjaM kaPapittaGnaM laGu vAtakzayApaham .
<>durnAmaSvAsakAsezu hitamagneH pradIpanam ..”
<>pittajaSlezmajayoranayorheturyaTA, --
  [and 28 more lines]
```
------------------------------------------
 Case 0177: durnAmnI  [page 2-732](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-732) 
```
skd:durnAmnI,16708:durnAmnI:n:

 <HI>durnAmnI, strI, (durninditaM nAma yasyAH . NIp .)
<>durnAmA . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0178: duzkarmma  [page 2-734](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-734) 
```
skd:duzkarmma,16746:duzkarmma:n:

 <HI>duzkarmma, [n] klI, (duzwaM karmma .) pApam . yaTA,
<>“duzkarmmajA nfRAM rogA yAnti cEva kamAcCamam .
<>japEH surArccanErhomErdAnEstezAM Samo Bavet ..”
<>iti SAtAtapIyakarmmavipAkaH ..
<>(ninditakarmma .. * .. duzwaM karmma asyeti vigrahe .
  [and 2 more lines]
```
------------------------------------------
 Case 0179: dusparSaH  [page 2-735](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-735) 
```
skd:dusparSaH,16764:dusparSaH:n:

 <HI>dusparSaH, puM, (duHKena spfSyate iti . dur + spfSa +
<>karmmaRi Kal . visargasya vA lopaH .) darAlaBA .
<>ityamarawIkAyAM BarataH .. (paryyAyo'sya yaTA,
<>“yAso yavAso dusparSo DanvayAsaH kunASakaH .
<>durAlaBA durAlamBA samudrAntA ca rodinI ..
  [and 2 more lines]
```
------------------------------------------
 Case 0180: dussTaH  [page 2-735](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-735) 
```
skd:dussTaH,16765:dussTaH:n:

 <HI>dussTaH, puM, (duHKena tizWatIti . duH + sTA +
<>kaH .) kukkuraH . kukkuwaH . iti SabdArTakalpa-
<>taruH .. (duHsTite, tri . yaTA, BawwO . 2 . 39 .
<>“balirbabanDe jalaDirmamanTe <>jahre'mftaM dEtyakulaM vijigye .
<>kalpAntadussTA vasuDA taTohe <>yenEza BAro'ti gururna tasya ..”)
```
------------------------------------------
 Case 0181: dfSI  [page 2-740](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-740) 
```
skd:dfSI,16888:dfSI:n:

 <HI>dfSI, strI, (dfSyate'nayeti . dfS + in . sa ca
<>kit . vA NIz .) cakzuH . iti Sabda- <>ratnAvalI .. (yaTA, --
<>“kiM samBUtaM rucirayordvijaSfNgayoste
<>maDye kfSo vahasi yatra dfSiH SritA me ..”
<>iti BAgavate . 5 . 2 . 12 .
  [and 11 more lines]
```
------------------------------------------
 Case 0182: devacikitsakO  [page 2-742](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-742) 
```
skd:devacikitsakO,16934:devacikitsakO:n:

 <HI>devacikitsakO, puM, (devAnAM cikitsakO .)
<>aSvinIkumArO . iti halAyuDaH .. dvivaca- <>nAnto'yam ..
```
------------------------------------------
 Case 0183: devadryaN  [page 2-743](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-743) 
```
skd:devadryaN,16960:devadryaN:n:

 <HI>devadryaN, [Yc] tri, (devAnaYcati pUjayatIti .
<>aYcu gatipUjanayoH + “ftvigdaDfk iti .”
<>3 . 2 . 59 . iti kvin . “vizvagdevayoSca
<>weradryaYcatAvapratyaye .” 6 . 3 . 92 . iti
<>adryAdeSaH .) devapUjakaH . devasamIpagantA .
  [and 1 more lines]
```
------------------------------------------
 Case 0184: devapUH  [page 2-744](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-744) 
```
skd:devapUH,16975:devapUH:n:

 <HI>devapUH, [r] strI, (devAnAM pUH purI .) amarA-
<>vatI . iti jawADaraH ..
```
------------------------------------------
 Case 0185: deSAKI  [page 2-748](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-748) 
```
skd:deSAKI,17054:deSAKI:n:

 <HI>deSAKI, strI, rAgiRIviSezaH . hanUmanmate
<>hindolarAgasya dvitIyarAgiRI . tasyA jAtiH
<>zAqazaH gfhaM gAnDArasvaraH gAnasamayaH vasa-
<>ntarttO pUrbbAhRaH . asyAH svarUpam . surUpA
<>nArI candravadanA kroDanA kalahapriyA romaharzA
  [and 11 more lines]
```
------------------------------------------
 Case 0186: drADimA  [page 2-759](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-759) 
```
skd:drADimA,17264:drADimA:n:

 <HI>drADimA, [n] puM, dIrGasya BAvaH . (dIrGa + ima-
<>nic . “priyasTirasPirorubahuleti .” 6 . 4 .
<>157 . iti drADAdeSaH .) dIrGatvam . iti
<>vyAkaraRam .. BUgolasya dIrGatA . ityADunika- <>jyotizikAH ..
```
------------------------------------------
 Case 0187: droRaduDA  [page 2-762](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-762) 
```
skd:droRaduDA,17337:droRaduDA:n:

 <HI>droRaduDA, strI, (droRaM dogDIti . duha + “duhaH
<>kap GaSca .” 3 . 2 . 70 . iti kap GaScAntAdeSaH .)
<>droRaparimitadugDadAtrI gavI . tatparyyAyaH .
<>droRakzIrA 2 . ityamaraH . 2 . 9 . 72 .. droRamAnA
<>3 droRaDA 4 payasvinI 5 . iti SabdacandrikA ..
  [and 2 more lines]
```
------------------------------------------
 Case 0188: droRaDA  [page 2-762](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-762) 
```
skd:droRaDA,17335:droRaDA:n:

 <HI>droRaDA, strI, (droRaduDA . pfzodarAditvAt
<>dulopaH .) droRaduDA . iti SabdacandrikA ..
```
------------------------------------------
 Case 0189: dvAHsTitadarSI  [page 2-764](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-764) 
```
skd:dvAHsTitadarSI,17371:dvAHsTitadarSI:n:

 <HI>dvAHsTitadarSI, [n] tri, (dvAri sTitaH san
<>paSyatIti . dfS + RiniH .) dvArapAlaH . iti
<>BarataDftaraBasaH ramAnATaSca ..
```
------------------------------------------
 Case 0190: dvAdaSAyuH  [page 2-765](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-765) 
```
skd:dvAdaSAyuH,17387:dvAdaSAyuH:n:

 <HI>dvAdaSAyuH, [s] puM, (dvAdaSavarzaparyyantaM Ayu-
<>ryasya .) kukkuraH . iti SabdamAlA ..
```
------------------------------------------
 Case 0191: dvitrAH  [page 2-769](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-769) 
```
skd:dvitrAH,17452:dvitrAH:n:

 <HI>dvitrAH, tri, (dvO vA trayo vA . “saMKyayAvyayA-
<>sannAdUrADikasaMKyAH saMKyeye .” 2 . 2 . 25 .
<>iti samAsaH . “bahubrIhO saMKyeye qajabahu-
<>gaRAt .” 5 . 4 . 73 . iti qac .) dbO vA
<>trayo vA ime . iti saMkziptasAraH . nityabahu-
  [and 2 more lines]
```
------------------------------------------
 Case 0192: dviSaH  [page 2-770](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-770) 
```
skd:dviSaH,17498:dviSaH:n:

 <HI>dviSaH, [s] vya, (dvi + “saMKyEkavacanAcca vIpsA-
<>yAm .” 5 . 4 . 43 . iti vIpsAyAM Sas .)
<>dvO dvO . iti vIpsArTe caSaspratyayaH .. (yaTA, <>suSrute . 1 . 41 .
<>“dviSo vA bahuSo vApi jYAtvA doze'vacA- <>rayet ..”) [Pagẹ-771-a+ 52]
```
------------------------------------------
 Case 0193: dvissvinnAnnaM  [page 2-771](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-771) 
```
skd:dvissvinnAnnaM,17509:dvissvinnAnnaM:n:

 <HI>dvissvinnAnnaM, klI, (dvissinnaM dviHpakvaM annaM
<>taRqulam .) dviHsidDataRqulam . yaTA, --
<>“dvisvinnamannaM pfTukaM SudDaM deSaviSezake .
<>nAtyantaSastaM viprARAM BakzaRe ca nivedane .. [Pagẹ-771-b+ 52]
<>aBakzyaYca yatInAYca viDavAbrahmacAriRAm .
  [and 2 more lines]
```
------------------------------------------
 Case 0194: DanASrIH  [page 2-778](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-778) 
```
skd:DanASrIH,17590:DanASrIH:n:

 <HI>DanASrIH, strI, rAgiRIviSezaH . DAnasI iti
<>BAzA . hanUmanmate SrIrAgasya tftIyaBAryyA .
<>asyAjAtiH zAqavaH . gfhaM zaqjasvaraH . gAna-
<>samayaH himarttO dbitIyapraharaH kasyacinmate
<>divAvasAnam . rAgamAlAyAmasyA rUpaM viyo-
  [and 5 more lines]
```
------------------------------------------
 Case 0195: DanuzmAn  [page 2-779](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-779) 
```
skd:DanuzmAn,17617:DanuzmAn:n:

 <HI>DanuzmAn, [t] puM, (Danurvidyate yasya . Danus +
<>matup .) DanurDaraH . ityamaraH . 2 . 8 . 69 .. [Pagẹ-780-a+ 52]
<>(yaTA, raGuH . 7 . 56 . <>“raTI nizaNgI kavacI DanuzmAn
<>dfptaH sa rAjanyakamekavIraH ..”)
```
------------------------------------------
 Case 0196: DImatI  [page 2-798](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-798) 
```
skd:DImatI,17875:DImatI:n:

 <HI>DImatI, strI, (DImat + striyAM NIp .) budDi-
<>matI . ityamaraH . 2 . 6 . 12 ..
```
------------------------------------------
 Case 0197: nakva  [page 2-813](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-813) 
```
skd:nakva,18142:nakva:n:

 <HI>nakva, ka nASane . iti kavikalpadrumaH .. (curAM-
<>paraM-sakaM-sew .) kopaDaH . nASanaM nazwIkara-
<>Ram . ka, nakvayati pApaM gaNgA . iti durgA- <>dAsaH ..
```
------------------------------------------
 Case 0198: naBaScakzuH  [page 2-826](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-826) 
```
skd:naBaScakzuH,18328:naBaScakzuH:n:

 <HI>naBaScakzuH, puM, (naBasaScakzuriva .) sUryyaH . iti <>SabdamAlA ..
```
------------------------------------------
 Case 0199: naBrAw  [page 2-827](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-827) 
```
skd:naBrAw,18351:naBrAw:n:

 <HI>naBrAw, [j] puM, (na BrAjate iti . BrAja dIptO
<>+ kvip .) meGaH . iti hemacandraH . 2 . 78 ..
```
------------------------------------------
 Case 0200: naladambuH  [page 2-836](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-836) 
```
skd:naladambuH,18435:naladambuH:n:

 <HI>naladambuH, puM, nimbavfkzaH . iti BUriprayogaH ..
```
------------------------------------------
 Case 0201: navagrahAH  [page 2-836](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-836) 
```
skd:navagrahAH,18455:navagrahAH:n:

 <HI>navagrahAH, puM, (nava grahAH .) sUryyAdayo nava .
<>yaTA, tiTitattve . <>“navagrahamaKaM kftvA tataH karmma samAraBet .
<>anyaTA PaladaM puMsAM na kAmyaM jAyate kvacit ..” <>(yaTA ca, --
<>“sUryyaScandro maNgalaSca buDaScApi bfhaspatiH .
<>SukraH SanEScaro rAhuH ketuSceti navagrahAH ..”)
  [and 141 more lines]
```
------------------------------------------
 Case 0202: navatiSaH  [page 2-838](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-838) 
```
skd:navatiSaH,18460:navatiSaH:n:

 <HI>navatiSaH, [s] vya, bahunavati . navati navatIti
<>vIpsAyAM caSaspratyayaH ..
```
------------------------------------------
 Case 0203: navatI  [page 2-838](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-838) 
```
skd:navatI,18461:navatI:n:

 <HI>navatI, strI, (navatiH + kfdikArAditi vA NIz .)
<>navatiH . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0204: nAganiryyahaH  [page 2-848](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-848) 
```
skd:nAganiryyahaH,18561:nAganiryyahaH:n:

 <HI>nAganiryyahaH, puM, (nAga iva niryyUho nAgadantaH .)
<>nAgadantakaH . iti jawADaraH ..
```
------------------------------------------
 Case 0205: nAmAparADI  [page 2-863](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-863) 
```
skd:nAmAparADI,18713:nAmAparADI:n:

 <HI>nAmAparADI, [n] tri, (nAmAparADo'stya-
<>syeti . iniH .) nAmAparADakft . yaTA, --
<>“nAmno janArddanasyAparADAn yaH kurute'niSam .
<>nAmAparADakflloko yAti satyamaDogatim ..
<>vinASayati nAmasTo'parADaM nAmato janaH .
  [and 6 more lines]
```
------------------------------------------
 Case 0206: nAsikyO  [page 2-874](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-874) 
```
skd:nAsikyO,18779:nAsikyO:n:

 <HI>nAsikyO, puM, (nAsikAyAM BavO . nAsikA +
<>“SarIrAvayavAt yat .” 5 . 1 . 6 . iti yat .)
<>aSvinIkumArO . dvivacanAnto'yam . iti <>hemacandraH . 2 . 96 ..
```
------------------------------------------
 Case 0207: niHSrayiRI  [page 2-874](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-874) 
```
skd:niHSrayiRI,18803:niHSrayiRI:n:

 <HI>niHSrayiRI, strI, (niHSrayati ASrayati prANga-
<>nAdisTAnamiti . Sri + Rini + NIp .) niHSra-
<>yaRI . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0208: nirjjitendrayagrAmaH  [page 2-892](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-892) 
```
skd:nirjjitendrayagrAmaH,19101:nirjjitendrayagrAmaH:n:

 <HI>nirjjitendrayagrAmaH, tri, (nirjjitaH parAjita
<>indriyagrAma indriyasamUho yena .) yatiH . itya-
<>maraH . 2 . 7 . 44 ..
```
------------------------------------------
 Case 0209: nizadvarI  [page 2-902](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-902) 
```
skd:nizadvarI,19305:nizadvarI:n:

 <HI>nizadvarI, strI, (ni + sada vizAde + aDikaraRe
<>zvarac . zittvAt NIp .) rAtriH . iti <>medinI . re, 273 ..
```
------------------------------------------
 Case 0210: nizkalI  [page 2-904](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-904) 
```
skd:nizkalI,19321:nizkalI:n:

 <HI>nizkalI, strI, (nizkala + NIz .) ftuhInA .
<>nivfttarajaskA . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0211: nizkuSitaH  [page 2-904](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-904) 
```
skd:nizkuSitaH,19330:nizkuSitaH:n:

 <HI>nizkuSitaH, tri, (nis + kuSa + ktaH .) nizkA- <>sitaH . yaTA, --
<>“kAkErnizkuSitaM SvaBiH kavalitaM vIcIBi- <>rAndolitam ..”
<>ityAdi vAlmIkikftagaNgAstotram ..
```
------------------------------------------
 Case 0212: nistrERapuzpikaH  [page 2-907](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-907) 
```
skd:nistrERapuzpikaH,19396:nistrERapuzpikaH:n:

 <HI>nistrERapuzpikaH, puM, rAjaDattUrakaH . iti rAja-
<>nirGaRwaH .. [Pagẹ-907-c+ 52]
```
------------------------------------------
 Case 0213: nihrutiH  [page 2-908](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-908) 
```
skd:nihrutiH,19415:nihrutiH:n:

 <HI>nihrutiH, strI, (ni + hru + ktin .) nihravaH . iti
<>SabdaratnAvalI .. [Pagẹ-908-b+ 52]
```
------------------------------------------
 Case 0214: nIcakEH  [page 2-908](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-908) 
```
skd:nIcakEH,19428:nIcakEH:n:

 <HI>nIcakEH, [s] vya, (nIcEs + “avyayasarvva-
<>nAmnAmakac prAk weH .” 5 . 3 . 71 . iti
<>weH prAgakac .) nIcEH . nIcEH Sabdasya weH
<>pUrbbakftenAkA nizpannaH . yaTA, uccakErityAdi ..
```
------------------------------------------
 Case 0215: nIcikI  [page 2-909](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-909) 
```
skd:nIcikI,19433:nIcikI:n:

 <HI>nIcikI, strI, nEcikI . ityamarawIkAyAM BarataH ..
```
------------------------------------------
 Case 0216: nfcakzuH  [page 2-918](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-918) 
```
skd:nfcakzuH,19573:nfcakzuH:n:

 <HI>nfcakzuH, [s] puM, (nFRAM prajAjanAnAM cakzuriva .)
<>sunITarAjaputtraH . iti mahABAratam .. (yaTA,
<>BAgavate . 9 . 22 . 41 .
<>“sunITastasya BavitA nfcakzuryat suKInalaH ..”
<>vizRupurARe tu . 4 . 21 . 3 .
  [and 1 more lines]
```
------------------------------------------
 Case 0217: nfSasaH  [page 2-920](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-920) 
```
skd:nfSasaH,19608:nfSasaH:n:

 <HI>nfSasaH, tri, (nFn narAn SaMsati hinastIti .
<>nf + Sansa hiMsAyAm + “karmmaRyaR .” 3 . 2 . 1 .
<>ityaR .) krUraH . paradrohI . ityamaraH . 3 . [Pagẹ-920-b+ 52]
<>1 . 47 .. (yaTA, paYcatantre . 3 . 142 .
<>“ye nfSaMsA durAtmAnaH prARinAM prARanASakAH .
  [and 1 more lines]
```
------------------------------------------
 Case 0218: nyakvAraH  [page 2-929](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-929) 
```
skd:nyakvAraH,19701:nyakvAraH:n:

 <HI>nyakvAraH, puM, (nyak kriyate iti . kf + GaY .)
<>nyakvaraRam . tatparyyAyaH . avajYA 2 parIhAraH 3
<>parihAraH 4 parABavaH 5 apamAnam 6 pari-
<>BavaH 7 tiraskAraH 8 tiraskiyA 9 avahelA
<>10 helA 11 avahelanam 12 helanam 13
  [and 8 more lines]
```
------------------------------------------
 Case 0219: pakzveqanA  [page 3-245](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-245) 
```
skd:pakzveqanA,22556:pakzveqanA:n:

 <HI>pakzveqanA, strI, (prakzveqana + wAp . yadvA, prakzve-
<>qanaM avyaktaSabdo'styasyA iti . ac . wAp .)
<>nArAcaH . ityamarawIkAyAM BagIraTaH ..
```
------------------------------------------
 Case 0220: paNkarvvawaH  [page 3-004](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-004) 
```
skd:paNkarvvawaH,19783:paNkarvvawaH:n:

 <HI>paNkarvvawaH, puM, (paNkezu karvvawo manoharaH .) jala-
<>yuktapaNkam . yaTA, -- <>“culuko GanajambAle dalAQye paNkakarvvawaH” .
<>iti trikARqaSezaH ..
```
------------------------------------------
 Case 0221: pacantI  [page 3-005](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-005) 
```
skd:pacantI,19827:pacantI:n:

 <HI>pacantI, strI, (odanAdIn pacati yA . pac +
<>Satf + striyAM NIp .) pAkakartrI . iti vyAka- <>raRam ..
```
------------------------------------------
 Case 0222: pacCaH  [page 3-006](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-006) 
```
skd:pacCaH,19834:pacCaH:n:

 <HI>pacCaH, [s] vya, (vIpsArTe pAdaM pAdamiti padBAvaH
<>tataH Sas .) pAdaM pAdam . iti saMkziptasAra- <>vyAkaraRam ..
```
------------------------------------------
 Case 0223: paYcakozAH  [page 3-006](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-006) 
```
skd:paYcakozAH,19846:paYcakozAH:n:

 <HI>paYcakozAH, puM, (paYca ca te kozASceti .) kozA
<>ivAtmAcCAdakatvena kozAH . te ca paYcaviDAH .
<>yaTA . annavikAratvAt sTUlaSarIraM annamaya-
<>kozaH 1 .. paYcakarmmendriyasahitaprARapaYcakaM prARa-
<>mayakozaH 2 .. paYcajYAnendriyasahitaM manaH mano-
  [and 16 more lines]
```
------------------------------------------
 Case 0224: paYcamahAyajYAH  [page 3-011](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-011) 
```
skd:paYcamahAyajYAH,19895:paYcamahAyajYAH:n:

 <HI>paYcamahAyajYAH, puM, (paYcaviDA mahAyajYAH .)
<>gfhasTakarttavyapaYcaprakAranityakarmmARi . yaTA, --
<>“pAWo homaScAtiTInAM saparyyA tarpaRaM valiH .
<>etEH paYca mahAyajYA brahmayajYAdinAmakEH ..”
<>ityamaraH . 2 . 7 . 14 .. <>(taTA ca manuH . 3 . 68-70 .
  [and 6 more lines]
```
------------------------------------------
 Case 0225: paYcayajYAH  [page 3-013](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-013) 
```
skd:paYcayajYAH,19906:paYcayajYAH:n:

 <HI>paYcayajYAH, puM, (paYcaviDA yajYAH .) gfhasTakarttavya-
<>paYcaprakArayajYaviSezAH . yaTA, --
<>“brahmayajYo nfyajYaSca dEvayajYaSca sattama ! .
<>pitfyajYo BUtayajYaH paYcayajYAH prakIrttitAH ..”
<>iti pAdme kriyAyogasAre 16 aDyAyaH .. <>SUdramaDikftya, --
  [and 2 more lines]
```
------------------------------------------
 Case 0226: paYcazAH  [page 3-015](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-015) 
```
skd:paYcazAH,19927:paYcazAH:n:

 <HI>paYcazAH, tri, paYca vA zaqvA parimARaM yezAM te .
<>bahuvacanAnto'yaM SabdaH . iti mugDaboDa- <>vyAkaraRam ..
```
------------------------------------------
 Case 0227: paYcasidDOzaDiH  [page 3-015](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-015) 
```
skd:paYcasidDOzaDiH,19928:paYcasidDOzaDiH:n:

 <HI>paYcasidDOzaDiH, puM, (paYca sidDA OzaDayo yasmin .)
<>paYcaprakArOzaDiviSezaH . yaTA, --
<>“tElakandasuDAkandakroqakandarudantikAH .
<>sarpanetrayutAH paYcasidDOzaDikasaMjYakaH ..” <>iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0228: paYcAmarAH  [page 3-016](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-016) 
```
skd:paYcAmarAH,19942:paYcAmarAH:n:

 <HI>paYcAmarAH, strI, (paYca amarAH . saMjYAtvAtkarmma-
<>DArayaH .) paYcaprakArAmaralatAdi . yaTA, --
<>“ekA tu amarA dUrvvA tasyA granTiM samAnayet .
<>anyA tu vijayA devI sidDirUpA sarasvatI ..
<>anyA tu vilvapatrasTA SivasantozakAriRI .
  [and 4 more lines]
```
------------------------------------------
 Case 0229: paYcozmARaH  [page 3-017](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-017) 
```
skd:paYcozmARaH,19962:paYcozmARaH:n:

 <HI>paYcozmARaH, puM, (paYca uzmARaH .) AhArapAcaka-
<>SarIrasTapaYcAgnayaH . yaTA, sArakOmudyAma .
<>“BOmApyAgneyavAyavyAH paYcozmARaH sanABasAH .
<>paYcAhAraguRAnsvAnsvAn pArTivAdIn pacantyanu .
<>pArTivAH pArTivAneva SezAH SezAMSadehagAn ..”
```
------------------------------------------
 Case 0230: patraDanA  [page 3-028](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-028) 
```
skd:patraDanA,20126:patraDanA:n:

 <HI>patraDanA, strI, (patrameva DanaM yasyAH . patra-
<>bAhulyAttaTAtvam .) sAtalAvfkzaH . iti rAja- <>nirGaRwaH ..
```
------------------------------------------
 Case 0231: patrapA  [page 3-028](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-028) 
```
skd:patrapA,20135:patrapA:n:

 <HI>patrapA, strI, (apatrapaRamiti . apa + trap + ac .
<>nipAtanAt akAralopaH .) apatrapA . lajjA . <>iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0232: padDatI  [page 3-040](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-040) 
```
skd:padDatI,20218:padDatI:n:

 <HI>padDatI, strI, (padByAM hanti gacCatIti . hana
<>gatO + ktin . “himakAzihatizu
<>ca .” 6 . 3 . 54 . iti padBAvaH . “bahvAdiByaSca .”
<>4 . 1 . 45 . iti vA NIz .) vartma . (yaTA, raGuH <>3 . 46 .
<>“paTaH SruterdarSayitAra ISvarAH <>malImasAmAdadate na padDatim ..”)
  [and 10 more lines]
```
------------------------------------------
 Case 0233: panTAH  [page 3-045](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-045) 
```
skd:panTAH,20285:panTAH:n:

 <HI>panTAH, [in] puM, patanti yAntyanena . (pata x
<>gatO + “patasTa ca .” uRAM . 4 . 12 . iti
<>iniH TaScAntAdeSaH .) raTyA . rAstA iti
<>BAzA .. (yaTA, manuH . 4 . 45 .
<>“na mUtraM paTi kurvvIta na Basmani na govraje ..”)
  [and 37 more lines]
```
------------------------------------------
 Case 0234: papIH  [page 3-046](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-046) 
```
skd:papIH,20295:papIH:n:

 <HI>papIH, puM, (pAti lokam . pA rakzaRe + “yApoH
<>kit dve ca .” uRAM . 3 . 159 . iti Ik
<>dvitvaYca .) sUryyaH . candraH . ityuRAdikozaH ..
```
------------------------------------------
 Case 0235: paraHSvaH  [page 3-048](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-048) 
```
skd:paraHSvaH,20328:paraHSvaH:n:

 <HI>paraHSvaH, [s] vya, Svo dinAt paramahaH paraHSvaH
<>paraH sahasravat pAraskarAditvAt suw paraHSva
<>iti maDuH . suwaM necCantyanye paraSva ityAhuH .
<>atikrAnte pUrbbatare dine paraSvo jAta iti
<>prayogo gORaH tatra paraYca tat SvaSceti karmma-
  [and 2 more lines]
```
------------------------------------------
 Case 0236: parivrAw  [page 3-065](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-065) 
```
skd:parivrAw,20604:parivrAw:n:

 <HI>parivrAw, [j] puM, (parityajya sarvvaM vizaya-
<>BogaM gfhasTASramAd vrajati
<>gacCatIti . pari + vraj + kvip dIrGaSca .)
<>caturTASramI . tatparyyAyaH . BikzuH 2 karmmandI 3
<>pArASarI 4 maskarI 5 . ityamaraH . 2 . 7 . 42 ..
  [and 9 more lines]
```
------------------------------------------
 Case 0237: paryyudaccaM  [page 3-073](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-073) 
```
skd:paryyudaccaM,20745:paryyudaccaM:n:

 <HI>paryyudaccaM, klI, (paryyudacyate iti . pari + ut +
<>aYca + “kftyalyuwo bahulam .” 3 . 3 . 113 .
<>iti lyuw .) fRam . ityamaraH . 2 . 9 . 3 .. [Pagė-074-a+ 52]
```
------------------------------------------
 Case 0238: parzadbalaH  [page 3-078](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-078) 
```
skd:parzadbalaH,20789:parzadbalaH:n:

 <HI>parzadbalaH, tri, (parzat saBA vidyate yasya . parzat
<>+ “rajaHkfzIti .” 5 . 2 . 112 . iti valac .)
<>pArizadaH . iti SabdaratnAvalI .. (yaTA, <>BawwiH . 4 . 12 .
<>“vrAtInavyAladIptAstraH sutvanaH paripUjayan .
<>parzadvalAn mahAbrahmErAwa nEkawikASramAn ..”)
```
------------------------------------------
 Case 0239: pavrajitaH  [page 3-297](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-297) 
```
skd:pavrajitaH,23166:pavrajitaH:n:

 <HI>pavrajitaH, puM, (pra + vraj + ktaH .) budDaBikzu-
<>SizyaH . tatparyyAyaH . celukaH 2 SrAmaReraH 3
<>mahApASakaH 4 gomI 5 . iti trikARqaSezaH ..
<>tri pravrajyASramaviSizwaH .. (yaTA, parASara- <>saMhitAyAm . 4 . 26 .
<>“nazwe mfte pravrajite klIve ca patite patO .
  [and 5 more lines]
```
------------------------------------------
 Case 0240: paSyatikarmmA  [page 3-087](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-087) 
```
skd:paSyatikarmmA,20902:paSyatikarmmA:n:

 <HI>paSyatikarmmA, [n] puM, (paSyatirdarSanameva karmma
<>yasya .) darSanakarmmA . tadvEdikaparyyAyAH .
<>cikyAt 1 cAkanat 2 Acakzma 3 cazwe 4
<>vicazwe 5 vicarzaRiH 6 viSvacarzaRiH 7 ava-
<>cAkaSat 8 . ityazwO paSyatikarmmARaH . iti <>vedaniGaRwO 3 aDyAyaH ..
```
------------------------------------------
 Case 0241: pAdaSaH  [page 3-111](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-111) 
```
skd:pAdaSaH,21149:pAdaSaH:n:

 <HI>pAdaSaH, [s] vya, pAdaM pAdam . pAdaSabdAt
<>vIpsAyAM caSaspratyayanizpannam . iti vyAkara-
<>Ram .. (yaTA, manuH . 1 . 83 .
<>“arogAH sarvvasidDArTAScaturvvarzaSatAyuzaH .
<>kfte tretAdizu hyezAmAyurhnasati pAdaSaH ..”)
```
------------------------------------------
 Case 0242: pArSvataH  [page 3-130](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-130) 
```
skd:pArSvataH,21340:pArSvataH:n:

 <HI>pArSvataH, [s] vya, (pArSva + “AdyAdiBya upa-
<>saMKyAnam .” 5 . 4 . 44 . ityasya vArtti-
<>koktyA tasiH .) pArSvAt . pArSve . ityAdi .
<>iti saMkziptasAravyAkaraRam .. (yaTA, raGuH . <>19 . 31 .
<>“mitrakftyamapadiSya pArSvataH <>prasTitaM tamanavasTitaM priyAH ..”)
```
------------------------------------------
 Case 0243: piRqamustAM  [page 3-141](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-141) 
```
skd:piRqamustAM,21523:piRqamustAM:n:

 <HI>piRqamustAM, strI, (piRqavat sTUlA mustA .)
<>nAgaramustA . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0244: pitarO  [page 3-143](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-143) 
```
skd:pitarO,21553:pitarO:n:

 <HI>pitarO, puM, janakajanayitryO . mAtA ca pitA
<>ceti vigrahe vikalpAdekaSeze pitarAviti
<>prAYcaH . iti BarataH .. tatparyyAyaH . mAtA-
<>pitarO 2 mAtarapitarO 3 tAtajanayitryO 4 .
<>ityamaraH . 2 . 6 . 37 .. (yaTA, raDuH . 1 . 1 .
  [and 9 more lines]
```
------------------------------------------
 Case 0245: puMScalaH  [page 3-161](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-161) 
```
skd:puMScalaH,21770:puMScalaH:n:

 <HI>puMScalaH, puM, (puMScalIva . upacArAt puMstvam .)
<>vyaBicArI . yaTA, gAruqe 66 aDyAye .
<>“lalAwopasftAstisro reKAH syuH SatavarziRAm .
<>nfpatvaM syAccatasfBirAyuH paYcanavatyaTa ..
<>areKeRAyurnavatirvicCinnABiSca puMScalAH ..” [Pagė-161-b+ 52]
```
------------------------------------------
 Case 0246: puttrezWikA  [page 3-170](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-170) 
```
skd:puttrezWikA,21873:puttrezWikA:n:

 <HI>puttrezWikA, strI, (puttrezwi + svArTe kan wAp ca .)
<>puttranimittakayAgaviSezaH . iti jawADaraH ..
```
------------------------------------------
 Case 0247: puttrO  [page 3-170](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-170) 
```
skd:puttrO,21874:puttrO:n:

 <HI>puttrO, puM, (duhitA ca puttraSca puttrO ityekaSezaH .)
<>puttraduhitarO . ityamaraH . 2 . 6 . 37 .. <>dvivacanAnto'yam ..
```
------------------------------------------
 Case 0248: punaHpunaH  [page 3-170](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-170) 
```
skd:punaHpunaH,21882:punaHpunaH:n:

 <HI>punaHpunaH [r] vya, (punar + vIpsAyAM dvitvam .)
<>vAraMvAram . tatparyyAyaH . muhuH 2 SaSvat 3
<>aBIkzRam 4 asakft 5 . ityamaraH . 3 . 4 . 1 ..
<>vAraMvAreRa 6 pOnaHpunyam 7 pratikzaRam 8 .
<>iti SabdaratnAvalI .. (yaTA, cARakye .
  [and 2 more lines]
```
------------------------------------------
 Case 0249: purataH  [page 3-174](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-174) 
```
skd:purataH,21915:purataH:n:

 <HI>purataH, [s] vya, (purati agre gacCatIti . pura +
<>bAhulakAt atasuc .) agrataH . ityamaraH . 3 .
<>4 . 7 .. (yaTA, rAjataraNgiRyAm . 1 . 107 .
<>“nirgate maYjarIkuYjAdapaSyat puratastataH .
<>kanye nIlanicolinyO sa keciccArulocane ..”)
```
------------------------------------------
 Case 0250: puroqAH  [page 3-197](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-197) 
```
skd:puroqAH,21989:puroqAH:n:

 <HI>puroqAH, [S] puM, (pura AdO dASyate dIyate
<>iti . puras + dASf dAne + kvip . nipAta-
<>nAt dasya qaH .) haviH . iti mugDaboDa- <>vyAkaraRam ..
```
------------------------------------------
 Case 0251: puzkarasrajO  [page 3-200](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-200) 
```
skd:puzkarasrajO,22031:puzkarasrajO:n:

 <HI>puzkarasrajO, puM, (puzkarasya padmasya srak yayoriti .)
<>aSvinIkumArO . dbivacAnto'yam . iti Sabda-
<>ratnAvalI .. (aSvinIkumAratulye, tri . yaTA,
<>vAjasaneyasaMhitAyAm . 2 . 33 .
<>“ADatta pitaro garBaM kumAraM puzkarasrajam . <>yaTeha puruzo'sat ..”
  [and 6 more lines]
```
------------------------------------------
 Case 0252: puzpavantO  [page 3-207](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-207) 
```
skd:puzpavantO,22077:puzpavantO:n:

 <HI>puzpavantO, puM, (puzpa vikAse + BAve GaY . puzpo
<>vikAso'styanayoriti . puzpa + matup . masya
<>vaH .) ekayoktyA candrasUryyO . ityamaraH . 1 . 4 . 10 ..
<>“ekayA uktyA apfTagvacanena candrArkO puzpa-
<>vacCabdavAcyO na tu puzpavAninduH sUryyo vetyaBi-
  [and 5 more lines]
```
------------------------------------------
 Case 0253: pUrvvajAH  [page 3-222](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-222) 
```
skd:pUrvvajAH,22226:pUrvvajAH:n:

 <HI>pUrvvajAH, puM, (pUrvve pUrvvasmin kAle jAyante iti .
<>jana + qaH .) divyapitfgaRaH . tatparyyAyaH .
<>pitaraH 2 candragolasTAH 3 nyastaSastrAH 4
<>svaDABujaH 5 kavyavAlAdayaH 6 . bahuvacanAnto-
<>'yam . iti trikARqaSezaH ..
```
------------------------------------------
 Case 0254: pUrvvaBadrapadAH  [page 3-223](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-223) 
```
skd:pUrvvaBadrapadAH,22240:pUrvvaBadrapadAH:n:

 <HI>pUrvvaBadrapadAH, strI, aSvinyAdisaptaviMSatinakza
<>trAntargatapaYcaviMSanakzatram . <>tatparyyAyaH . prozWapadAH 2 .
<>ityamaraH .. “dve pUrbbaBAdrapadottaraBAdrapadAsu .
<>prozWo gOH BadraSca gOH tasyeva yAda AsAM
<>tAstaTA . grahapatiriva BadrapAdAnugato rAje-
  [and 10 more lines]
```
------------------------------------------
 Case 0255: pUrvvaBAdrapadAH  [page 3-223](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-223) 
```
skd:pUrvvaBAdrapadAH,22241:pUrvvaBAdrapadAH:n:

 <HI>pUrvvaBAdrapadAH, strI, aSvinyAdisaptaviMSatinakza
<>trAntargatapaYcaviMSanakzatram . <>tatparyyAyaH . prozWapadAH 2 .
<>ityamaraH .. “dve pUrbbaBAdrapadottaraBAdrapadAsu .
<>prozWo gOH BadraSca gOH tasyeva yAda AsAM
<>tAstaTA . grahapatiriva BadrapAdAnugato rAje-
  [and 19 more lines]
```
------------------------------------------
 Case 0256: pUrvvAH  [page 3-223](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-223) 
```
skd:pUrvvAH,22253:pUrvvAH:n:

 <HI>pUrvvAH, puM, pUrvvajAH . pUrvvapuruzAH . bahuvacanAnto-
<>'yam . ityamaraH . 3 . 3 . 133 .. (yaTA, raDuH . 1 . 67 .
<>“matparaM durlaBaM matvA nUnamAvarjjitaM mayA .
<>payaH pUrbbEH svaniSvAsEH kavozRamupaBujyate ..”)
```
------------------------------------------
 Case 0257: pfTakkzetrAH  [page 3-225](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-225) 
```
skd:pfTakkzetrAH,22285:pfTakkzetrAH:n:

 <HI>pfTakkzetrAH, puM, (pfTak BinnaM kzetraM utpattisTAnaM
<>yezAmiti .) ekasmAnnAnAvarRastrIjAtaputtrAH . <>iti mitAkzarA ..
```
------------------------------------------
 Case 0258: pfzWataH  [page 3-231](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-231) 
```
skd:pfzWataH,22354:pfzWataH:n:

 <HI>pfzWataH, [s] vya, (pfzWa + “pratiyoge paYcamyA-
<>stasiH .” 5 . 4 . 44 . ityasya “AdyAdiBya
<>upAsaMKyAnam .” iti vArttikoktyA tasiH .)
<>paScAt . yaTA, BawwiH . 1 . 24 . <>“taM pfzWataH prazWamiyAya namro
<>hiMsrezu dIprAstraDaraH kumAraH ..”
  [and 3 more lines]
```
------------------------------------------
 Case 0259: pOMsraM  [page 3-238](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-238) 
```
skd:pOMsraM,22461:pOMsraM:n:

 <HI>pOMsraM, klI, (puMsa idam . puMs + “strIpuMsAByAM
<>naYsraYO BavanAt .” 4 . 1 . 87 . iti
<>sraY .) puMstvam . iti SabdamAlA .. (DEryyam .
<>yaTA, BAgavate . 4 . 26 . 26 . <>“kA devaraM vaSagataM kusumAstravega-
<>visrasta pOMsnamuSatI na Bajeta kftye ..”
  [and 6 more lines]
```
------------------------------------------
 Case 0260: pOrbbAhRikaH  [page 3-240](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-240) 
```
skd:pOrbbAhRikaH,22490:pOrbbAhRikaH:n:

 <HI>pOrbbAhRikaH, tri, (pUrbbAhRa + “viBAzA pUrbbAhRA-
<>parAhRAByAm .” 4 . 3 . 24 . iti WaY .)
<>pUrbbAhRe BavaH . pUrbbAhRasambanDI . iti sidDAnta- <>kOmudI ..
```
------------------------------------------
 Case 0261: pracaradrUpaH  [page 3-247](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-247) 
```
skd:pracaradrUpaH,22598:pracaradrUpaH:n:

 <HI>pracaradrUpaH, tri, vyaktarUpaH . pracAraviSizwaH . praca-
<>rat prakASamAnaM rUpaM svarUpaM yasya saH ..
```
------------------------------------------
 Case 0262: prajYaptI  [page 3-249](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-249) 
```
skd:prajYaptI,22641:prajYaptI:n:

 <HI>prajYaptI, strI, (prajYapti + vA NIz .) jinavidyA-
<>devIviSezaH . iti hemacandraH . 2 . 153 ..
```
------------------------------------------
 Case 0263: pramARavADitArTakaH  [page 3-285](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-285) 
```
skd:pramARavADitArTakaH,23047:pramARavADitArTakaH:n:

 <HI>pramARavADitArTakaH, puM, (pramARena vADitaH arTo
<>yasya . tataH kap .) tarkaviSezaH . tasya lakza-
<>Ram . AtmASrayAdicatuzkAnyaprasaNgatvam . sa
<>dviviDaH . vyAptigrAhakaH vizayapariSoDakaSca .
<>tatrAdyo yaTA . DUmo yadi vahnivyaBicArI
  [and 2 more lines]
```
------------------------------------------
 Case 0264: pralambaDnaH  [page 3-289](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-289) 
```
skd:pralambaDnaH,23099:pralambaDnaH:n:

 <HI>pralambaDnaH, puM, (pralambaM hantIti . han + kaH .)
<>balarAmaH . ityamaraH . 1 . 1 . 24 ..
```
------------------------------------------
 Case 0265: prasatvarI  [page 3-298](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-298) 
```
skd:prasatvarI,23195:prasatvarI:n:

 <HI>prasatvarI, strI, (prasatvan + “vano raca .” 4 .
<>1 . 7 . iti NIbrO .) pratipattiH . iti <>saMkziptasAroRAdivfttiH ..
```
------------------------------------------
 Case 0266: prAw  [page 3-309](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-309) 
```
skd:prAw,23377:prAw:n:

 <HI>prAw, [C] puM, pfcCati yaH . (pracCa + “kvip
<>vacipracCIti .” uRA0 2 . 57 . iti kvip
<>dIrGaH samprasAraRABAvaSca .) praSnakarttA . iti
<>mugDaboDavyAkaraRam ..
```
------------------------------------------
 Case 0267: prARAH  [page 3-310](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-310) 
```
skd:prARAH,23396:prARAH:n:

 <HI>prARAH, puM, (prARityeBiriti . pra + ana + karaRe
<>GaY .) asavaH . ityamaraH . 2 . 9 . 119 .. [Pagė-310-b+ 52]
<>“dve aNgasannihitezu paYcavAyuzu . asyante asavaH
<>nAmnIti uH . prARiti eBiH prARAH GaY .
<>evamiti asuvat prARA api puM bahutve ityarTaH .
  [and 71 more lines]
```
------------------------------------------
 Case 0268: prARApAnO  [page 3-310](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-310) 
```
skd:prARApAnO,23399:prARApAnO:n:

 <HI>prARApAnO, puM, (prARaSca apAnaSca tO . tadA-
<>KyakO vAyU . yaTA, vizRupurARe . 6 . 7 . 41 .
<>“paraspareRABiBavaM prARApAnO yadAnilO ..”) <>aSvinIkumArO . yaTA, --
<>“prARApAnO kaTaM devAvaSvinO saMbaBUvatuH ..”
<>iti prajApAlapraSne . [Pagė-311-a+ 52] <>mahAtapA uvAca .
  [and 7 more lines]
```
------------------------------------------
 Case 0269: priyaprepasuH  [page 3-368](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-368) 
```
skd:priyaprepasuH,23528:priyaprepasuH:n:

 <HI>priyaprepasuH, tri, (priyaM prepsatIti . pra + Ap +
<>san + uH .) izwArTodyuktam . utsuKaH . <>iti jawADaraH ..
```
------------------------------------------
 Case 0270: prozWapadAH  [page 3-374](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-374) 
```
skd:prozWapadAH,23635:prozWapadAH:n:

 <HI>prozWapadAH, strI, (prozWo gOstasyeva pAdA yAsAm .
<>“suprAtasuSveti .” 5 . 4 . 120 . iti bahu-
<>vrIhAvac padBAvaSca nipAtitaH .) pUrbbaBAdrapada-
<>nakzatram 25 . uttaraBAdrapadanakzatram 26 .
<>tatparyyAyaH . BAdrapadAH 2 . ityamaraH . 1 . 3 .
  [and 9 more lines]
```
------------------------------------------
 Case 0271: PalAWyA  [page 3-382](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-382) 
```
skd:PalAWyA,23783:PalAWyA:n:

 <HI>PalAWyA, strI, (Palena AWyA sampannA .) kAzWa-
<>kadalI . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0272: PalesaktaH  [page 3-382](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-382) 
```
skd:PalesaktaH,23805:PalesaktaH:n:

 <HI>PalesaktaH, tri, (Pale sakta AsaktaH .) PalAsaktaH .
<>PalakAmI . yaTA, BagavadgItAyAm .
<>“yuktaH karmmaPalaM tyaktvA SAntimApnoti nEzWi- <>kIm ..
<>ayuktaH kAmakAreRa Palesakto nibaDyate ..” [Pagė-383-a+ 52]
<>‘PalesaktaH mama PalAyedaM karmma karomItyevaM Pale-
  [and 1 more lines]
```
------------------------------------------
 Case 0273: PAH  [page 3-383](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-383) 
```
skd:PAH,23822:PAH:n:

 <HI>PAH, puM, santApaH . nizPalaBAzaRam . yaTA, --
<>“PiH kope PASca santApe taTA nizPalaBAzaRe ..”
<>iti SabdaratnAvalI .. [Pagė-383-c+ 52]
<>vfdDiH . bardDakaH . yaTA “PA vfdDO vardDake <>pumAn .” iti viSvaH ..
```
------------------------------------------
 Case 0274: PAlAH  [page 3-384](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-384) 
```
skd:PAlAH,23830:PAlAH:n:

 <HI>PAlAH, puM, (PAlayantIti . Pala + Ric + ac .)
<>jambIrabIjAni . iti durgawIkAwippanIkft- <>kulacandraH ..
```
------------------------------------------
 Case 0275: PiraNgiRI  [page 3-385](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-385) 
```
skd:PiraNgiRI,23839:PiraNgiRI:n:

 <HI>PiraNgiRI, strI, (PiraNgadeSo janmasTAnatvenA-
<>styasyA iti . PiraNga + ini . NIp .)
<>PiraNgadeSodBavanArI . yaTA, -- [Pagė-385-b+ 52]
<>“ganDarogaH PiraNgo'yaM jAyate dehinAM Druvam .
<>PiraNgiRo'tisaMsargAt PiraNgiRyAH prasaNgataH ..”
  [and 1 more lines]
```
------------------------------------------
 Case 0276: baqavAsutO  [page 3-390](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-390) 
```
skd:baqavAsutO,23916:baqavAsutO:n:

 <HI>ba(va)qavAsutO, puM, (baqavAyA GowakIrUpAyAH tvazwf-
<>sutAyAH saMjYAyAH sutO .) aSvinIkumArO . <>yaTA, hemacandraH .
<>“svarvEdyAvaSvinIputtrAvaSvinO baqavAsutO ..”
<>(tayorutpattikaTA yaTA, harivaMSe . 9 . 49 -- 53 .
<>“gacCa deva ! nijAM BAryyAM kurUMScarati sotta- <>rAn .
  [and 10 more lines]
```
------------------------------------------
 Case 0277: baqiSI  [page 3-390](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-390) 
```
skd:baqiSI,23919:baqiSI:n:

 <HI>baqiSI, strI, (baqiSa + gOrAditvAt NIz .)
<>baqiSam . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0278: baRijaH  [page 3-390](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-390) 
```
skd:baRijaH,23927:baRijaH:n:

 <HI>ba(va)RijaH, puM, (vaRigeva . baRij + svArTe aR .
<>aBiDAnAt na vfdDiH .) baRik . ityamaraH .
<>2 . 9 . 78 .. vavAdyekAdaSakaraRAntargatazazWa-
<>karaRam . tajjAtaPalaM yaTA, kozWIpradIpe .
<>“prAjYaH kftajYo guRavAn guRajYo <>baRigjanaprAptamanoraTaH syAt .
  [and 1 more lines]
```
------------------------------------------
 Case 0279: baDUjanaH  [page 3-392](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-392) 
```
skd:baDUjanaH,23958:baDUjanaH:n:

 <HI>baDUjanaH, puM, (baDUreva janaH .) yozit . iti
<>trikARqaSezaH .. (yaTA, mAGe . 3 . 52 .
<>“kzitipratizWo'pi muKAravindE- <>rbaDUjanaScandramaDaScakAra ..”)
```
------------------------------------------
 Case 0280: baDUwaSayanaM  [page 3-392](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-392) 
```
skd:baDUwaSayanaM,23959:baDUwaSayanaM:n:

 <HI>baDUwaSayanaM, klI, (baDUwInAM Sayanamiva . pfzo-
<>darAdikArasyAkAraH .) gavAkzaH . yaTA, -- [Pagė-392-c+ 61]
<>“vAtAyanaM gfhAkzaH syAdbaDUwaSayanantaTA ..” <>iti trikARqaSezaH ..
```
------------------------------------------
 Case 0281: baDUwiH  [page 3-392](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-392) 
```
skd:baDUwiH,23960:baDUwiH:n:

 <HI>baDUwiH strI, (alpavayaskA baDUH . alpArTe
<>wIH . pakze NIz . yadbA, baDU + “vayasya-
<>carama iti vAcyam .” 4 . 1 . 20 . ityasya
<>vArtti0 iti NIp .) puttraBAryyA . iti Barata
<>DftaratnakozaH . suvAsinI . iti hemacandraH .. <>alpA baDUH . yaTA, --
  [and 3 more lines]
```
------------------------------------------
 Case 0282: baDrI  [page 3-392](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-392) 
```
skd:baDrI,23964:baDrI:n:

 <HI>baDrI, strI, (baDyate'nayA . banDa + “sarvvaDAtuBya-
<>zwran .” uRA0 4 . 158 . iti zwran zitvAt
<>NIz .) carmmarajjuH . ityamaraH . 2 . 10 . 13 ..
<>baDrI dbirePeti kecit . bardDryAM carmmabanDe BavA
<>bArdDrI dIrGAdiriti haqqaH ..
```
------------------------------------------
 Case 0283: bahulAH  [page 3-411](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-411) 
```
skd:bahulAH,24181:bahulAH:n:

 <HI>bahulAH, strI, kfttikAnakzatram . nityabahuvacanA-
<>nto'yaM SabdaH . iti medinI . le, 117 ..
```
------------------------------------------
 Case 0284: bahuSaH  [page 3-412](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-412) 
```
skd:bahuSaH,24194:bahuSaH:n:

 <HI>bahuSaH, vya, (bahUni dadAti karotyAdi vA . bahu
<>+ “bahvalpArTAcCaskArakAdanyatarasyAm .” 5 .
<>4 . 42 . iti Sas .) bahUni . iti vyAka-
<>raRam .. (yaTA, mArkaRqeye . 52 . 29 .
<>“kaTyante bahuSaScEte pitA puttratrayaYca yat ..”)
```
------------------------------------------
 Case 0285: bahmavardDanaM  [page 3-450](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-450) 
```
skd:bahmavardDanaM,24497:bahmavardDanaM:n:

 <HI>bahmavardDanaM, klI, (brahmaRastapaso vardDanaM yasmAt .)
<>tAmram . iti hemacandraH . 4 . 106 ..
```
------------------------------------------
 Case 0286: bAqaveyO  [page 3-413](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-413) 
```
skd:bAqaveyO,24215:bAqaveyO:n:

 <HI>bA(vA)qaveyO, puM, (baqavAyA GowakarUpaDAriRyAH
<>sUryyapatnyA apatye pumAMsO . baqavA + Qak .)
<>aSvinIkumArO . iti SabdamAlA ..
```
------------------------------------------
 Case 0287: bAliKilyAH  [page 3-427](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-427) 
```
skd:bAliKilyAH,24300:bAliKilyAH:n:

 <HI>bAliKilyAH, puM, pulastyakanyAyAM sannatyAM kratoH
<>zazwisahasrasaMKyakaputtrA fziviSezAH . yaTA, --
<>“prItyAM pulastyo BagavAn dattAtrimasfjat pramuH .
<>pUrbbajanmani so'gastyaH smftaH svAyamBuve'ntare ..
<>vedabAhuM taTA kanyAM sannatirnAma nAmataH .
  [and 15 more lines]
```
------------------------------------------
 Case 0288: bAluNgikA  [page 3-428](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-428) 
```
skd:bAluNgikA,24319:bAluNgikA:n:

 <HI>bA(vA)luNgikA, strI, karkawI . iti SabdaratnA- <>valI ..
```
------------------------------------------
 Case 0289: bAlmIkiH  [page 3-428](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-428) 
```
skd:bAlmIkiH,24327:bAlmIkiH:n:

 <HI>bAlmIkiH, puM, balmIkasyApatyam . (balmIka +
<>iY .) balmIkapraBavo yasmAt tasmAdbAlmIki-
<>rityasO iti brahmavEvarttokteH . balmIkapraBavatvena
<>gORI puttrAdivadgORamasya balmIkApatyatvaM gfhI-
<>tvEva sADurapatyArTaH . yadvA balmIka iti fzi-
  [and 16 more lines]
```
------------------------------------------
 Case 0290: bukkI  [page 3-431](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-431) 
```
skd:bukkI,24364:bukkI:n:

 <HI>bukkI, strI, (bukka + gOrAditvAt NIz .) bukkam .
<>ityamarawIkAyAM BarataH ..
```
------------------------------------------
 Case 0291: bekawaH  [page 3-437](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-437) 
```
skd:bekawaH,24407:bekawaH:n:

 <HI>bekawaH, puM, matsyaBedaH . taruRaH . vidUzakaH .
<>iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0292: brahmArpraRaM  [page 3-455](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-455) 
```
skd:brahmArpraRaM,24533:brahmArpraRaM:n:

 <HI>brahmArpraRaM, klI, (brahmEvArpaRamitivigrahe yadarpaRaM
<>tadbrahmEva . yaTA, gItAyAm . 4 . 24 ..
<>“brahmArpaRaM brahmahavirbrahmAgnO brahmaRAhutam ..”
<>“brahmArpaRaM yena karaRena prakAreRa brahmavidBi-
<>ragnAvarpayati tadbrahmEveti paSyati tasyAtma-
  [and 15 more lines]
```
------------------------------------------
 Case 0293: brahmotaM  [page 3-456](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-456) 
```
skd:brahmotaM,24540:brahmotaM:n:

 <HI>brahmotaM, tri, brahmaRi A sampak prakAreRa UtaM
<>graTitam . (“lopo'syomAqoH .” iti sUtreRa
<>akAra lopaH .) iti mugDaboDavyAkaraRam ..
```
------------------------------------------
 Case 0294: brAhmaRqaM  [page 3-454](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-454) 
```
skd:brAhmaRqaM,24527:brAhmaRqaM:n:

 <HI>brAhmaRqaM, klI, (brahmaRo jagatsrazwuraRqam . caturddaSa-
<>BuvanagolakaH . yadvA, brahmaRA viSvasfjA kfta-
<>maRqam .) BuvanakozaH . viSvagolakaH . yaTA, --
<>“brahmARqaracanAKyAnaM SrUyatAM sAvaDAnataH .
<>brahmARqaracanArTaYca BaktA jAnanti yatnataH ..
  [and 190 more lines]
```
------------------------------------------
 Case 0295: brIqaH  [page 4-564](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-564) 
```
skd:brIqaH,34603:brIqaH:n:

 <HI>brIqaH, puM, (vrIqa + BAve GaY .) lajjA . itya-
<>maraH .. (yaTA, kumAre . 7 . 67 . <>“na nUnamArUQaruzA SarIra-
<>manena dagDaM kusumAyuDasya . <>vrIqAdamuM devamudIkzya manye
<>sannyastadehaH svayameva kAmaH ..”)
```
------------------------------------------
 Case 0296: BakzyAlAyuH  [page 3-466](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-466) 
```
skd:BakzyAlAyuH,24594:BakzyAlAyuH:n:

 <HI>BakzyAlAyuH, strI, (BakzyA BakzArhA alAvuH .)
<>rAjAlAvuH . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0297: BAryyApatI  [page 3-504](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-504) 
```
skd:BAryyApatI,24986:BAryyApatI:n:

 <HI>BAryyApatI, puM, (BAryyA ca patiSca tO . “rAja-
<>dantAdizu param .” 2 . 2 . 31 . iti sADuH .)
<>yozitpatI . nityadvivacanAnto'yaM SabdaH .
<>tatparyyAyaH . dampatI 2 jampatI 3 jAyApatI
<>4 . ityamaraH . 2 . 6 . 38 .. [Pagė-504-c+ 52]
```
------------------------------------------
 Case 0298: Biz  [page 3-512](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-512) 
```
skd:Biz,25086:Biz:n:

 <HI>Biz, rugjaye . sOtraDAturayam . iti kavikalpa-
<>drumaH .. (BvA0-para0-saka0-sew .) Bizak
<>BezajaM Bezati . iti durgAdAsaH ..
```
------------------------------------------
 Case 0299: BIH  [page 3-513](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-513) 
```
skd:BIH,25097:BIH:n:

 <HI>BIH, strI, (BI BItyAm + sampadAditvAt kvip .)
<>Bayam . ityamaraH . 1 . 7 . 21 .. (yaTA, <>AryyAsaptaSatyAm . 387 .
<>“pUrbbADiko gfhiRyAm bahumAnaH premanarmma- <>viSvAsaH .
<>BIraDikeyaM kaTayati rAgaM bAlAviBaktamiva ..”)
```
------------------------------------------
 Case 0300: BUtAra  [page 3-532](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-532) 
```
skd:BUtAra,25247:BUtAra:n:

 <HI>BUtAra, klI, (BUtAnAM ariH tannivArakatvAt aBi-
<>DAnAt klIvatvam .) hiNgu . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0301: BUriSaH  [page 3-536](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-536) 
```
skd:BUriSaH,25344:BUriSaH:n:

 <HI>BUriSaH, [s] vya, bahuSaH . bahvalpArTAt kAcca-
<>Sas veti kArakaBUriSabdAt caSaspratyayanizpa-
<>nno'yaM SabdaH . iti mugDaboDavyAkaraRam ..
<>(BUrIRi BUrIRi iti vIpsAyAM Sas . (yaTA,
<>mahAnirvvARatantre . 1 . 52 .
  [and 1 more lines]
```
------------------------------------------
 Case 0302: BUvadarI  [page 3-537](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-537) 
```
skd:BUvadarI,25355:BUvadarI:n:

 <HI>BUvadarI, strI, (BUlagnA vadarI SAkapArTivAdivat
<>samAsaH .) kzudrakolI . Jaqavera iti hindI-
<>BAzA . tatparyyAyaH . kzitivadarI 2 vallIvadarI
<>3 vadaravallI 4 bahuPalikA 5 laGuvadarI 6
<>vadaraPalI 7 sUkzmavadarI 8 . asyA guRAH .
  [and 3 more lines]
```
------------------------------------------
 Case 0303: BfgulApatiH  [page 3-538](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-538) 
```
skd:BfgulApatiH,25378:BfgulApatiH:n:

 <HI>BfgulApatiH, puM, paraSurAmaH . iti SabdaratnAvalI ..
<>BragURAmpatirityapi pAWaH ..
```
------------------------------------------
 Case 0304: BfYa  [page 3-538](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-538) 
```
skd:BfYa,25368:BfYa:n:

 <HI>BfYa, BftipuzwyoH . iti kavikalpa- <>drumaH .. (BvA0-uBa0-saka0-
<>aniw . li-hvA0-uBa0-saka0-aniw .) Bfti-
<>rBaraRaM puzwiH pozaRam . li Ya “biBartti SaSinaM
<>SamBurbiBfte janakaH sutam . wu BaraTuH . Nu
<>Bftrimam . BvAdipakze BftiH pUraRam . Ya Barati
  [and 1 more lines]
```
------------------------------------------
 Case 0305: BogamUmiH  [page 3-548](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-548) 
```
skd:BogamUmiH,25477:BogamUmiH:n:

 <HI>BogamUmiH, strI, (BogArTEva BUmiH .) suKa-
<>sTAnam . BAratavarzAtiriktavarzam . yaTA, --
<>“tatrApi BArataM SrezWaM jambudbIpe mahAmune ! .
<>yato hi karmmaBUrezA tato'nyA BogaBUmayaH ..”
<>iti vizRupurARe 2 aMSe 3 aDyAyaH ..
```
------------------------------------------
 Case 0306: BnASa  [page 3-556](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-556) 
```
skd:BnASa,25520:BnASa:n:

 <HI>BnASa, ya Na BAsi . iti kavikalpadrumaH .. (divA0
<>Atma0-aka0-sew .) ya Na BnASyate . iti <>durgAdAsaH ..
```
------------------------------------------
 Case 0307: BnAsa  [page 3-556](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-556) 
```
skd:BnAsa,25522:BnAsa:n:

 <HI>BnAsa, ya Na BAsi . iti kavikalpadrumaH .. (divA0-
<>Atma0-aka-0sew .) ya Na BnASyate . iti <>durgAdAsaH ..
```
------------------------------------------
 Case 0308: BramaracCalI  [page 3-557](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-557) 
```
skd:BramaracCalI,25541:BramaracCalI:n:

 <HI>BramaracCalI, strI, (BramarAn CalayatIti . Cali +
<>ac . gOrAditvAt NIz .) latAviSezaH . tat-
<>paryyAyaH . BfNgAhvA 2 BramarA 3 BfNgamUlikA 4 .
<>asyA guRAH . yaTA, rAjanirGaRwe .
<>“BfNgacCallI ca kawukA tiktA dIpanarocanI .”
```
------------------------------------------
 Case 0309: BrAtarO  [page 3-558](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-558) 
```
skd:BrAtarO,25561:BrAtarO:n:

 <HI>BrAtarO, puM, (BrAtA ca BaginI ca . BrAtfsvasro-
<>rityAdinA ekaSezaH .) BrAtfBaginyO . nitya-
<>dvivaMcanAnto'yam . ityamaraH . 2 . 6 . 36 ..
```
------------------------------------------
 Case 0310: maGavatI  [page 3-563](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-563) 
```
skd:maGavatI,25653:maGavatI:n:

 <HI>maGavatI, strI, (maGavat + NIp . indrARI .
<>iti mugDaboDavyAkaraRam ..
```
------------------------------------------
 Case 0311: maGonI  [page 3-564](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-564) 
```
skd:maGonI,25661:maGonI:n:

 <HI>maGonI, strI, (maGonaH patnIti . maGavan +
<>striyAM NIz . vakArasya ca samprasAraRam .)
<>indrARI . iti mugDaboDavyAkaraRam ..
```
------------------------------------------
 Case 0312: maNkza  [page 3-566](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-566) 
```
skd:maNkza,25684:maNkza:n:

 <HI>maNkza, vya, majjatIti . masj + bahulavacanAt suH .
<>“masjinaSorJali .” 7 . 1 . 60 . iti
<>num . “skoH saMyogAdyorante ca .” 8 . 2 .
<>29 . iti salopaH .) drutam ! ityamaraH . 3 .
<>4 . 2 . (yaTA, mATe . 5 . 37 .
  [and 3 more lines]
```
------------------------------------------
 Case 0313: maNgalyakumumA  [page 3-566](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-566) 
```
skd:maNgalyakumumA,25679:maNgalyakumumA:n:

 <HI>maNgalyakumumA, strI, (maNgalyAni kumumAni
<>yasyAH .) SaNKapuzpI . iti BAvaprakASaH ..
```
------------------------------------------
 Case 0314: maRIva  [page 3-576](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-576) 
```
skd:maRIva,25772:maRIva:n:

 <HI>maRIva, vya, maRitulyam . maRiSabdena saha iva-
<>Sabdasya zazWItatpuruzasamAsaH . praTamAdvivaca-
<>nAntamaRiSabdAt ivArTavaSabdena vASabdenA- <>pyevaM rUpam yaTA, --
<>“maRIvozwrasyeti tu ivArTe vaSabdo vASabdo <>vA boDyaH .”
<>iti sidDAntakOmudI ..
```
------------------------------------------
 Case 0315: madanakAkUravaH  [page 3-587](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-587) 
```
skd:madanakAkUravaH,25892:madanakAkUravaH:n:

 <HI>madanakAkUravaH, puM, (madanena hetunA kAkuH kAma-
<>janyo vikfto ravaH asPuwaDvaniryasya .) pArA-
<>vataH . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0316: maDukEwaBO  [page 3-597](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-597) 
```
skd:maDukEwaBO,25984:maDukEwaBO:n:

 <HI>maDukEwaBO, puM, (maDuSca kEwaBaSca tO . itaretara-
<>dvandvaH .) asuraviSezO . tayorutpattyAdi yaTA,
<>“dEnandine tu pralaye prasupte garuqaDvaje .
<>tasya SravaRaviqjAtAvasurO maDukEwaBO ..
<>kUrmmapfzWagatA pfTvI vilInevABavajjalEH .
  [and 98 more lines]
```
------------------------------------------
 Case 0317: maDurAmrakaH  [page 3-601](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-601) 
```
skd:maDurAmrakaH,26057:maDurAmrakaH:n:

 <HI>maDurAmrakaH, puM, (maDuraScAsO amraSceti nitya-
<>karmmaDArayaH . tataH svArTe kan .) AmrAtakaH . <>iti SabdacandrikA ..
```
------------------------------------------
 Case 0318: maDurAmraPalaH  [page 3-601](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-601) 
```
skd:maDurAmraPalaH,26058:maDurAmraPalaH:n:

 <HI>maDurAmraPalaH, puM, (maDuraM mizwaM Amravat PalaM
<>yasya .) rePalaH . tatparyyAyaH . AraH 2 . <>iti ratnamAlA ..
```
------------------------------------------
 Case 0319: maDurAlAvunI  [page 3-601](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-601) 
```
skd:maDurAlAvunI,26060:maDurAlAvunI:n:

 <HI>maDurAlAvunI, strI, (alAvU + bAhulakAt nak .
<>pfzodarAditvAt hrasvaH NIz ca . tataH maDurA [Pagė-601-b+ 52]
<>cAsO alAvunI ceti nityakarmmaDArayaH .)
<>rAjAlAvuH . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0320: maDyataH  [page 3-603](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-603) 
```
skd:maDyataH,26113:maDyataH:n:

 <HI>maDyataH, [s] vya, (maDya + tasil .) maDyAt .
<>maDye . iti sidDAntakOmudI .. syAdeH sTAne
<>tasAdayo ye AdeSAstezAmadantatvamavyayatva-
<>miti prAYcaH . tena kutaH kutaH AgataM kOta-
<>skutam . ityatrAvyayatvAt wilopaH . iti durgA- <>dAsaH ..
```
------------------------------------------
 Case 0321: manuzyI  [page 3-613](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-613) 
```
skd:manuzyI,26185:manuzyI:n:

 <HI>manuzyI, strI, (manuzya + gOrAditvAt NIz .
<>yopaDapratizeDe “hayagavayamukayamanuzyamatsyAnA-
<>mapratizeDaH .” iti NIzi yalopABAvaH .) mAnuza-
<>strIjAtiH . tatparyyAyaH . mAnuzI 2 nArI 3
<>mAnavI 4 . iti SabdaratnAvalI .. martyA 5
  [and 1 more lines]
```
------------------------------------------
 Case 0322: manTAH  [page 3-622](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-622) 
```
skd:manTAH,26246:manTAH:n:

 <HI>manTAH, [Tin] puM, (manTa + “manTaH .” uRA0
<>4 . 11 . iti iniH . sa ca kit .) manTAna-
<>daRqaH . ityamaraH . 2 . 9 . 74 .. (yaTA, <>kirAtArjjunIye . 4 . 16 .
<>“muhuH praRunnezu maTAM vivarttanE-
<>rnadatsu kummezu mfdaNgamanTaram ..”)
```
------------------------------------------
 Case 0323: mandAyuH  [page 3-623](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-623) 
```
skd:mandAyuH,26269:mandAyuH:n:

 <HI>mandAyuH, [s] tri, (mandamalpamAyuryasya .)
<>alpAyuH . yaTA, SrIBAgavate . 1 . 16 . 10 .
<>“mandasya mandaprajYasya vayo mandAyuzaSca vE .
<>nidrayA hriyate naktaM divA ca vyarTakarmmaBiH ..”
```
------------------------------------------
 Case 0324: marudvartma  [page 3-640](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-640) 
```
skd:marudvartma,26369:marudvartma:n:

 <HI>marudvartma, klI, (marutAM vAyUnAM devAnAM vA vartma
<>panTAH .) AkASam . iti trikARqaSezaH ..
```
------------------------------------------
 Case 0325: mahaCaH  [page 3-652](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-652) 
```
skd:mahaCaH,26563:mahaCaH:n:

 <HI>mahaCaH, puM, (mahaH kAyati prakASayatIti .
<>mahas + kE + kaH . pfzodarAditvAt sADuH .)
<>bahulAmodaH . iti jawAGaraH .. [Pagė-652-b+ 52]
```
------------------------------------------
 Case 0326: maharzaBI  [page 3-653](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-653) 
```
skd:maharzaBI,26572:maharzaBI:n:

 <HI>maharzaBI, strI, (mahatI cAsO fzaBI ceti
<>karmmaDArayaH .) kapikacCuH . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0327: mahArvvudaM  [page 3-670](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-670) 
```
skd:mahArvvudaM,26794:mahArvvudaM:n:

 <HI>mahArvvudaM klI, (mahat arvvudam .) daSArvvudam .
<>SatakowisaMKyA . iti jyotizam ..
```
------------------------------------------
 Case 0328: mAnuzikaM  [page 3-701](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-701) 
```
skd:mAnuzikaM,27146:mAnuzikaM:n:

 <HI>mAnuzikaM, tri, (manuzyasya BAvaH karmma vA .
<>manuzya + WaY .) manuzyasya karmmAdi . manuzya-
<>syedamitIdamarTe zRikapratyayena nizpannam ..
```
------------------------------------------
 Case 0329: mArttaRqaballaBA  [page 3-711](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-711) 
```
skd:mArttaRqaballaBA,27247:mArttaRqaballaBA:n:

 <HI>mArttaRqaballaBA, strI, (mArttaRqasya ballaBA priyA .
<>yadbA, mArttaRqo vallaBaH priyo'syAH .) Aditya-
<>BaktA . iti rAjanirGaRwaH .. sUryyapatnI ca ..
```
------------------------------------------
 Case 0330: mAzaSaH  [page 3-718](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-718) 
```
skd:mAzaSaH,27309:mAzaSaH:n:

 <HI>mAzaSaH, [s] vya, pratimAzam . mAzaM mAzaM dadAti
<>ityarTe Saspratyayena nizpannam . iti sidDAnta- <>kOmudI ..
```
------------------------------------------
 Case 0331: misI  [page 3-725](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-725) 
```
skd:misI,27415:misI:n:

 <HI>misI, strI, (masyati pariRamatIti + in
<>bAhulakAdata ikAraH . pakze NIz .) <>maDurikA . (yaTAsya paryyAyaH .
<>“misirmaDurikA matA .” <>iti gAruqe . 208 aH ..) jawAmAMsI . Sata-
<>puzpA . ityamaraBaratO .. (SatapuzpArTe paryyAyo <>yaTA, --
<>“SatapuzpA SatAhvA ca maDurA kAravI misiH .
  [and 4 more lines]
```
------------------------------------------
 Case 0332: mIQvAn  [page 3-726](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-726) 
```
skd:mIQvAn,27426:mIQvAn:n:

 <HI>mIQvAn, [s] puM, (miha secane . “dASvAn
<>sAhvAn mIQvAMSca .” 6 . 1 . 12 . iti
<>Candasi kvasuH . dvitvABAvaH aniwtvaM upaDA-
<>dIrGatvaM QatvaYca nipAtyate .) SivaH . yaTA, --
<>“tato mIQvAMsamAmantrya sunAsIrAH saharziBiH .
  [and 7 more lines]
```
------------------------------------------
 Case 0333: muktidaM  [page 3-734](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-734) 
```
skd:muktidaM,27482:muktidaM:n:

 <HI>muktidaM, klI, (muktiM dadAtIti . mukti + dA +
<>kaH .) BagavannAmaviSezaH . yaTA, --
<>“alpAkzaraYca bahvarTaM mahAsAraYca muktidam .
<>nAnAsidDipradAtAraM paraM cittAnuraYjakam ..
<>guhyAt guhyataraM cAnyannAma cAzwAkzaraM param .
  [and 6 more lines]
```
------------------------------------------
 Case 0334: muktiMdatte  [page 2-468](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-468) 
```
skd:muktiMdatte,12919:muktiMdatte:n:

 <HI>muktiMdatte sarvvadopAsyamAnA puMsAM SradDASAlinI vizRuBaktiH ..
<>DyAtA mUrttiH kzaRamapyacyutasya <>SreRI nAmnAM gaditA helayApi .
<>saMsAre'smin duritaM hanti puMsAM <>vAtormmIpotamivAmBoDimaDye ..
<>mugDe ! mAnaM parihara na cirAttAruRyante saPalayatu hariH .
<>PullAvallI BramaravilasitABAve SoBAM kalayati kimu tAm ..
  [and 524 more lines]
```
------------------------------------------
 Case 0335: muKavanDanaM  [page 3-740](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-740) 
```
skd:muKavanDanaM,27508:muKavanDanaM:n:

 <HI>muKavanDanaM, klI, (muKaM prAramBavizayaH tasya vanDanaM
<>saMgraho'tra .) granTAramBe tatra vaktavyakaTanam .
<>tasya nAmAntaram . anukramaRikA . BUmikA .
<>iti CandomaYjaryyAM muKabanDanAKyapraTamastavakaH .
<>iti tadgranTakarttfgaNgAdAsavEdyena liKitam ..
```
------------------------------------------
 Case 0336: mudgArdravawaH  [page 3-743](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-743) 
```
skd:mudgArdravawaH,27596:mudgArdravawaH:n:

 <HI>mudgArdravawaH, puM, (mudgenArdraH vawaH .) vawaka-
<>viSezaH . AdAvaqA iti BAzA . yaTA, --
<>“mudgapizwIviracitAn vawakAMstElapAcitAn .
<>hastena cUrRayet samyak tasmiMScUrRe vinikzipet ..
<>BfzwaM hiNgvArdrakaM sUkzmaM maricaM jIrakaM taTA .
  [and 8 more lines]
```
------------------------------------------
 Case 0337: munca  [page 3-749](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-749) 
```
skd:munca,27620:munca:n:

 <HI>munca u gatyAm . iti kavikalpadrumaH .. (BvA0-
<>para0-saka0-sew .) paYcamasvarI . ozWyavarga-
<>SezAdiH . nopaDaH . u muYcitvA muktvA . iti <>durgAdAsaH ..
```
------------------------------------------
 Case 0338: muSalikA  [page 3-752](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-752) 
```
skd:muSalikA,27646:muSalikA:n:

 <HI>muSa(za)likA, strI, (muza + “vfzAdiByaScit .”
<>uRA0 1 . 108 . iti kalaScit syAt . wAp .
<>tataH saMjYAyAM kan . akArasyetvam .) pallI .
<>tAlamUlI . tatparyyAyaH suvahA 2 tAlapatrikA 3
<>goDApadI 4 hemapuzpI 5 BUtAlI 6 dIrGa-
  [and 10 more lines]
```
------------------------------------------
 Case 0339: muzalI  [page 3-753](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-753) 
```
skd:muzalI,27651:muzalI:n:

 <HI>muzalI, strI, (muzyata iti . muz + kalaH NIz .)
<>tAlamUlikA . gfhagoDikA . iti Sabda- <>ratnAvalI ..
```
------------------------------------------
 Case 0340: mUzAtUtTaM  [page 3-764](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-764) 
```
skd:mUzAtUtTaM,27778:mUzAtUtTaM:n:

 <HI>mUzAtUtTaM, klI, (mUzAjAtaM tUtTam .) nIla-
<>tUtTam . tatparyyAyaH . kAMsyanIlam 2 hema-
<>tUtTam 3 vitunnakam 4 . iti hemacandraH . <>4 . 118 ..
```
------------------------------------------
 Case 0341: mfqI  [page 3-768](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-768) 
```
skd:mfqI,27871:mfqI:n:

 <HI>mfqI, strI, (mfqa + wAp NIp ca .) durgA . <>iti halAyuDaH ..
```
------------------------------------------
 Case 0342: meqa  [page 3-778](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-778) 
```
skd:meqa,28032:meqa:n:

 <HI>meqa, f unmAde . iti kavikalpadrumaH .. (BvA0-
<>para0-aka0-sew .) f, amimeqat . meqati
<>unmAdyatItyarTaH . iti durgAdAsaH ..
```
------------------------------------------
 Case 0343: meRwaH  [page 3-778](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-778) 
```
skd:meRwaH,28035:meRwaH:n:

 <HI>meRwaH, puM, hastipakaH . iti hArAvalI ..
```
------------------------------------------
 Case 0344: medonDraH  [page 3-780](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-780) 
```
skd:medonDraH,28057:medonDraH:n:

 <HI>medonDraH puM, (medobahulaH anDraH .) varRasaNkarajAti-
<>BedaH . iti jawADaraH .. (yaTA, manuH . 10 . 48 .
<>“medonDracuYcumadgUnAmAraRyapaSuhiMsanam ..”
<>kvacit medAnDra ityapi pAWaH ..)
```
------------------------------------------
 Case 0345: mozitA  [page 3-788](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-788) 
```
skd:mozitA,28181:mozitA:n:

 <HI>mozitA, [f] tri, mozaRakarttA . cOraH . iti
<>muzaDAtostfnpratyayena nizpannaH ..
```
------------------------------------------
 Case 0346: yaTitraH  [page 4-532](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-532) 
```
skd:yaTitraH,34367:yaTitraH:n:

 <HI>yaTitraH, tri, (vyaTa + ktaH .) pIqitaH . (yaTA,
<>ftusraMhAre . 6 . 19 . <>“kAntAnanadyutimuzAmacirodgatAnAM
<>SoBAM parAM kuruvakadrumamaYjarIRAm .
<>dfzwvA priye hi paTikasya Bavenna kasya
<>kandarpavARanikarErvyaTitaM hi cetaH ..”) <>duHKitaH . yaTA, --
  [and 3 more lines]
```
------------------------------------------
 Case 0347: yamalArjjunO  [page 4-021](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-021) 
```
skd:yamalArjjunO,28423:yamalArjjunO:n:

 <HI>yamalArjjunO, puM, (yamalO ca tAvarjjunO ceti
<>karmmaDArayaH .) gokulasTArjjunavfkzadvayam . tO
<>kuveraputtrO nalakUvaramanigrIvO . nAradaSApAt
<>vfkzatAM gatO SrIkfzRena udDAritO . yaTA, --
<>“yadimO lokapAlasya puttrO BUtvA tamaHplutO .
  [and 29 more lines]
```
------------------------------------------
 Case 0348: yayIH  [page 4-023](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-023) 
```
skd:yayIH,28438:yayIH:n:

 <HI>yayIH, puM (yAyate prApyate BaktEriti . yA + “yApoH
<>kit dve ca” . uRA0 3 . 159 . iti IH
<>dvitvaYca .) mahAdevaH . ityuRAdikozaH . 1 . 354 ..
<>(yAtidrutaM gacCatIti . aSvaH . ityujjvaladattaH ..)
```
------------------------------------------
 Case 0349: yAcan  [page 4-031](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-031) 
```
skd:yAcan,28521:yAcan:n:

 <HI>yAcan, [t] tri, (yAcatIti . yAc + Satf .) <>yAcakaH . yaTA, --
<>“muKaBaNgaH svaro dIno gAtrasvedo mahadBayam .
<>maraRe yAni cihnAni tAni cihnAni yAcataH ..”
<>iti gAruqe nItisAre 115 aDyAyaH ..
```
------------------------------------------
 Case 0350: yAmanAlIH  [page 4-040](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-040) 
```
skd:yAmanAlIH,28579:yAmanAlIH:n:

 <HI>yAmanAlIH, strI, (yAmasya nAlIva .) yAma-
<>GozA . iti trikARqaSezaH ..
```
------------------------------------------
 Case 0351: yAvAnI  [page 4-026](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-026) 
```
skd:yAvAnI,28477:yAvAnI:n:

 <HI>yAvAnI, strI, (duzwo yavaH . “yavAddoze .”
<>4 . 1 . 49 . ityasya varttikoktyA
<>NIz AnugAgamaSca . pakze svArTe kan .)
<>ozaDiBedaH . yoyAni iti BAzA . tatparyyAyaH .
<>dIpyakaH 2 dIpyaH 3 yavasAhvaH 4 yavAgrajaH 5
  [and 21 more lines]
```
------------------------------------------
 Case 0352: yuN  [page 4-045](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-045) 
```
skd:yuN,28652:yuN:n:

 <HI>yuN [j] tri, (yujir yoge + kvin . “yujera-
<>samAse .” 7 . 1 . 71 . iti numAgamaH .)
<>yogakarttA . melanakarttA . iti mugDaboDa-
<>vyAkaraRam .. (yaTA, BawwikAvye . 6 . 118 .
<>“guhAyA niragAdvAlI siMho mfgamiva dyuvan .
  [and 7 more lines]
```
------------------------------------------
 Case 0353: yujO  [page 4-046](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-046) 
```
skd:yujO,28660:yujO:n:

 <HI>yujO, [j] puM, (yuj + kvip .) aSvinIkumArO .
<>iti trikARqaSezaH .. nityadbivacanAnto'yam ..
```
------------------------------------------
 Case 0354: yUTInaH  [page 4-049](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-049) 
```
skd:yUTInaH,28704:yUTInaH:n:

 <HI>yUTInaH, puM, (yUTaM pAtIti . yUTa + KaH .)
<>yUTapaH . iti SabdacandrikA ..
```
------------------------------------------
 Case 0355: yonyarSaH  [page 4-066](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-066) 
```
skd:yonyarSaH,28779:yonyarSaH:n:

 <HI>yonyarSaH, [s] klI, (yonijAtamarSaH .) yoni-
<>jAtarogaviSezaH . tatparyyAyaH . kandasaMjYam 2 .
<>iti trikARqaSezaH .. (yaTA, --
<>“kecittu BUyAMsameva deSamupadiSantyarSasAM SiSna-
<>mapaTyapaTaM galamuKanAsikAkarRAkzivartmAni
  [and 1 more lines]
```
------------------------------------------
 Case 0356: raktapiktahA  [page 4-075](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-075) 
```
skd:raktapiktahA,28865:raktapiktahA:n:

 <HI>raktapiktahA, strI, (raktapittaM hantIti . han +
<>qaH . striyAM wAp .) raktaGnI . iti Sabda-
<>candrikA .. gA~WiyA dUrvvA iti BAzA ..
<>(vivftirasyAH raktaGnISabde jYAtavyA ..)
```
------------------------------------------
 Case 0357: raktareRukAM  [page 4-076](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-076) 
```
skd:raktareRukAM,28890:raktareRukAM:n:

 <HI>raktareRukAM, strI, (raktAH reRavaH parAgA asyAm .
<>kan . wAp ca .) palASakalikA . tatparyyAyaH .
<>aNgArikA 2 . iti SabdamAlA ..
```
------------------------------------------
 Case 0358: raktasandaSikA  [page 4-077](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-077) 
```
skd:raktasandaSikA,28913:raktasandaSikA:n:

 <HI>raktasandaSikA, strI, (raktAya raktapAnAya samyak
<>daSatIti . danSa + Rvul . wApi ata itvaYca .)
<>jalOkA . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0359: raktArvvudaH  [page 4-078](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-078) 
```
skd:raktArvvudaH,28938:raktArvvudaH:n:

 <HI>raktArvvudaH, puM, klI, (raktAnAmarvvudamatra .) roga-
<>viSezaH . yaTA, SAtAtapIyakarmmavipAke .
<>“jalodarayakftplIhaSUlarogavraRAni ca .
<>SvAsAjIrRajvaracCarddiBramamohagalagrahAH .
<>raktArvvudavisarpAdyA upapApodBavA gadAH ..”
  [and 26 more lines]
```
------------------------------------------
 Case 0360: raNGaH  [page 4-082](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-082) 
```
skd:raNGaH,29012:raNGaH:n:

 <HI>raNGaH, [s] klI, (raMGyate prApyate iti . raGi
<>+ “aGiraGiByAmasun .” iti asun . ityu-
<>jjvaladattaH . 4 . 213 .) raMhaH . vegaH . iti
<>BaratadbirUpakozaH .. (yaTA, sUryyaSatake . 71 . [Pagé-083-a+ 52]
<>“raMGaH saMGo surARAM jagadupakftaye nityamuktasya <>yasya ..”)
```
------------------------------------------
 Case 0361: rajyANgaM  [page 4-131](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-131) 
```
skd:rajyANgaM,29524:rajyANgaM:n:

 <HI>rajyANgaM, klI, (rAjyasya aNgam .) rAjyasyopAyaH .
<>tatparyyAyaH . prakftiH 2 . tattu azwaviDaM yaTA .
<>svAmI 1 amAtyaH 2 suhft 3 kozaH 4
<>rAzwram 5 durgam 6 balam 7 pOrARAM SreRa-
<>yaSca 8 . ityamaraH . 2 . 8 . 18 .. “svAmyA-
  [and 23 more lines]
```
------------------------------------------
 Case 0362: raYjanaduH  [page 4-086](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-086) 
```
skd:raYjanaduH,29052:raYjanaduH:n:

 <HI>raYjanaduH, puM, (raYjayatIti . ranj + Ric + lyuH .
<>raYjanaScAsO druSceti .) acCukavfkzaH . Ac- <>gACa iti BAzA ..
```
------------------------------------------
 Case 0363: ratiramaRqaH  [page 4-088](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-088) 
```
skd:ratiramaRqaH,29106:ratiramaRqaH:n:

 <HI>ratiramaRqaH, puM, (ratyA ramaRaH .) kAmadevaH . iti
<>trikARqaSezaH ..
```
------------------------------------------
 Case 0364: ramBorUH  [page 4-096](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-096) 
```
skd:ramBorUH,29216:ramBorUH:n:

 <HI>ramBorUH, strI, ramBe iva UrU yasyAH sA . iti
<>vyAkaraRam .. (yaTA, raGO . 6 . 35 . <>“anena yUnA saha pArTivena
<>ramBoru ! kaccinmanaso ruciste ..”)
```
------------------------------------------
 Case 0365: ravIndaM  [page 4-098](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-098) 
```
skd:ravIndaM,29250:ravIndaM:n:

 <HI>ravIndaM, klI, (raviRA sUryyakarasparSena indati
<>prakASate iti . inda + ac .) padmam . iti <>DaraRiH ..
```
------------------------------------------
 Case 0366: rAjagAmi  [page 4-114](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-114) 
```
skd:rAjagAmi,29396:rAjagAmi:n:

 <HI>rAjagAmi, [n] tri, (rAjAnaM gacCatIti . gam
<>+ RiniH .) rAjasambanDi . yaTA, --
<>“anftaYca samutkarze rAjagAmi ca pESunam .
<>guroScAlIkanirbbanDaH samAni brahmahatyayA ..”
<>iti mAnave 11 aDyAyaH ..
  [and 8 more lines]
```
------------------------------------------
 Case 0367: rAjaPaRijaJakaH  [page 4-121](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-121) 
```
skd:rAjaPaRijaJakaH,29439:rAjaPaRijaJakaH:n:

 <HI>rAjaPaRijaJakaH, puM, (rAjate iti . rAj + ac .
<>rAjo dIptiSAlI PaRijJakaH .) nAgaraNga- <>vfkzaH . iti SabdamAlA ..
```
------------------------------------------
 Case 0368: ruksadma  [page 4-164](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-164) 
```
skd:ruksadma,29716:ruksadma:n:

 <HI>ruksadma, [n] klI, malam . iti kecit ..
```
------------------------------------------
 Case 0369: ruq  [page 4-167](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-167) 
```
skd:ruq,29750:ruq:n:

 <HI>ruq, [z] strI, (ruza kruDi + kvip .) ruw . <>iti vyAkaraRam ..
```
------------------------------------------
 Case 0370: ruvuk  [page 4-172](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-172) 
```
skd:ruvuk,29793:ruvuk:n:

 <HI>ruvuk, puM, eraRqavfkzaH . iti ratnamAlA ..
```
------------------------------------------
 Case 0371: ruvyaM  [page 4-167](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-167) 
```
skd:ruvyaM,29734:ruvyaM:n:

 <HI>ruvyaM, klI, (rocate iti . ruc + “rAjasUyasUryya-
<>mfzodyeti .” 3 . 1 . 114 . iti kyappratyayena
<>nipAtitam .) sOvarccalam . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0372: ruvyaH  [page 4-167](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-167) 
```
skd:ruvyaH,29735:ruvyaH:n:

 <HI>ruvyaH, puM, (ruc + kyap .) katakavfkzaH . SAli-
<>DAnyam . iti rAjaniGaRwaH .. patiH . iti
<>hemacandraH . 3 . 191 .. sundare, tri . ityamaraH ..
<>(rucikaraH . yaTA, --
<>“pakvaM varRakaraM rucyaM mAMsaSukrabalapradam .
  [and 2 more lines]
```
------------------------------------------
 Case 0373: rekaRaH  [page 4-175](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-175) 
```
skd:rekaRaH,29842:rekaRaH:n:

 <HI>rekaRaH, [s] klI, (riRaktIti . ric + “ricerDane
<>Git kicca .” uRA0 4 . 198 . iti asun .
<>cAt pratyayasya nuw . GitvAt kutvam .) svarRam .
<>iti sidDAntakOmudyAmuRAdivfttiH ..
```
------------------------------------------
 Case 0374: rodasyO  [page 4-187](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-187) 
```
skd:rodasyO,29938:rodasyO:n:

 <HI>rodasyO, strI, BUmisvargO . yaTA, --
<>“prasUraSvApi BUdyAvO rodasyO rodasI ca te ..”
<>ityamaraH . 3 . 3 . 228 ..
<>“BUrBUmiH dyOrAkASaM ete sahite vigfhIte ca
<>rodasIrodaHSabdayorvAcyete iti BUdyAvO parA-
  [and 7 more lines]
```
------------------------------------------
 Case 0375: lakzmIH  [page 4-196](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-196) 
```
skd:lakzmIH,30064:lakzmIH:n:

 <HI>lakzmIH, strI, (lakzayati paSyati udyoginamiti .
<>lakzi + “lakzermuw ca .” uRA0 3 . 160 . iti
<>Ipratyayo muqAgamaSca .) vizRupatnI . tatparyyAyaH .
<>padmAlayA 2 padmA 3 kamalA 4 SrIH 5 hari-
<>priyA 6 indirA 7 lokamAtA 8 mA 9
  [and 461 more lines]
```
------------------------------------------
 Case 0376: lampAwahaH  [page 4-206](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-206) 
```
skd:lampAwahaH,30227:lampAwahaH:n:

 <HI>lampAwahaH, puM, pawahavAdyam . iti hArAvalI ..
```
------------------------------------------
 Case 0377: laSUnaH  [page 4-213](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-213) 
```
skd:laSUnaH,30309:laSUnaH:n:

 <HI>laSUnaH, puM, (rasena UnaH rasya latvam . pfzo-
<>darAditvAt sasya SaH akAralopaSca .) laSunaH . <>iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0378: lAjAH  [page 4-215](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-215) 
```
skd:lAjAH,30358:lAjAH:n:

 <HI>lAjAH, puM, BUmni, (lAYjyante ye te . lAj +
<>GaY .) BfzwaDAnyam . lAoyA iti KE iti
<>ca BAzA . (yaTA, rAmAyaRe . 2 . 43 . 13 .
<>“kadA prARisahasrARi rAjamArge mamAtmajO .
<>lAjeravakarizyanti praviSantAvarindamO ..”)
  [and 25 more lines]
```
------------------------------------------
 Case 0379: lAvUH  [page 4-216](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-216) 
```
skd:lAvUH,30387:lAvUH:n:

 <HI>lAvUH, strI, alAvuH . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0380: liNganI  [page 4-223](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-223) 
```
skd:liNganI,30417:liNganI:n:

 <HI>liNganI, strI, (liNgamastyasyA iti . liNga +
<>iniH .) latAviSezaH . paYcaguriyA iti hindI
<>BAzA . tatparyyAyaH . bahupatrI 2 ISvarI 3
<>SivavallikA 4 svayamBaH 5 liNgasamBUtA 6
<>lENgI 7 citraPalA 8 cARqAlI 9 liNgajA 10
  [and 8 more lines]
```
------------------------------------------
 Case 0381: luNguzaH  [page 4-225](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-225) 
```
skd:luNguzaH,30455:luNguzaH:n:

 <HI>luNguzaH, puM, ColaNgaH . iti ratnamAlA ..
```
------------------------------------------
 Case 0382: luRwAkI  [page 4-225](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-225) 
```
skd:luRwAkI,30476:luRwAkI:n:

 <HI>luRwAkI, strI, (luRwAka + zitvAt NIp .) cOrA
<>strI . iti mugDaboDavyAkaraRam ..
```
------------------------------------------
 Case 0383: lopApikA  [page 4-233](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-233) 
```
skd:lopApikA,30608:lopApikA:n:

 <HI>lopApikA, strI, (lopApaka + striyAM wAp .
<>ata itvam .) SfgAlI . iti SabdamAlA ..
```
------------------------------------------
 Case 0384: lomapAdapUH  [page 4-234](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-234) 
```
skd:lomapAdapUH,30624:lomapAdapUH:n:

 <HI>lomapAdapUH, strI, (lomapAdasya pUH .) purI-
<>viSezaH . aDunA BAgalapura iti KyAtA .
<>tatparyyAyaH . campA 2 mAlinI 3 karRapUH 4 . <>iti hemacandraH ..
```
------------------------------------------
 Case 0385: vatmakabIjaM  [page 4-256](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-256) 
```
skd:vatmakabIjaM,30959:vatmakabIjaM:n:

 <HI>vatmakabIjaM, klI, (vatsakasya bIjam .) indra-
<>yavaH . iti rAjanirGaRwaH .. (yaTA, --
<>“vyozaM vatsakabIjaYca nimbaBUnimbamArkavam .
<>citrakaM rohiRIM pAWAM dArvvImativizAM <>samAm ..”
<>iti vEdyakacakrapARisaMgrahe jvarAtisAre <>vyozAdyacUrRe ..)
```
------------------------------------------
 Case 0386: vatmakAmA  [page 4-256](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-256) 
```
skd:vatmakAmA,30960:vatmakAmA:n:

 <HI>vatmakAmA, strI, (vatsaM kAmayate iti . kam +
<>ac . wAp .) vatsABilAsinI gOH . tat-
<>paryyAyaH . vatsalA 2 . iti rAjanirGaRwaH .. <>puttrAdikAmA strI ca ..
```
------------------------------------------
 Case 0387: vatsraraH  [page 4-257](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-257) 
```
skd:vatsraraH,30966:vatsraraH:n:

 <HI>vatsraraH, puM, (vasantyasmin ayanarttumAsapakzavArA-
<>daya iti . vasa nivAse + “vaseSca .” uRA0
<>3 . 71 . iti saran . “saH syArdDaDAtuke .”
<>7 . 4 . 49 . iti sasya taH .) dvAdaSamAsA-
<>tmakakAlaH . ayanadbayAtmakaSca . tatparyyAyaH .
  [and 245 more lines]
```
------------------------------------------
 Case 0388: vaduzyaM  [page 4-511](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-511) 
```
skd:vaduzyaM,34212:vaduzyaM:n:

 <HI>vaduzyaM, klI, (viduzaH karmma BAvo vA . vidbas +
<>zyaY .) viduzo BAvaH . vidbasSabdAt zRya-
<>pratyayena nizpannamidam .. (yaTA, rAjataraNgi- <>RyAm . 1 . 12 .
<>“pAwavaM duzwavEduzyatIvrA suvrataBAratI ..”)
```
------------------------------------------
 Case 0389: vadrumaH  [page 4-394](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-394) 
```
skd:vadrumaH,32715:vadrumaH:n:

 <HI>vadrumaH, puM, (viSizwo drumaH . yadbA, viSizwo
<>drurvfkzo'styasyeti . “dyudruByAM maH .” 5 . 2 .
<>108 . iti maH .) prabAlaH . (yaTA, ftu- <>saMhAre . 6 . 17 .
<>“AmUlato vidrumarAgatAmrAH <>sapallavaM puzpacayaM daDAnAH .
<>kurvvantyaSokA hfdayaM saSokaM
  [and 4 more lines]
```
------------------------------------------
 Case 0390: vanavarvvarikA  [page 4-262](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-262) 
```
skd:vanavarvvarikA,31055:vanavarvvarikA:n:

 <HI>vanavarvvarikA, strI, (vanajAtA varvvarikA .)
<>araRyajavarvvarI . vanavAvui iti BAzA . tat-
<>paryyAyaH . suganDiH 2 suprasannakaH 3 dozA-
<>kleSI 4 vizaGnaH 5 sumuKaH 6 sUkzmapatrakaH 7
<>nidrAluH 8 SoPahArI 9 suvaktraH 10 . asya
  [and 2 more lines]
```
------------------------------------------
 Case 0391: vabaH  [page 4-267](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-267) 
```
skd:vabaH,31154:vabaH:n:

 <HI>vabaH, puM, ekAdaSakaraRAntargatapraTamakaraRam .
<>asyADipatirindraH . tatra vihitAni yaTA, --
<>“pOzwikasTiraSuBAni vabAKye .” <>iti jyotistattvam ..
<>atra jAtaPalam . <>“vabABiDAne jananaM hi yasya
<>SUro'tiDIro manujaH kftI syAt . <>padmAlayA tannilaye nivAmraM
  [and 3 more lines]
```
------------------------------------------
 Case 0392: vammakaRwakaH  [page 4-292](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-292) 
```
skd:vammakaRwakaH,31407:vammakaRwakaH:n:

 <HI>vammakaRwakaH, puM, parpawaH . iti rAjanirGaRwaH ..
<>vivftirasya parpawaSabde vijYeyA ..)
```
------------------------------------------
 Case 0393: varttutaM  [page 4-290](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-290) 
```
skd:varttutaM,31381:varttutaM:n:

 <HI>varttutaM, tri, (varttate iti . vft + bAhulakAt
<>ulac .) golavastu . tatparyyAyaH . nista-
<>lam 2 vfttam 3 . ityamaraH . 3 . 1 . 69 .. [Pagé-290-c+ 52]
<>maRqalAyitam 4 . iti SabdaratnAvalI .. * ..
<>(yaTA, BAgavate . 5 . 16 . 5 .
  [and 12 more lines]
```
------------------------------------------
 Case 0394: vardDrI  [page 4-291](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-291) 
```
skd:vardDrI,31403:vardDrI:n:

 <HI>vardDrI, strI, (vardDra + gOrAditvAt NIz .) carmma-
<>rjjuH . vadI iti BAzA . tatparyyAyaH . nardDrA 2
<>varatrA 3 . ityamaraH .. vadDI 4 . iti BarataH ..
```
------------------------------------------
 Case 0395: varvvarI  [page 4-292](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-292) 
```
skd:varvvarI,31422:varvvarI:n:

 <HI>varvvarI, strI, (varvvara + wAp . pakze zitvAt
<>NIz .) kzudravfkzaviSezaH . vAyui
<>iti BAzA . tatparyyAyaH . kavarI 2 tuNgI 3
<>KarapuzpA 4 ajaganDikA 5 ajaganDA 6
<>kavarA 7 KarapuzpikA 8 . ityamaraSabdaratnA-
  [and 8 more lines]
```
------------------------------------------
 Case 0396: varzAH  [page 4-296](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-296) 
```
skd:varzAH,31440:varzAH:n:

 <HI>varzAH, strI Bumni, (varzo varzaRamastyAsu iti .
<>varza + arSaAditvAt ac . wAp . yadvA, vriyante
<>iti . vf + “vftFvadIti .” uRA0 3 . 62 .
<>iti saH . tatazwAp .) svanAmaKyAta ftuH .
<>tatparyyAyaH . prAvfw 2 . ityamaraH . 1 . 4 . 19 ..
  [and 84 more lines]
```
------------------------------------------
 Case 0397: varhirjjotiH  [page 4-298](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-298) 
```
skd:varhirjjotiH,31479:varhirjjotiH:n:

 <HI>varhirjjotiH, [s] puM, (varhizi yajYe jyoti-
<>rasya .) vahriH . iti hemacandraH . 4 . 164 .
```
------------------------------------------
 Case 0398: varhizkaSaH  [page 4-298](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-298) 
```
skd:varhizkaSaH,31482:varhizkaSaH:n:

 <HI>varhizkaSaH, puM, (varhirdIptireva keSa iva yasya .)
<>agniH . iti SocizkeSaSabdadarSanAt ..
```
------------------------------------------
 Case 0399: valiSI  [page 4-299](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-299) 
```
skd:valiSI,31494:valiSI:n:

 <HI>va(ba)liSI, strI, (valiSi + kfdikArAditi
<>NIz .) vaqiSam . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0400: valkadumaH  [page 4-299](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-299) 
```
skd:valkadumaH,31501:valkadumaH:n:

 <HI>valkadumaH, puM, (valkapraDAno drumaH .) BUrjjavfkzaH .
<>iti rAjanirGaRwaH .. (vivaraRamasya BUrjjaSabde <>jYAtavyam ..)
```
------------------------------------------
 Case 0401: valBaRaM  [page 4-299](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-299) 
```
skd:valBaRaM,31519:valBaRaM:n:

 <HI>valBaRaM, klI, (valBa BakzaRe + BAve lyuw .) Bakza-
<>Ram . iti hemacandraH ..
```
------------------------------------------
 Case 0402: vaSca  [page 4-564](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-564) 
```
skd:vaSca,34592:vaSca:n:

 <HI>vaSca, U Sa Cede . iti kavikalpadrumaH .. (tudA0-
<>para0-saka0-vew .) dantyopaDaH cayogAt tAlavyaH .
<>tena kvipi SacCrAjeti zaNi nimittABAve nEmi-
<>ttikasyApyaBAva iti nyAyAt tAlavyasya dantyatve
<>saMyogAdeH sasya lope suvfw iti . taTA ca .
  [and 8 more lines]
```
------------------------------------------
 Case 0403: vaskayanI  [page 4-308](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-308) 
```
skd:vaskayanI,31650:vaskayanI:n:

 <HI>vaskayanI, strI, (vaskaya ekahAyano batsaH . tena
<>nIyate iti . nI + kvip . NIz .) ciraprasUtA
<>gOH . ityamarawIkAyAM rAyamukuwaH .. asyA <>dugDaguRAH .
<>“vaskayinyAstridozaGnaM tarpaRaM balakft payaH .”
<>iti BAvaprakASaH ..
```
------------------------------------------
 Case 0404: vastayaH  [page 4-308](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-308) 
```
skd:vastayaH,31658:vastayaH:n:

 <HI>vastayaH, puM strI BUmni, (vas + “vasestiH .” uRA0
<>4 . 179 . iti tiH bahuvacanAnto'yam .)
<>vastrasya daSAH . yaTA, amaraH . 2 . 6 . 114 .
<>“striyAM bahutve vastrasya daSAH syurvvastayo <>dvayoH ..”
```
------------------------------------------
 Case 0405: vastukI  [page 4-312](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-312) 
```
skd:vastukI,31665:vastukI:n:

 <HI>vastukI, strI, (vastuka + gOrAditvAt NIz .)
<>SvetacillISAkaH . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0406: vAgduzWaH  [page 4-319](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-319) 
```
skd:vAgduzWaH,31777:vAgduzWaH:n:

 <HI>vAgduzWaH, tri, (vAcA SudDe'pi vastuni aSudDa-
<>rUpatvadurvvAkyena duzwaH .) vAcA dozayuktaH . <>yaTA, --
<>“vAgBAvaduzwASca taTA duzwEScopahatAstaTA .
<>vAsasA cAvaDUtAni varjjyAni SrAdDakarmmaRi ..”
<>iti SrAdDatattvam .. * ..
  [and 5 more lines]
```
------------------------------------------
 Case 0407: vAtapramIH  [page 4-327](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-327) 
```
skd:vAtapramIH,31863:vAtapramIH:n:

 <HI>vAtapramIH, puM, strI, (vAtaM pramimIte vAtABi-
<>muKaM gacCatIti . vAta + pra + mA mAne +
<>“vAtapramIH .” uRA0 4 . 1 . iti Ipratyayena
<>sADuH .) vAtamfgaH . ityamaraH . 2 . 5 . 7 ..
<>vAow hariRa iti BAzA .. nakulaH . aSvaH . [Pagé-327-c+ 52]
  [and 3 more lines]
```
------------------------------------------
 Case 0408: vAtardDaH  [page 4-329](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-329) 
```
skd:vAtardDaH,31877:vAtardDaH:n:

 <HI>vAtardDaH, puM, kAzWalOhoBayanirmitapAtram . tat-
<>paryyAyaH . kAzWalOhI 2 . iti trikARqa- <>SezaH ..
```
------------------------------------------
 Case 0409: vAtiNganaH  [page 4-330](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-330) 
```
skd:vAtiNganaH,31908:vAtiNganaH:n:

 <HI>vAtiNganaH, puM, vArttAkuH . iti trikARqaSezaH ..
```
------------------------------------------
 Case 0410: vAdarAyaRiH  [page 4-331](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-331) 
```
skd:vAdarAyaRiH,31929:vAdarAyaRiH:n:

 <HI>vAdarAyaRiH, puM, (vAdarAyaRa eva . svArTe iY .)
<>vyAsadevaH . iti SabdaratnAvalI .. (vAdarA-
<>yaRasyApatyamiti apatyArTe iY . SukadevaH ..)
```
------------------------------------------
 Case 0411: vAppakaH  [page 4-354](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-354) 
```
skd:vAppakaH,32208:vAppakaH:n:

 <HI>vAppakaH, puM, (vAzpa + saMjYAyAM kan .) mArizaH .
<>nawiyA SAka iti BAzA . yaTA, --
<>“mArizo vAzpako mArzaH Sveto raktaSca sa smftaH .
<>mArizo maDuraH SIto vizWamBI pittanudguruH ..
<>vAtaSlezmakaro raktapittanudvizamAgnijit .
  [and 2 more lines]
```
------------------------------------------
 Case 0412: vArttikrA  [page 4-352](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-352) 
```
skd:vArttikrA,32147:vArttikrA:n:

 <HI>vArttikrA, strI, (vArttika + wAp .) pakziviSezaH .
<>vawera iti BAzA . tatparyyAyaH . vizRuliNgI 2 . <>iti hArAvalI ..
```
------------------------------------------
 Case 0413: vArdDeyaM  [page 4-353](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-353) 
```
skd:vArdDeyaM,32159:vArdDeyaM:n:

 <HI>vArdDeyaM klI, (vArdDeH samudrasyedamiti . vArdDi +
<>QaY .) droRIlavaRam . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0414: vArdDrInasaH  [page 4-353](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-353) 
```
skd:vArdDrInasaH,32162:vArdDrInasaH:n:

 <HI>vArdDrI(Ra)nasaH, puM, vArdDrIva nAsikA yasya . nAsAyA
<>nasAdeSaH .) gaRqakaH . gaRqAra iti BAzA . <>yaTA, --
<>“vArdDrInasastvekacaro gaRotsAhaSca gaRqakaH ..”
<>iti trikARqaSezaH ..
```
------------------------------------------
 Case 0415: vAzikI  [page 4-353](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-353) 
```
skd:vAzikI,32176:vAzikI:n:

 <HI>vAzikI, strI, (varzAsu BavA . varza + “varzABya-
<>zwak .” 4 . 3 . 18 . iti Wak . NIz .) trAya-
<>mARA . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0416: vAzpIkA  [page 4-354](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-354) 
```
skd:vAzpIkA,32212:vAzpIkA:n:

 <HI>vAzpIkA, strI, hiNgupatrI . iti rAjanirGaRwaH ..
<>rA~DunI iti KyAtaSca . asya guRAH . kawu-
<>tvam . tIkzRatvam . uzRatvam . kfmiSlezmanASi-
<>tvaYca . iti rAjavallaBaH ..
```
------------------------------------------
 Case 0417: vAsOkaH  [page 4-358](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-358) 
```
skd:vAsOkaH,32256:vAsOkaH:n:

 <HI>vAsOkaH, [s] klI, (vAsAya okaH sTAnam .)
<>vAsagfham . yaTA, -- [Pagé-358-b+ 52]
<>“garBAgAre'pavarako vAsOkaH SayanAspadam .” <>iti hemacandraH ..
```
------------------------------------------
 Case 0418: vAhAvAhavi  [page 4-368](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-368) 
```
skd:vAhAvAhavi,32280:vAhAvAhavi:n:

 <HI>vA(bA)hAvA(bA)havi, vya, (vAhuBirvAhuBiH prahftya
<>idaM yudDaM pravfttaM tat .) vAhuyudDam . hAtA-
<>hAti iti BAzA . iti mugDaboDavyAkaraRam ..
```
------------------------------------------
 Case 0419: vAhulaH  [page 4-369](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-369) 
```
skd:vAhulaH,32288:vAhulaH:n:

 <HI>vA(bA)hulaH, puM, kArttikamAsaH . ityamaraH .. aTa
<>kArttikamAhAtmyam . <>nArada uvAca .
<>“kArttikasya tu mAhAtmyaM mAsasya vaktumarhasi .
<>pUrbbaM kArttikamAhAtmyaM yaTAvat vaktumarhasi .. <>brahmovAca .
<>sADu pfzwaM tvayA puttra lokodDAraRahetave .
  [and 118 more lines]
```
------------------------------------------
 Case 0420: vAhulyaM  [page 4-369](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-369) 
```
skd:vAhulyaM,32289:vAhulyaM:n:

 <HI>vA(bA)hulyaM, klI, bahutvam . vahulasya BAva ityarTe
<>zRyapratyayena nizpannam ..
```
------------------------------------------
 Case 0421: viMSatitaMmaH  [page 4-370](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-370) 
```
skd:viMSatitaMmaH,32304:viMSatitaMmaH:n:

 <HI>viMSatitaMmaH, tri, (viMSateH pUraRaH . viMSati +
<>qaw . “viMSatyAdiByastamaqanyatarasyAm .” 5 .
<>2 . 56 . iti tamaqAgamaH .) viMSaH . iti <>sidDAntakOmudI ..
```
------------------------------------------
 Case 0422: viGAtfBUH  [page 4-397](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-397) 
```
skd:viGAtfBUH,32737:viGAtfBUH:n:

 <HI>viGAtfBUH, puM, (viDAturbrahmaRo BUrutpattiryasya .)
<>nAradamuniH . iti trikARqaSezaH ..
```
------------------------------------------
 Case 0423: viGnazinAyakaH  [page 4-377](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-377) 
```
skd:viGnazinAyakaH,32438:viGnazinAyakaH:n:

 <HI>viGnazinAyakaH, puM, (viGnAnAM vinAyakaH .) gaReSaH .
<>iti kASIKaRqaH ..
```
------------------------------------------
 Case 0424: vitardDI  [page 4-385](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-385) 
```
skd:vitardDI,32597:vitardDI:n:

 <HI>vitardDI, strI, vedikA . ityamarawIkAyAM BarataH .
```
------------------------------------------
 Case 0425: vitunnikA  [page 4-385](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-385) 
```
skd:vitunnikA,32610:vitunnikA:n:

 <HI>vitunnikA, strI, (vitunnA + svArTe kan . wApi
<>ata itvam .) BUmyAmalakI . iti rAja- <>nirGaRwaH ..
```
------------------------------------------
 Case 0426: vidAhi  [page 4-387](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-387) 
```
skd:vidAhi,32659:vidAhi:n:

 <HI>vidAhi, [n] klI, (vidahatIti . vi + daha +
<>RiniH .) dAhajanakadravyam . asya guRO .
<>pittamftyukAritve . iti rAjavallaBaH .. (dAha-
<>janakamAtre, tri . yaTA, gItAyAm . 17 . 9 .
<>“kawvamlalavaRAtyuzRatIkzRa rUkzavidAhinaH .
  [and 1 more lines]
```
------------------------------------------
 Case 0427: viduzmatI  [page 4-388](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-388) 
```
skd:viduzmatI,32670:viduzmatI:n:

 <HI>viduzmatI, tri, (vidvAnasti asyAmiti . vidvas
<>matup . striyAM NIp .) paRqitavatI . yaTA, --
<>“dyOrvvAcaspatineva pannagapurI SezAhinevA- <>Bavad-
<>yenEkena viduzmatI vasumatI muKyena saMKyAva- <>tAm ..”
<>iti vopadevapraSaMsA ..
```
------------------------------------------
 Case 0428: vidyuttvAn  [page 4-392](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-392) 
```
skd:vidyuttvAn,32705:vidyuttvAn:n:

 <HI>vidyuttvAn, [t] tri, (vidyutaH santyasminniti .
<>vidyut + matup .) vidyudbiSizwaH . vidyu- [Pagé-393-a+ 52]
<>cCabdAt vatupratyayena nizpannaH .. (yaTA, meGa- <>dUte . 66 .
<>“vidyuttvantaM lalitavanitAH sendracApaM sacitrAH
<>saNgItAya prahatamurajAH snigDagamBIraGozam ..”
  [and 3 more lines]
```
------------------------------------------
 Case 0429: viDAtrAyuH  [page 4-397](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-397) 
```
skd:viDAtrAyuH,32738:viDAtrAyuH:n:

 <HI>viDAtrAyuH, [s] puM, (viDAturAyurjjIvita-
<>kAlaparimARaM yasmAt . sUryyakriyAM vinA vatsa-
<>rAdijYAnAsamBavAdevAsya taTAtvam .) sUryyaH . <>yaTA, --
<>“veSadAno viDAtrAyurddivyavastro divAkaraH ..” <>iti SabdacandrikA ..
<>klI, brahmaRo vayaH . tadyaTA . caturddaSa-
  [and 8 more lines]
```
------------------------------------------
 Case 0430: vinirmmokaH  [page 4-403](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-403) 
```
skd:vinirmmokaH,32804:vinirmmokaH:n:

 <HI>vinirmmokaH, puM, vyatirekaH . yaTA . divAkaravAra-
<>vinirmmoke tu gargaH . iti cUqAprakaraRe jyoti-
<>stattvam .. (vigato nirmmoko yasya iti vigrahe <>muktakaYcuke, tri ..)
```
------------------------------------------
 Case 0431: vipAw  [page 4-407](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-407) 
```
skd:vipAw,32865:vipAw:n:

 <HI>vipAw, [S] strI, vipASAnadI . ityamaraH ..
<>(yaTA, fgvede . 3 . 33 . 1 . <>“gAveva SuBre mAtarA rihARe
<>vipAwCutudrI payasA javete ..”
<>“vipAw kUlavipAwanAt vipASanAt Sataputtra-
<>maraRodBUtatamovftermmumUrzorvaSizWasya pASA asyAM
  [and 6 more lines]
```
------------------------------------------
 Case 0432: viBrAw  [page 4-414](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-414) 
```
skd:viBrAw,32948:viBrAw:n:

 <HI>viBrAw, [j] tri, (viSezeRa BrAjate iti . <>vi + BrAj + “anyeByo'pi
```
------------------------------------------
 Case 0433: viBrAq  [page 4-414](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-414) 
```
skd:viBrAq,32949:viBrAq:n:

 <HI>viBrAq, [j] tri, (viSezeRa BrAjate iti .
<>vi + BrAj + “anyeByo'pi [Pagé-414-b+ 56]
<>dfSyate .” 3 . 2 . 177 . iti kvip .) alaNkA-
<>rAdinA dIptiSIlaH . tatparyyAyaH . BrAjizRuH
<>2 rocizRuH 3 . ityamaraH .. (yaTA, fgvede . <>10 . 170 . 1 .
  [and 5 more lines]
```
------------------------------------------
 Case 0434: virUpikA  [page 4-420](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-420) 
```
skd:virUpikA,33060:virUpikA:n:

 <HI>virUpikA, strI, (vikftaM rUpaM yasyAH . kan .
<>wApi ata itvam .) kurUpA . yaTA, -- [Pagé-420-c+ 52]
<>“nAgnayaH parivindanti na yajYA na tapAMsi ca .
<>na ca SrAdDaM kanizWasya yA ca kanyA virU- <>pikA ..”
<>ityudvAhatattvam ..
```
------------------------------------------
 Case 0435: vilvapezikA  [page 4-426](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-426) 
```
skd:vilvapezikA,33136:vilvapezikA:n:

 <HI>vi(bi)lvapezikA, strI, (vilvasya pezikA .)
<>SuzkavilvaKaRqam . velaSu~WA iti BAzA . <>yaTA, --
<>“kaPavAtAmaSUlaGnI grAhiRI vilvapezikA .” <>iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0436: viSvakSenaH  [page 4-438](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-438) 
```
skd:viSvakSenaH,33274:viSvakSenaH:n:

 <HI>viSvakSenaH, puM, vizRuH . ityamarawIkAyAM BarataH ..
<>trayodaSamanuH . yaTA, --
<>“manurBUteH sutastadbat BOtyo nAma Bavizyati .
<>tatastu merusAvarRabrahmasUnurmanuH smftaH ..
<>ftuSca ftuDAmA ca viSvakSeno manustayA ..
  [and 10 more lines]
```
------------------------------------------
 Case 0437: viSvagvAyuH  [page 4-438](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-438) 
```
skd:viSvagvAyuH,33282:viSvagvAyuH:n:

 <HI>viSvagvAyuH, puM, (viSvaggato vAyuH .) sarvvatogAmi-
<>pavanaH . asya guRAH . <>“viSvagvAyuranAyuzyaM prARinAM nEkadozakft .
<>sarvvarttuliNgako hantA kftyotpAtapuraHsaraH ..”
<>iti rAjavallaBaH .. [Pagé-438-b+ 52]
```
------------------------------------------
 Case 0438: viSvaN  [page 4-438](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-438) 
```
skd:viSvaN,33283:viSvaN:n:

 <HI>viSvaN, [c] tri, sarvvatragAmI . viSvaM aYcati
<>ityarTe kvippratyayena nizpannamidam ..
```
------------------------------------------
 Case 0439: viSvataH  [page 4-438](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-438) 
```
skd:viSvataH,33288:viSvataH:n:

 <HI>viSvataH, [s] vya, (viSva + tasil .) sarvvataH .
<>ityamarawIkA .. (yaTA, BAgavate . 10 . 31 . 3 .
<>“vfzamayAtmajAdviSvato BayAt <>fzaBa te vayaM rakzitA muhuH ..”)
```
------------------------------------------
 Case 0440: viSvadryaN  [page 4-439](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-439) 
```
skd:viSvadryaN,33291:viSvadryaN:n:

 <HI>viSvadryaN, [c] tri, sarvvatragamanakarttA . vizvak
<>samantAt aYcati gacCati vizvadryaN sarvvato-
<>gAmI mUrdDanyamaDyaH tAlavyamaDyo'pItyeke . itya- <>maraBaratO ..
```
------------------------------------------
 Case 0441: vizamuzWiH  [page 4-448](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-448) 
```
skd:vizamuzWiH,33382:vizamuzWiH:n:

 <HI>vizamuzWiH, puM, kzupaviSezaH . vizadoqi iti
<>kaqsiNge iti ca hindIBAzA . tatparyyAyaH .
<>keSamuzwiH 2 sumuzwiH 3 raRamuzwikaH 4 kzupa-
<>qoqamuzwiH 5 . asya guRAH . kawutvam . tikta-
<>tvam . dIpanatvam . kaPavAtakaRWAmayaraktapittA-
  [and 1 more lines]
```
------------------------------------------
 Case 0442: vizayi  [page 4-449](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-449) 
```
skd:vizayi,33387:vizayi:n:

 <HI>vizayi, [n] klI, (vizayAH santyasyeti . vizaya
<>+ iniH .) indriyam . iti medinI . ne, 211 ..
```
------------------------------------------
 Case 0443: vizvadryaN  [page 4-461](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-461) 
```
skd:vizvadryaN,33486:vizvadryaN:n:

 <HI>vizvadryaN, [c] tri, (vizvagaYcatIti . aYca +
<>kvip . “vizvagdevayoSceti .” 6 . 3 . 92 . iti
<>weH sTAne adrItyAdeSaH .) sarvvatogAmI .
<>ityamaraH .. (yaTA, mAGe . 18 . 25 . <>“vizvadrIcIrvikzipan sEnyavIcI-
<>rAjAvantaH kvApi dUraM prayAtam ..”
```
------------------------------------------
 Case 0444: vIjopticakraM  [page 4-469](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-469) 
```
skd:vIjopticakraM,33614:vIjopticakraM:n:

 <HI>vIjopticakraM, klI, (vIjAnAmuptaye SuBASuBasUcakaM
<>cakram .) vIjavapanajanyaSuBASuBajYAnArTasarpA-
<>kAracakram . yaTA, --
<>“sUryyaBAduragaH sTApyastrinAqye kAntarakramAt .
<>muKe trIRi gale trIRi BAni dbAdaSa tUdare ..
  [and 12 more lines]
```
------------------------------------------
 Case 0445: vIBatsuH  [page 3-431](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=3-431) 
```
skd:vIBatsuH,24355:vIBatsuH:n:

 <HI>vIBatsuH, puM, (bIBatsatIti . vaDa + san + u .
<>yudDe nyAyapaTena SatruhananAdasya taTAtvam .
<>etanniruktiruktA . yaTA, mahABArate 4 . <>42 . 18 .
<>“na kuryyAM karmma bIBatsaM yuDyamAnaH kaTaYcana .
<>tena devamanuzyezu bIBatsuriti viSrutaH ..”) <>arjjunaH . yaTA, --
  [and 3 more lines]
```
------------------------------------------
 Case 0446: vuqa  [page 4-565](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-565) 
```
skd:vuqa,34617:vuqa:n:

 <HI>vuqa, Si saMvftisaMhatyoH . majje . iti kavikalpa-
<>drumaH .. (tudA0-kuwA0-para0-saka0-akaYca-sew .)
<>rePayuktaH . Si, vruqati avruqIt vuvroqa .
<>majjo majjanam . iti durgAdAsaH ..
```
------------------------------------------
 Case 0447: vfttataH  [page 4-479](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-479) 
```
skd:vfttataH,33771:vfttataH:n:

 <HI>vfttataH, [s] vya, (vftta + tasil .) vfttena . yaTA,
<>“atigrahAvyaTanakzepezvakarttari tftIyAyAH .
<>akarttari tftIyAntAt vA tasiH syAt . avya-
<>TanaM acalanam . vfttena na vyaTate vfttato na
<>vyaTate . vfttena na calatItyarTaH . kzepe . vfttena
  [and 4 more lines]
```
------------------------------------------
 Case 0448: vfntAkI  [page 4-483](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-483) 
```
skd:vfntAkI,33833:vfntAkI:n:

 <HI>vfntAkI, strI, vArttAkI . iti rAjanirGaRwaH .. [Pagé-483-b+ 52]
```
------------------------------------------
 Case 0449: vfhajjIvantI  [page 4-491](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-491) 
```
skd:vfhajjIvantI,33930:vfhajjIvantI:n:

 <HI>vf(bf)hajjIvantI, strI, vfhajjIvantikAvfkzaH .
<>tatparyyAyaH . patraBadrA 2 priyaNkarI 3 maDurA 4
<>jIvapuzwA 5 bfhajjIvA 6 yaSaskarI 7 . asyA
<>guRAH . bahuvIryyadAtftvam . BUtavidrAvaRatvam .
<>vegAdrasaniyAmakaYca . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0450: vfhaqQakkA  [page 4-491](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-491) 
```
skd:vfhaqQakkA,33931:vfhaqQakkA:n:

 <HI>vf(bf)haqQakkA, strI, (vfhatI QakkA .) QakkA- <>viSezaH . yaTA, --
<>“vfhaqQakkA tu BerI strI pumAn dunduBirA- <>nakaH .
<>dragaqaH pratipattUryyamAnakaH pawaho'striyAm ..” <>iti jawADaraH ..
```
------------------------------------------
 Case 0451: vfhatgfhaH  [page 4-492](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-492) 
```
skd:vfhatgfhaH,33958:vfhatgfhaH:n:

 <HI>vf(bf)hatgfhaH, puM, (vfhadgfhaM yasmin .) kArUza-
<>deSaH . sa tu vinDyaparvvatasya paScAt mAlava-
<>deSasya nikawe varttate . iti hemacandraH ..
<>trikARqaSeze vfhadguha iti pAWaH ..
```
------------------------------------------
 Case 0452: vfhattikA  [page 4-492](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-492) 
```
skd:vfhattikA,33942:vfhattikA:n:

 <HI>vf(bf)hattikA, strI, (vfhan tikto yasyAH .)
<>pAWA . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0453: vfhatpArevataM  [page 4-492](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-492) 
```
skd:vfhatpArevataM,33949:vfhatpArevataM:n:

 <HI>vf(bf)hatpArevataM, klI, (vfhat mahat pArevatam .)
<>mahApArevatam . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0454: vfhatpuzpI  [page 4-492](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-492) 
```
skd:vfhatpuzpI,33952:vfhatpuzpI:n:

 <HI>vf(bf)hatpuzpI, strI, (vfhat puzpaM yasyAH . NIz .)
<>GaRwAravA . iti jawADaraH ..
```
------------------------------------------
 Case 0455: vEkatikaH  [page 4-507](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-507) 
```
skd:vEkatikaH,34155:vEkatikaH:n:

 <HI>vEkatikaH, puM, maRikAraH . iti hemacandraH ..
```
------------------------------------------
 Case 0456: vEqAlavratiH  [page 4-509](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-509) 
```
skd:vEqAlavratiH,34179:vEqAlavratiH:n:

 <HI>vE(bE)qAlavratiH, puM, aNganAdyaBAvAt kftabrahma-
<>caryyaH . iti jawADaraH ..
```
------------------------------------------
 Case 0457: vEtrakeyaH  [page 4-510](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-510) 
```
skd:vEtrakeyaH,34200:vEtrakeyaH:n:

 <HI>vEtrakeyaH, tri, vetrasambanDIyaH . vetraSabdAt zReya-
<>pratyayena nizpannametat ..
```
------------------------------------------
 Case 0458: vyAqraH  [page 4-546](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-546) 
```
skd:vyAqraH,34454:vyAqraH:n:

 <HI>vyAqraH, puM, sarpaH . mAMsaBakzakapaSuH . ityamaraH ..
<>(yaTA, mAkraReye .. 15 . 10 . [Pagé-547-a+ 52]
<>“SvA Sfgalo vfko gfDro vyAqaH kaNkastaTA- <>kramAt ..”)
<>indraH . iti SabdaratnAvalI .. vaYcakaH . iti <>rAyamukuwaH ..
```
------------------------------------------
 Case 0459: vyAvaBAsI  [page 4-551](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=4-551) 
```
skd:vyAvaBAsI,34511:vyAvaBAsI:n:

 <HI>vyAvaBAsI, strI, (vi + ava + BAsa + Rac .
<>svArTe aY . NIp .) vyAvakroSI . ityamara- <>wIkAyAM rAyamukuwaH ..
```
------------------------------------------
 Case 0460: SaMsTAH  [page 5-001](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-001) 
```
skd:SaMsTAH,34635:SaMsTAH:n:

 <HI>SaMsTAH, stri, (SaM + sTA + “sTaH kaca .” 3 . 2 .
<>77 . iti kkip .) SuBAnvitaH . Sam-
<>pUrvvasTADAtoH kvippratyayena nizpannametat ..
```
------------------------------------------
 Case 0461: SakkuH  [page 5-069](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-069) 
```
skd:SakkuH,35509:SakkuH:n:

 <HI>SakkuH, tri, avyavasAyI . iti trikARqaSezaH ..
```
------------------------------------------
 Case 0462: SaktyardDaH  [page 5-006](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-006) 
```
skd:SaktyardDaH,34689:SaktyardDaH:n:

 <HI>SaktyardDaH, puM, (SakterardDaH .) SramadvArA kukzilalAwa-
<>grIvAsUtpanno GarmmaH dIrGaniSvAsaSca . yaTA, --
<>“kukzO lalAwe grIvAyAM yadA GarmmaH pravarttate .
<>SaktyardDaM taM vijAnIyAdAyatocCAsameva ca ..” <>iti rAjavallaBaH ..
```
------------------------------------------
 Case 0463: SaNKaDmAH  [page 5-012](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-012) 
```
skd:SaNKaDmAH,34769:SaNKaDmAH:n:

 <HI>SaNKaDmAH, puM, (SaNKaM DamatIti . DmA + kvip .)
<>SaNKavAdakaH . iti mugDaboDavyAkaraRam ..
```
------------------------------------------
 Case 0464: SatAruH  [page 5-017](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-017) 
```
skd:SatAruH,34888:SatAruH:n:

 <HI>SatAruH, [s] klI, kuzWaBedaH . iti SabdaratnA-
<>valI .. tasya rUpamAha .
<>“raktaM SyAvaM sadAhArtti SatAruH syAdvahuvraRam .”
<>iti BAvaprakASaH ..
```
------------------------------------------
 Case 0465: SanEH  [page 5-020](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-020) 
```
skd:SanEH,34919:SanEH:n:

 <HI>SanEH, [s] vya, adrutam . ityamaraH . yaTA, --
<>“SanErvvidyA SanEH kanTA SanEH parvvatamAruhet .
<>SanEH kAmaSca DarmmaSca paYcEtAni SanEH SanEH ..”
<>iti gAruqe 109 aDyAyaH .. <>SanEScaraH . iti medinI ..
```
------------------------------------------
 Case 0466: Saratparvva  [page 5-028](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-028) 
```
skd:Saratparvva,35025:Saratparvva:n:

 <HI>Saratparvva, [n] klI, (SaradaH parvva .) kojAgara-
<>pUrRimA . yaTA, -- <>“kojAgaraH Saratparvva SAradI dyUtapUrRimA .”
<>iti jawADaraH ..
```
------------------------------------------
 Case 0467: SarkarArvvudaH  [page 5-032](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-032) 
```
skd:SarkarArvvudaH,35077:SarkarArvvudaH:n:

 <HI>SarkarArvvudaH, puM, klI, (SarkkarAvadarvvudaH .) kzudraroga-
<>viSezaH . tallakzaRaM yaTA, --
<>“prApya mAMsaM SirAH snAyuM medaH SlezmA taTAnilaH .
<>granTiM karotyasO Binno maDusarpirvvasAniBam ..
<>sravatyAsrAvamatyarTaM tatra vfdDiM gato'nilaH .
  [and 5 more lines]
```
------------------------------------------
 Case 0468: SardDanaM  [page 5-033](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-033) 
```
skd:SardDanaM,35086:SardDanaM:n:

 <HI>SardDanaM, klI, (SardDa + lyuw .) aDovAyuH . iti
<>manuwIkAyAM kallUka-BawwaH .. vAtakarmma iti <>BAzA ..
```
------------------------------------------
 Case 0469: SarmmikA  [page 5-130](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-130) 
```
skd:SarmmikA,36200:SarmmikA:n:

 <HI>SarmmikA, strI, lOhapratimA . iti Sabda- <>ratnAvalI ..
```
------------------------------------------
 Case 0470: SalAkApuruzAH  [page 5-034](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-034) 
```
skd:SalAkApuruzAH,35114:SalAkApuruzAH:n:

 <HI>SalAkApuruzAH, puM, bOdDAnAM trizazwidEvapuruzAH .
<>tatra dbAdaSa cakravarttinaH . caturvviMSatirjinAH .
<>nava vAsudevAH . nava baladevAH . nava prati- <>vAsudevASca . yaTA, --
<>“yaH sarvvamaRqalasyeSo rAjasUyaYca yo'yajat .
<>cakravarttI sArvvaBOmaste tu dbAdaSa BArate ..
  [and 17 more lines]
```
------------------------------------------
 Case 0471: SavalI  [page 5-037](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-037) 
```
skd:SavalI,35158:SavalI:n:

 <HI>SavalI, strI, (SavalayogAdanyato NIz .) Savala-
<>varRA gOH . ityamaraBaratO ..
```
------------------------------------------
 Case 0472: SAkunikah  [page 5-043](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-043) 
```
skd:SAkunikah,35266:SAkunikah:n:

 <HI>SAkunikah, puM, (SAkunAn hantIti . Sakuna +
<>“pakzimatsyamfgAn hanti .” 4 . 4 . 35 .
<>iti Wak .) pakzihantA . pAKimArA iti
<>ciqimAra iti ca BAzA . tatparyyAyaH .
<>jIvAntakaH 2 . ityamaraH .. (yaTA, manuH . <>8 . 260 .
  [and 1 more lines]
```
------------------------------------------
 Case 0473: SAskulaH  [page 5-067](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-067) 
```
skd:SAskulaH,35481:SAskulaH:n:

 <HI>SAskulaH, tri, mAMsASI . iti hemacandraH ..
```
------------------------------------------
 Case 0474: SAskulikaM  [page 5-067](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-067) 
```
skd:SAskulikaM,35482:SAskulikaM:n:

 <HI>SAskulikaM, klI, (Saskula + samUhArTe Wak .)
<>SaskulisamUhaH . ityamaraH ..
```
------------------------------------------
 Case 0475: SipA  [page 5-073](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-073) 
```
skd:SipA,35595:SipA:n:

 <HI>SipA, strI, nadIviSezaH . yaTA, --
<>“tato himavataH prasTe pratIcyAM tatpurasya ca .
<>Sipra nAmaM saraH pUrRaM dadfSurdruhiRAdayaH ..
<>tadrahasTAnamAsAdya brahmaSakrAdayaH surAH .
<>upavizwA yaTAnyAyaM puraskftya maheSvaram ..
  [and 8 more lines]
```
------------------------------------------
 Case 0476: SirastrARAM  [page 5-074](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-074) 
```
skd:SirastrARAM,35621:SirastrARAM:n:

 <HI>SirastrARAM, klI, (SirastrAyate anena . trE +
<>lyuw .) SirorakzaRasannAhaH . KopaqA iti
<>wop iti ca BAzA . tatparyyAyaH . SIrzaRyam 2
<>SIrzakama 3 Siraskam 4 . iti hemacandraH .. [Pagẽ-074-b+ 52]
<>Sirastram 5 . ityamaraH .. (yaTA, raGO . <>4 . 64 .
  [and 1 more lines]
```
------------------------------------------
 Case 0477: SivapiyaH  [page 5-094](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-094) 
```
skd:SivapiyaH,35722:SivapiyaH:n:

 <HI>SivapiyaH, puM, (Sivasya priyaH .) vakavfkza . iti
<>jawADaraH .. sPawikaH . DustUraH . iti rAja- <>nirGaRwaH ..
```
------------------------------------------
 Case 0478: SivAhnAdaH  [page 5-105](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-105) 
```
skd:SivAhnAdaH,35753:SivAhnAdaH:n:

 <HI>SivAhnAdaH, puM, (SivasyAhRAdo yasmAt .) vaka-
<>vfkzaH . iti rAjanirGaRwaH .. SivasyAnandaSca ..
```
------------------------------------------
 Case 0479: SIH  [page 5-109](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-109) 
```
skd:SIH,35790:SIH:n:

 <HI>SIH, strI, (SI + kvip .) SAntiH . Sayanam . <>iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0480: SIzaMGAtI  [page 5-112](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-112) 
```
skd:SIzaMGAtI,35901:SIzaMGAtI:n:

 <HI>SIzaMGAtI, [n] tri, (SIrzaM hantIti . hana +
<>“kumAraSIrzayorRiniH .” 3 . 2 . 51 . iti
<>RiniH .) mastakacCedakaH . SIrzaSabdapUrvvaka-
<>hanaDAtoH karmmaByAM kumAraSIrzAByAM hana iti
<>maMkziptamArasUtreRa Rinapratyayena nizpannaH ..
```
------------------------------------------
 Case 0481: SucipraRIH  [page 5-119](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-119) 
```
skd:SucipraRIH,36009:SucipraRIH:n:

 <HI>SucipraRIH, puM, (SuciM praRayatIti . pra + nI +
<>kvip .) Acamanam . yaTA, -- [Pagẽ-119-b+ 52]
<>“AcAmaH syAdAcamanamupasparSaH SucipraRIH .” <>iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0482: Suwa  [page 5-119](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-119) 
```
skd:Suwa,36015:Suwa:n:

 <HI>Suwa, Kowane . iti kavikalpadrumaH .. (BvA0-para0-
<>aka0-sew .) Kowanamiha gatyAGAtaH . Sowati
<>payaH setunA . iti durgAdAsaH ..
```
------------------------------------------
 Case 0483: SudDajaNgraH  [page 5-120](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-120) 
```
skd:SudDajaNgraH,36036:SudDajaNgraH:n:

 <HI>SudDajaNgraH, puM, (SudDA jaNGA yasya .) gardaBaH .
<>iti trikARqaSezaH .. pavitrajaNGAyukte, tri ..
```
------------------------------------------
 Case 0484: SudDvodanasutaH  [page 5-122](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-122) 
```
skd:SudDvodanasutaH,36046:SudDvodanasutaH:n:

 <HI>SudDvodanasutaH, puM, (SudDodanasya sutaH .) budDaBedaH .
<>iti hemacandraH ..
```
------------------------------------------
 Case 0485: SuzkalI  [page 5-125](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-125) 
```
skd:SuzkalI,36118:SuzkalI:n:

 <HI>SuzkalI, strI, SuzkamAMsam . mAMsamAtram . yaTA,
<>“SuzkalI SuzkamAMse syAnmAMsamatre'pi <>dfSyate .”
<>iti SOzkalaSabdawIkAyAM BarataH ..
```
------------------------------------------
 Case 0486: SUkaqI  [page 5-126](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-126) 
```
skd:SUkaqI,36134:SUkaqI:n:

 <HI>SUkaqI iti hindI BAzA . tatpraryyAyaH . SUkam
<>2 SUkAQyam 3 kanizWakam 4 . asya guRaH .
<>“durmmarantu paSUnAYca SUkABAve hitaYca tat ..” <>iti rAjanirGaRwaH ..
<>durmmarantusTAne darjaranun iti pAWaH .. [Pagẽ-126-b+ 52]
```
------------------------------------------
 Case 0487: SUya  [page 5-133](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-133) 
```
skd:SUya,36230:SUya:n:

 <HI>SUya, prasave . iti kavikalpadrumaH .. (BvA0-para0-
<>maka0-sew .) SUyati . iti durgAdAsaH ..
```
------------------------------------------
 Case 0488: SUrpaRaKI  [page 5-130](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-130) 
```
skd:SUrpaRaKI,36193:SUrpaRaKI:n:

 <HI>SUrpaRaKI, strI, (SUrpAkArARi naKAni yasyAH .
<>kevalayOgikatve NIz .) rAvaRaBaginI . iti
<>SabdamAlASabdaratnAvalyO ..
```
------------------------------------------
 Case 0489: SfNgaberaM  [page 5-134](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-134) 
```
skd:SfNgaberaM,36250:SfNgaberaM:n:

 <HI>SfNgaberaM, klI, (SfNgasyeva beraM SarIraM yasya .)
<>Ardrakam . ityamaraH .. SuRWI . iti rAja-
<>nirGaRwaH .. (yaTA, suSrute . 1 . 38 .
<>“pippalImarIcaSfNgaberARi trikawukam ..”)
<>guhacaRqAlapuram . yaTA, --
  [and 5 more lines]
```
------------------------------------------
 Case 0490: SfNgaberABamUlakaH  [page 5-134](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-134) 
```
skd:SfNgaberABamUlakaH,36252:SfNgaberABamUlakaH:n:

 <HI>SfNgaberABamUlakaH, puM, (SfNgaberABaM mUlaM yasya .
<>kan .) erakA iti BAvaprakASaH ..
```
------------------------------------------
 Case 0491: Sma  [page 5-145](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-145) 
```
skd:Sma,36468:Sma:n:

 <HI>Sma, [n] klI, muKam . iti SmaSruSabdawIkAyAM
<>BarataH .. (SarIram . iti niruktiH . 3 . 5 ..
<>SavaH . iti purARam . asya pramARaM SmaSAna- <>Sabde drazwavyam ..)
```
------------------------------------------
 Case 0492: SraBa  [page 5-154](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-154) 
```
skd:SraBa,36536:SraBa:n:

 <HI>SraBa u Ba ya Yi ir tapaHKedayoH . iti kavi-
<>kalpadrumaH .. (divA0-para0-aka0-sew . ktvAvew .)
<>u, SramitvA SrAntvA . Ba ya, SrAmyati lokaH
<>tapaH karoti Kidyati vetyarTaH . Yi, SrAnto-
<>'sti . ir aSramat aSrIt . asmAt puzA-
  [and 2 more lines]
```
------------------------------------------
 Case 0493: SrAntamaMvAhanaM  [page 5-164](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-164) 
```
skd:SrAntamaMvAhanaM,36566:SrAntamaMvAhanaM:n:

 <HI>SrAntamaMvAhanaM, klI, (SrAntasya saMvAhanam .)
<>SramAnvitasya AsanAdipradAnadvArA Sramopa- <>Samanam . iti kecit ..
```
------------------------------------------
 Case 0494: SrIH  [page 5-165](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-165) 
```
skd:SrIH,36587:SrIH:n:

 <HI>SrIH, puM, (Sri + kvip dIrGaSca .) rAgaviSezaH . sa
<>tu hanumanmate zaqrAgAntargatapaYcamarAgaH .
<>pfTivyA nABito nirgataH . asya jAtiH
<>sampU rRA . tasya svarAvaliH . za f ga ma pa Da
<>ni asya gfhaM zaqjasvaraH . hemantarttO apa-
  [and 75 more lines]
```
------------------------------------------
 Case 0495: SrIkaRqapadalAYCanaH  [page 5-165](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-165) 
```
skd:SrIkaRqapadalAYCanaH,36589:SrIkaRqapadalAYCanaH:n:

 <HI>SrIkaRqapadalAYCanaH, puM, (SrIkaRWa iti padaM
<>lAYCanaM yasya .) BavaBUtiH . sa tu mAlatI-
<>mADavAdinAwakakarttA . iti SabdamAlA ..
<>(yaTA, uttaracarite prastAvagAyAm .
<>“asti Kalu tatraBavAn kASyapaH SrIkaRWa-
  [and 1 more lines]
```
------------------------------------------
 Case 0496: SrIpaM  [page 5-166](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-166) 
```
skd:SrIpaM,36610:SrIpaM:n:

 <HI>SrIpaM, tri, (SrI + pA + kaH .) SriyaM pAti yat .
<>iti mugDaboDavyAkaraRam ..
```
------------------------------------------
 Case 0497: SlakzRatvaka  [page 5-173](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-173) 
```
skd:SlakzRatvaka,36733:SlakzRatvaka:n:

 <HI>SlakzRatvaka, [c] puM, (SlakzRA manoharA tvak
<>yasya .) aSmantakavfkzaH . iti rAjanirGaRwaH .. <>sundaravalkalaSca ..
```
------------------------------------------
 Case 0498: SvadazwrA  [page 5-176](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-176) 
```
skd:SvadazwrA,36776:SvadazwrA:n:

 <HI>SvadazwrA, strI, (Suno daMzwreva kaRwakAvftatvAt .)
<>gokzurakaH . ityamaraH .. (yaTA, suSrute . 1 . 46 .
<>“pippalyAdInAM SvadaMzwAvasukAmavaH kuzmARqA-
<>dInAM dArvvIkarAmavaH ..”)
```
------------------------------------------
 Case 0499: SvaSurO  [page 5-177](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-177) 
```
skd:SvaSurO,36796:SvaSurO:n:

 <HI>SvaSurO, puM, (SvaSrUSca SvaSuraSca tO . “SvaSuraH
<>SvaSrvA .” 1 . 2 . 71 . iti pAkzika eka-
<>SezaH .) SvaSrUSvaSurO . dvivacanAnto'yam .
<>ityamaraH .. SvaSura SASuqI iti BAzA ..
```
------------------------------------------
 Case 0500: SvasanotasukaH  [page 5-177](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-177) 
```
skd:SvasanotasukaH,36806:SvasanotasukaH:n:

 <HI>SvasanotasukaH, puM, (SvasanAya utsukaH . sarpaH .
<>iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0501: SvASvAH  [page 5-178](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-178) 
```
skd:SvASvAH,36820:SvASvAH:n:

 <HI>SvASvAH, puM, (SvA kukkuraH aSva iva vAhanaM yasya .)
<>BErava . kukkuravAhanatvAt . iti kecit ..
```
------------------------------------------
 Case 0502: SvitraGrI  [page 5-180](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-180) 
```
skd:SvitraGrI,36831:SvitraGrI:n:

 <HI>SvitraGrI, strI, (Svita hantIti . hana + wak .
<>NIz .) pItaparRI . iti SabdacandrikA ..
<>viCuwI iti BAzA .. [Pagẽ-181-a+ 52]
```
------------------------------------------
 Case 0503: zawkarmma  [page 5-185](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-185) 
```
skd:zawkarmma,36930:zawkarmma:n:

 <HI>zawkarmma, [n] klI, (zawprakAraM karmma .) zaw-
<>prakAraSAntyAdikarmma . yaTA, SAradAyAm .
<>“aTABiDAsye tantre'smin samyak zawkarmma- <>lakzaRam .
<>sarvvatantrAnusAreRa prayogaH PalasidDidaH ..
<>SAntivaSyastamBanAni vidvezoccAwane tataH .
  [and 228 more lines]
```
------------------------------------------
 Case 0504: zaqAmnAyAH  [page 5-188](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-188) 
```
skd:zaqAmnAyAH,36955:zaqAmnAyAH:n:

 <HI>zaqAmnAyAH, puM, (zaw AmnAyAH .) Sivasya
<>zaqvaktravinirgatazawprakAratantraSAstram . yaTA, <>SrISiva uvAca .
<>“aTedAnIM pravakzyAmi AmnAyakaTanaM pfTak .
<>pUrvvAmnAyo yadA mantrastadA prAcIdiSi sTita
<>sadASivo'haM BagavAnAcAraH parikIrttitaH .
  [and 43 more lines]
```
------------------------------------------
 Case 0505: zazwisaMvatsarAH  [page 5-191](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-191) 
```
skd:zazwisaMvatsarAH,37006:zazwisaMvatsarAH:n:

 <HI>zazwisaMvatsarAH, puM, (zazwiguRitAH saMvatsarAH .)
<>praBavAdizazwisaMKyakavarzam . yaTA, -- [Pagẽ-191-b+ 52]
<>aTa zazwisaMvatsaragaRanA . <>“SAkendrakAlaH pfTagAkftiGnaH 22
<>SaSANkanandASviyugEH 4291 sametaH .
<>SarAdrivasvindu 1875 hftaH salabDaH
  [and 159 more lines]
```
------------------------------------------
 Case 0506: zazWAlukAlakaM  [page 5-192](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-192) 
```
skd:zazWAlukAlakaM,37010:zazWAlukAlakaM:n:

 <HI>zazWAlukAlakaM, klI, dvitryahAnantaraBuktam . yaTA
<>“dvyantare tryantare BuktamAhuH zazWAlukAlakam .”
<>iti trikARqaSezaH ..
```
------------------------------------------
 Case 0507: zinBa  [page 5-194](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-194) 
```
skd:zinBa,37033:zinBa:n:

 <HI>zinBa, u hiMsane . dIptO . iti kavikalpadrumaH ..
<>(BvA0-para0-saka0-dIptO aka0-sew . kvAvew
<>u, simBitvA sibDvA . iti durgAdAsaH ..
```
------------------------------------------
 Case 0508: zoqaSakalAH  [page 5-195](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-195) 
```
skd:zoqaSakalAH,37064:zoqaSakalAH:n:

 <HI>zoqaSakalAH, strI, (zoqaSasaMKyAnvitAH kalAH .)
<>candramaRqalasya zaqaDikadaSaBAgAH . tAsAM
<>nAmAni yaTA . tataScandrasya amftAdizoqaSa-
<>kalAH pUrvvavat prARapratizWAM kftvA pUjayet . aM
<>amftAyE namaH 1 . AM mAnadAyE 2 . iM
  [and 7 more lines]
```
------------------------------------------
 Case 0509: zoqaSaBUjA  [page 5-195](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-195) 
```
skd:zoqaSaBUjA,37066:zoqaSaBUjA:n:

 <HI>zoqaSaBUjA, strI, (zoqaSa BUjA yasyAH .) zoqa-
<>SahastayuktA durgA . yaTA, --
<>“yadA tu zoqaSaBujAM mahAmAyAM prapUjayet .
<>durgAtantreRa mantreRa viSezaM tatra vE SfRu ..
<>kanyAyAM kfzRapakzasya ekAdaSyAmupozitaH .
  [and 8 more lines]
```
------------------------------------------
 Case 0510: zoqaSamAtfkAH  [page 5-195](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-195) 
```
skd:zoqaSamAtfkAH,37067:zoqaSamAtfkAH:n:

 <HI>zoqaSamAtfkAH, strI, (zoqaSasaMKyakAH mAtfkAH .)
<>zoqaSasaMKyakadevoviSezaH . yaTA . gOryyAdi-
<>zoqaSamAtfkAByo namaH . iti durgotsava-
<>padDatiH .. tAsAM nAmAni yaTA, SrAdDatattve .
<>“gOrI padmA SacI meDA sAvitrI vijayA jayA .
  [and 3 more lines]
```
------------------------------------------
 Case 0511: zoqaSASuH  [page 5-195](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-195) 
```
skd:zoqaSASuH,37069:zoqaSASuH:n:

 <HI>zoqaSASuH, puM, (zoqaSa aMSavo yasya .) SukraH .
<>iti SabdamAlA .. zoqaSakiraRayukte, tri [Pagẽ-196-a+ 52]
```
------------------------------------------
 Case 0512: zRuma  [page 5-198](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-198) 
```
skd:zRuma,37114:zRuma:n:

 <HI>zRuma, ya Bakze . iti kavikalpadrumaH .. (divA0
<>para0-saka0-sew .) dantyanakArayuktaH . ya,
<>snumyati suzRoma . iti durgAdAsaH ..
```
------------------------------------------
 Case 0513: saMvarAriH  [page 5-311](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-311) 
```
skd:saMvarAriH,38460:saMvarAriH:n:

 <HI>saMvarAriH, puM, (saMvarasya ariH .) kAmadevaH . itya-
<>maraH . 1 . 1 . 23 .. “saMvarasya dEtyaviSezasya ariH .
<>‘klIvantu saMvaraM nIrabOdDavrataviSezayoH . [Pagẽ-311-c+ 52]
<>viSeze puMsi dEtyasya matsyasya hariRasya ca ..’
<>iti dantyAdO raBasaH . saMvaraRaM saMvaraH BAve'l .
  [and 4 more lines]
```
------------------------------------------
 Case 0514: saMsargo  [page 5-205](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-205) 
```
skd:saMsargo,37239:saMsargo:n:

 <HI>saMsargo, [n] (saMsargo'syAstIti . iniH .
<>yadvA, saM + sfja + “saMpfcAnuruDeti .” 3 . 2 .
<>142 . iti GiRun .) saMsargaviSizwaH . yaTA .
<>viBAgAnantaraM mEtryAt pitfBrAtfpitfzyaBrAtf-
<>puttrARAM yaTAyaTaM ekatrAvasTAnaM saMsargaH . tad-
  [and 3 more lines]
```
------------------------------------------
 Case 0515: saNGazitaM  [page 5-220](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-220) 
```
skd:saNGazitaM,37434:saNGazitaM:n:

 <HI>saNGazitaM, tri, samyagGozaRASrayaH .. saMpUrvva-
<>GuzaDAtoH ktapratyaye kfte vA ruzA-
<>mahfzatvarasaMGuzAsvana ityanena vikalpena imA
<>nizpannam . iti mugDaboDavyAkaraRam .. (yaTA,
<>mahAnirvvARatantre . 1 . 4 .
  [and 1 more lines]
```
------------------------------------------
 Case 0516: saNGazwaM  [page 5-220](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-220) 
```
skd:saNGazwaM,37435:saNGazwaM:n:

 <HI>saNGazwaM, tri, samyagGozaRASrayaH .. saMpUrvva-
<>GuzaDAtoH ktapratyaye kfte vA ruzA-
<>mahfzatvarasaMGuzAsvana ityanena vikalpena imA
<>nizpannam . iti mugDaboDavyAkaraRam .. (yaTA,
<>mahAnirvvARatantre . 1 . 4 .
  [and 1 more lines]
```
------------------------------------------
 Case 0517: saMNkrAntiH  [page 5-213](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-213) 
```
skd:saMNkrAntiH,37388:saMNkrAntiH:n:

 <HI>saMNkrAntiH, strI, (saM + krama + ktin .) rASya-
<>ntarasaMyogAnukUlavyApAraH . yaTA, --
<>“nAqInakzatradivase raviBOmaSanEScarAH .
<>saNkrAntiM yasya kurvvanti tasya kleSo'BijAyate ..
<>iti tiTyAditattvam .. <>ravisaMkrAntiryaTA . BavizyamAtsyajyotizezu .
  [and 317 more lines]
```
------------------------------------------
 Case 0518: satkarmma  [page 5-223](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-223) 
```
skd:satkarmma,37518:satkarmma:n:

 <HI>satkarmma, [n] klI, (sat praSastaM karmma . veda-
<>vihitakriyA . yaTA . vizRupurARe .
<>“satkarmmayogyo na jano nEvApaH SOcakAraRam
<>yasminnanudite tasmE namo devAya BAsvate ..”
<>iti prAyaScittatattvam ..
```
------------------------------------------
 Case 0519: sattvasantannaH  [page 5-224](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-224) 
```
skd:sattvasantannaH,37538:sattvasantannaH:n:

 <HI>sattvasantannaH, tri, (sattvaM sampannaH .) sattvaprAptaH .
<>iti lokaprasidDiH ..
```
------------------------------------------
 Case 0520: saDarmmo  [page 5-237](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-237) 
```
skd:saDarmmo,37638:saDarmmo:n:

 <HI>saDarmmo, [n] tri, (saha Darmmo'styasyeti .
<>“DarmmaSIlavarRAntAcca .” 5 . 2 . 132 . iti
<>iniH . “vopasarjjanasya .” 6 . 3 . 82 . iti
<>sahasya saH .) samAnaDarmmAcArI . yaTA, --
<>“paSUnAM samajo'nyezAM samAjo'Ta saDarmi- <>RAm ..” <>ityamaraH ..
```
------------------------------------------
 Case 0521: saDryaN  [page 5-237](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-237) 
```
skd:saDryaN,37643:saDryaN:n:

 <HI>saDryaN, [c] tri, (saha aYcatIti . aYca gatO +
<>ftvigAdinA kvin . sahasya saDriH .) sahacaraH
<>ityamaraH .. (yaTA, BAgavate . 2 . 7 . 47 .
<>“saDyraN niyamya yatayo yamakarttahetiM .
<>jahyuH svarAqiva nipAnaKanitramindraH ..”
  [and 3 more lines]
```
------------------------------------------
 Case 0522: santrataM  [page 5-238](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-238) 
```
skd:santrataM,37672:santrataM:n:

 <HI>santrataM, klI, (sam + tana + ktaH . samo vA hita-
<>tatayoH iti pakze malopABAvaH .) satatam .
<>tadvati, tri . ityamaraH ..
```
------------------------------------------
 Case 0523: sannaDvaH  [page 5-244](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-244) 
```
skd:sannaDvaH,37748:sannaDvaH:n:

 <HI>sannaDvaH, tri, (saM + naha + ktaH) varmmitaH .
<>kftasannAhaH . (yaTA, BAgavate . 7 . 10 . 66 .
<>sannadDo raTamAsTAya SaraM DanurupAdade ..”)
<>vyUQaH . vyUhavinyAsayuktaH . iti medinI ..
<>AtatAyI . vaDodyataH . ityamarawIkAyAM rAya-
  [and 5 more lines]
```
------------------------------------------
 Case 0524: saptarzayaH  [page 5-264](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-264) 
```
skd:saptarzayaH,37819:saptarzayaH:n:

 <HI>saptarzayaH, puM, BUmni, (sapta fzayaH .) saptasaMKyakarzayaH
<>tezAM nAmAni yaTA . marIciH 1 atriH 2
<>aNgirAH 3 pulastyaH 4 pulahaH 5 kratuH 6 vaSizWaH 7 .
<>ityamaraH .. api ca . <>“maptarzimaRqalaM tasmAddfSyate sarvvatopari .
<>tatra saptarzayaH santi viniyuktAH prajAsfjA ..
  [and 11 more lines]
```
------------------------------------------
 Case 0525: saParI  [page 5-265](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-265) 
```
skd:saParI,37833:saParI:n:

 <HI>saParI, strI, (saPara + NIz .) matsyaviSezaH .
<>iti BarataH .. pu~wI iti BAzA .. tatparyyAyaH .
<>SaParI 2 prozWI 3 . ityamaraH .. SaParaH 4
<>SvetakolaH 5 . iti trikARqaSezaH .. (yaTA, --
<>“agADajalasaYcArO rohito'pi sTirAyate .
  [and 1 more lines]
```
------------------------------------------
 Case 0526: saBarA  [page 2-478](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=2-478) 
```
skd:saBarA,12920:saBarA:n:

 <HI>saBarA nagO yalagAstrayodaSayatirmatteBavikrI- <>qitam
<>..','..,'.',...,''',.'',.,'
<>mraBnEryAnAM trayeRa trimuniyatiyutA sragDarA <>kIrttiteyam
<>''','.','..,...,.'',.'',.'' <>najaBajajA jarO yadi tadA gaditA sarasI
<>kavISvarEH <>...,.'.,'..,.'.,.'.,.'.,'.'
  [and 406 more lines]
```
------------------------------------------
 Case 0527: samAkarzo  [page 5-271](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-271) 
```
skd:samAkarzo,37931:samAkarzo:n:

 <HI>samAkarzo, [n] puM, (samAkarzati cittamiti .
<>sam + A + kfz + RiniH .) atidUragAmi-
<>ganDaH . tatparyyAyaH . nirhArI 2 . ityamaraH ..
<>tfzRAjanakaganDayukte BakzyAdidravye dvayametaditi
<>sAYJaH .. tatra, tri ..
```
------------------------------------------
 Case 0528: samupeyivAn  [page 5-280](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-280) 
```
skd:samupeyivAn,38083:samupeyivAn:n:

 <HI>samupeyivAn, [s] tri, gamanakarttA . gamana-
<>viSizwaH . samupapUrvveDAtoH kvasupratyayena nizpannaH ..
```
------------------------------------------
 Case 0529: samBUtamAtre  [page 5-212](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-212) 
```
skd:samBUtamAtre,37363:samBUtamAtre:n:

 <HI>samBUtamAtre, tri . yaTA, mahABArate . 13 . <>41 . 8 .
<>“tvadarTamAgataM vidDi devendraM mAM Sucismite .
<>kliSyamAnamanaNgena tvatsaNkalpaBavena ha ..”)
```
------------------------------------------
 Case 0530: saraHkAkI  [page 5-287](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-287) 
```
skd:saraHkAkI,38210:saraHkAkI:n:

 <HI>saraHkAkI, strI, (sarasaH kAkI .) haMsI . iti <>SabdaratnAvalI ..
```
------------------------------------------
 Case 0531: saratAmpatiH  [page 5-294](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-294) 
```
skd:saratAmpatiH,38261:saratAmpatiH:n:

 <HI>saratAmpatiH, puM, (saritAM patiH . aluk samAsaH .)
<>samudraH . iti SabdaratnAvalI .. (yaTA, kumAre . <>2 . 37 .
<>“tasyopAyanayogyAni ratnAni saritAmpatiH .
<>kaTamapyamBasAmantaH AnizpatteH pratIkzate ..”)
```
------------------------------------------
 Case 0532: saritAMnATaH  [page 5-294](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-294) 
```
skd:saritAMnATaH,38260:saritAMnATaH:n:

 <HI>saritAMnATaH, puM, (saritAM nadInAM nATaH . aluk
<>samAsaH .) samudraH . yaTA, --
<>“tvaM deva saritAMnATa ! tvaM devi saritAM vare .
<>uBayoH saNgame snAtvA muYcAmi duritAni vE ..”
<>iti prAyaScittatattve gaNgAsAgarasnAnamantraH ..
```
------------------------------------------
 Case 0533: sarvvadryaN  [page 5-307](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-307) 
```
skd:sarvvadryaN,38383:sarvvadryaN:n:

 <HI>sarvvadryaN, [Yca] tri, sarvvasya pUjakaH . sarvvAnaYcati
<>ityarTe kvippratyayena nizpannaH . iti vyAkaraRam ..
```
------------------------------------------
 Case 0534: sahasraveDi  [page 5-316](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-316) 
```
skd:sahasraveDi,38561:sahasraveDi:n:

 <HI>sahasraveDi, [n] klI, (sahasraM viDatIti . viDa
<>CidrIkaraRe + RiniH .) hiNgu . iti medinI- <>rAjanirGaRwO ..
```
------------------------------------------
 Case 0535: sahikeyaH  [page 5-414](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-414) 
```
skd:sahikeyaH,40173:sahikeyaH:n:

 <HI>sahikeyaH, puM, (siMhikAyA apatyaM pumAniti .
<>siMhikA + Qak .) rAhuH . ityamaraH .. (yaTA, <>mAGe . 2 . 35 .
<>“Dniyate yAvadevako'pi ripustAvat kutaH suKam
<>puraH kliSnAti somaM hi sEMhikeyo'sura- <>dvizAm ..”)
```
------------------------------------------
 Case 0536: sAMSitaM  [page 5-204](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-204) 
```
skd:sAMSitaM,37218:sAMSitaM:n:

 <HI>sAMSitaM, tri, (saM + So + ktaH .) samyaksampA-
<>ditam . vratavizayakayatnavAn . yaTA saMSitaM
<>vratam . samyak sampAditamityarTaH . saMSito
<>brAhmaRaH . vratavizakayatnavAnityarTaH . iti <>sidDAntakOmudI ..
```
------------------------------------------
 Case 0537: sAttvatAH  [page 5-325](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-325) 
```
skd:sAttvatAH,38656:sAttvatAH:n:

 <HI>sAttva(tva)tAH, puM, BUmni, deSaBedaH . yaTA, --
<>“yadavastu daSArhAH syuH sAttvatAH kukurASca te .”
<>iti trikARqaSezaH ..
```
------------------------------------------
 Case 0538: sAttvatI  [page 5-325](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-325) 
```
skd:sAttvatI,38657:sAttvatI:n:

 <HI>sAttva(tva)tI, strI, nAwakavfttiviSezaH . yaTA, --
<>“aBineyaprakArAH syurBAzAH zaw saMskftA- <>dikAH .
<>BAratI sAttvatI kESikyAraBawyO ca vfttayaH ..” <>iti hemacandraH ..
<>(yaTA, SfNgAratilake . 3 . 42 -- 43 . <>“harzapraDAnADikasattvavftti-
<>styAgottarodAravacomanojYA . <>AScaryyasampatsuBagA ca yA syA-
  [and 20 more lines]
```
------------------------------------------
 Case 0539: sADayantI  [page 5-328](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-328) 
```
skd:sADayantI,38678:sADayantI:n:

 <HI>sADayantI, strI, (sADayatIti . sADa + Ric +
<>Satf . NIp .) upAsanAkartrI . yaTA, --
<>“saKi matprARanATantu sADayantI nirantaram .
<>atiSrAntAsi sadBAvasnehayoriyamOcitI ..” <>iti kAvyacandrikA ..
```
------------------------------------------
 Case 0540: sArasvatAH  [page 5-340](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-340) 
```
skd:sArasvatAH,38846:sArasvatAH:n:

 <HI>sArasvatAH, puM, BUmni, (sArasvate BavAH . aR .)
<>sArasvatadeSajAtamanuzyaH . yaTA, hemacandre .
<>“kASmIrAstu mADumatAH sArasvatAstu vika- <>rRikAH ..”
```
------------------------------------------
 Case 0541: sArTarkaH  [page 5-341](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-341) 
```
skd:sArTarkaH,38858:sArTarkaH:n:

 <HI>sArTarkaH, tri, (sArTa + kan .) arTena saha vartta-
<>mAnaH . yaTA, -- <>“SabdAntaramapekzyEva sArTakaH sArTaboDakft .
<>prakftiH pratyayaScEva nipAtaSceti sa triDA ..”
<>iti SabdaSaktiprakASikA ..
<>(saPalaH . yaTA, BAgavate . 10 . 41 . 45 .
  [and 2 more lines]
```
------------------------------------------
 Case 0542: sikatAH  [page 5-347](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-347) 
```
skd:sikatAH,38965:sikatAH:n:

 <HI>sikatAH, strI, BUmni, (sika + atac .) vAlukA .
<>iti medinI . te, 169 .. amaraSca . 3 . 3 . 73 ..
<>(yaTA, mahABArate . 2 . 76 . 16 .
<>“sikatA vapan savyasAco rAjAnamanugacCati
<>asaktAH sikatAstasya yaTA saMprati BArata .
  [and 1 more lines]
```
------------------------------------------
 Case 0543: sudaSanI  [page 5-369](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-369) 
```
skd:sudaSanI,39398:sudaSanI:n:

 <HI>sudaSanI, strI, (suzWu darSanaM yasyAH . NIp .)
<>amarAvatI . iti viSvaH ..
```
------------------------------------------
 Case 0544: sunBa  [page 5-374](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-374) 
```
skd:sunBa,39491:sunBa:n:

 <HI>sunBa, dyutO . hiMse . iti kavikalpadrumaH ..
<>(BvA0-para0-aka0-hiMse saka0-sew .) sumBate . <>iti durgAdAsaH ..
```
------------------------------------------
 Case 0545: supaRakadalI  [page 5-386](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-386) 
```
skd:supaRakadalI,39785:supaRakadalI:n:

 <HI>supaRakadalI, strI, (suvarRA suvarRavarRA sundara-
<>varRA vA kadalI .) kadalIviSezaH . cA~pA
<>kalA iti BAzA . tatparyyAyaH . suvarRaramBA 2
<>kanakamocA 3 potA 4 suvarRamocA 5 campaka-
<>ramBA 6 suraBikA 7 suBagA 8 hemaPalA 9
  [and 6 more lines]
```
------------------------------------------
 Case 0546: surApARAH  [page 5-383](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-383) 
```
skd:surApARAH,39726:surApARAH:n:

 <HI>surApARAH, puM, BUmni, (surA pAnaM yezAm . “pAnaM
<>deSe .” 8 . 4 . 9 iti Ratvam .) pUrvvadeSasTAH .
<>yaTA . surApARAH prAcyAH . iti saMkziptasAra- <>wIkAyAM goyIcandraH
```
------------------------------------------
 Case 0547: susnAH  [page 5-389](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-389) 
```
skd:susnAH,39878:susnAH:n:

 <HI>susnAH, puM, (suzWu snAtyanena rUkzatvAt . su + snE
<>+ kvip .) SamIDAnyaBedaH . KesArI iti <>BAzA . asya guRAH .
<>“susnA durvvAtalo rUkzaH kazAyo viSado guruH .” <>iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0548: sUkzmaSarIrARi  [page 5-390](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-390) 
```
skd:sUkzmaSarIrARi,39915:sUkzmaSarIrARi:n:

 <HI>sUkzmaSarIrARi, klI, (sUkzmARi SarIrARi .)
<>jYAnendriyapaYcakaM karmmendriyapaYcakaM vAyupaYcakaM [Pagẽ-390-c+ 52]
<>budDimanasI ceti saptadaSAvayavAni . iti <>vedAntasAraH ..
```
------------------------------------------
 Case 0549: sUtrabIRA  [page 5-394](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-394) 
```
skd:sUtrabIRA,39991:sUtrabIRA:n:

 <HI>sUtrabIRA, strI, (sUtrabadDA bIRA .) bIRA-
<>BedaH . tatparyyAyaH . lAvukI 2 . iti hArA- <>valI ..
```
------------------------------------------
 Case 0550: sfkki  [page 5-406](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-406) 
```
skd:sfkki,40086:sfkki:n:

 <HI>sfkki, klI, sfkkaRI . ityaruRaH . ityamarawIkAyAM <>BarataH ..
```
------------------------------------------
 Case 0551: sfPa  [page 5-407](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-407) 
```
skd:sfPa,40116:sfPa:n:

 <HI>sfPa, u hiMse . iti kavikalpadrumaH .. (BvA0-para-
<>saka0-sew . kvAvew .) saptamasvarI . u,
<>sarBitvA sfbDvA . iti durgAdAsaH ..
```
------------------------------------------
 Case 0552: seMhaH  [page 5-414](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-414) 
```
skd:seMhaH,40170:seMhaH:n:

 <HI>seMhaH, tri, (siMhasyAyamiti . siMha + aR .)
<>siMhasambanDI . siMhatulyaH . iti sidDAnta-
<>kOmudI .. (striyAM NIp . yaTA, mAGe . 1 . 47 .
<>“sawAcCawABinnaGanena viBratA <>nfsiMha sEMhOmatanuM tanuM tvayA .
<>sa mugDakAntAstanasaNgaBaNgurE- <>rurovidAraM praticaskare naKEH ..
```
------------------------------------------
 Case 0553: seMhikaH  [page 5-414](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-414) 
```
skd:seMhikaH,40172:seMhikaH:n:

 <HI>seMhikaH, puM, (siMhikAyAM BavaH . aR .) rAhuH .
<>iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0554: sekataM  [page 5-414](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-414) 
```
skd:sekataM,40174:sekataM:n:

 <HI>sekataM, klI, (sikatAH santyatreti . aR .)
<>vAlukAmayatawam . ityamaraH . 1 . 3 . 26 .. <>(yaTA, kumAre . 1 . 29 .
<>“mandAkinIsEkatavedikABiH <>sA kandukEH kftrimaputtrakESca .
<>reme muhurmaDyagatA saKInAM <>krIqArasaM nirviSatIva bAlye ..”
```
------------------------------------------
 Case 0555: senAnIH  [page 5-409](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-409) 
```
skd:senAnIH,40141:senAnIH:n:

 <HI>senAnIH, puM, (senAM nayatIti . nI + “satsUdvi-
<>zeti .” 3 . 2 . 61 . iti kvip .) kArtti-
<>keyaH . ityamaraH . 1 . 1 . 42 .. (yaTA, raGuH . 2 . 37 .
<>“aTEnamadre stanayA SuSoca <>senAnyamAlIQamivAsurAstrEH ..”)
<>vAhinIpatiH . iti cAmaraH . 2 . 8 . 62 ..
  [and 3 more lines]
```
------------------------------------------
 Case 0556: seviPale  [page 5-413](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-413) 
```
skd:seviPale,40164:seviPale:n:

 <HI>seviPale, klI, . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0557: soH  [page 5-415](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-415) 
```
skd:soH,40199:soH:n:

 <HI>soH, strI, pArvvatI . iti kecit .. zoDAtoH
<>kvippratyayena nizpannA ..
```
------------------------------------------
 Case 0558: sotkaRwaH  [page 5-415](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-415) 
```
skd:sotkaRwaH,40202:sotkaRwaH:n:

 <HI>sotkaRwaH, tri, (utkaRwayA saha varttamAnaH .)
<>utkaRwAyuktaH . tatparyyAyaH . utkaH 2 unmanAH
<>3 . iti jawADaraH .. (yaTA, kaTAsarit- <>sAgare . 31 . 45 .
<>“tatrodyAnagata sA taM vatseSaM saKyudIritam .
<>dadarSa dUrAt sotkaRWA cakIrIvAmftatvi- <>zam ..”)
```
------------------------------------------
 Case 0559: sOmyakfcCaH  [page 5-425](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-425) 
```
skd:sOmyakfcCaH,40326:sOmyakfcCaH:n:

 <HI>sOmyakfcCaH, puM, (sOmyaH anugraH kfcCraH .) vrata-
<>viSezaH . yaTA, gAruqe . 105 . 68 .
<>“piRyAkAcAmatakrAmbuSaktUnAM prativAsaram .
<>ekEkamupavAsaSca kfcCraH sOmyo'yamucyate ..”
```
------------------------------------------
 Case 0560: sOmyAH  [page 5-426](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-426) 
```
skd:sOmyAH,40331:sOmyAH:n:

 <HI>sOmyAH, strI, (somo devatA yAsAm . soma +
<>WyaR .) illalAH . tAstu mfgaSironakzatra-
<>SiraHsTAH paYca tArAH . iti viSvahemacandrO ..
```
------------------------------------------
 Case 0561: sOlvakaH  [page 5-427](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-427) 
```
skd:sOlvakaH,40357:sOlvakaH:n:

 <HI>sOlvakaH, puM, (sulvaM tAmrapAtrAdinirmmARaM
<>Silpamasya . sulva + Wak .) tAmrakuwwakaH . <>ityamarawIkA ..
```
------------------------------------------
 Case 0562: stananDayI  [page 5-432](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-432) 
```
skd:stananDayI,40424:stananDayI:n:

 <HI>stananDayI, strI, (stananDaya + wittvAt NIp .)
<>atibAlikA . ityamaraH . 2 . 6 . 41 ..
```
------------------------------------------
 Case 0563: stambapUH  [page 5-432](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-432) 
```
skd:stambapUH,40448:stambapUH:n:

 <HI>stambapUH, [r] strI, (stambAnAM pUriva .) purIBedaH .
<>tatparyyAyaH . <>“tAmaliptaM dAmaliptaM tAmAMlaptI tamAlinI .
<>stambapUrvvizRugfhaYca syAdvidarBA tu kuRqinam ..”
<>iti hemacandraH ..
```
------------------------------------------
 Case 0564: strIpuMsO  [page 5-443](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-443) 
```
skd:strIpuMsO,40528:strIpuMsO:n:

 <HI>strIpuMsO, puM, (strI ca pumAMSca . “acaturavicatu-
<>reti .” 5 . 4 . 77 . iti acpratyayena
<>sADuH .) strIpuMsayoryugmam . tatparyyAyaH .
<>miTunam 2 dvandvam 3 . ityamaraH . 2 . 5 . 38 .
<>dvivacanAnto'yam .. (yaTA, manuH . 9 . 25 .
  [and 1 more lines]
```
------------------------------------------
 Case 0565: snAyvarma  [page 5-456](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-456) 
```
skd:snAyvarma,40744:snAyvarma:n:

 <HI>snAyvarma, [n] klI, netrarogapraBedaH . tasya lakzaRaM yaTA
<>“sTiraM prastAri mAMsAQyaM SuzkaM snAyvarmapaYcamam ..”
<>itimADavakaraH .. anyat netrarogaSabde drazwavyam [Pagẽ-456-b+ 52]
```
------------------------------------------
 Case 0566: sparSAnandAH  [page 5-458](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-458) 
```
skd:sparSAnandAH,40799:sparSAnandAH:n:

 <HI>sparSAnandAH, strI, (sparSena Anando yAsAm .)
<>apsarasaH . iti trikARqaSezaH ..
```
------------------------------------------
 Case 0567: sPUrjaGuH  [page 5-462](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-462) 
```
skd:sPUrjaGuH,40888:sPUrjaGuH:n:

 <HI>sPUrjaGuH, puM, (sPUrjatIti . sPUrja vajranirGoze +
<>aTuc .) vajrapAtajanitaSabdaH . tatparyyAyaH .
<>vajranippezaH 2 . ityamaraH . 1 . 3 . 10 .. sPurjaTuH 3
<>visPUrjaTuH 4 visPurjaTuH 5 vajranirGozaH 6 . <>iti tawwIkA ..
```
------------------------------------------
 Case 0568: syannabIRaH  [page 5-464](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-464) 
```
skd:syannabIRaH,40960:syannabIRaH:n:

 <HI>syannabIRaH, tri, (syannA bIRA yatra .) stutaH .
<>iti hemacandraH ..
```
------------------------------------------
 Case 0569: sranBa  [page 5-466](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-466) 
```
skd:sranBa,40989:sranBa:n:

 <HI>sranBa, x u Na viSvAse . iti kavikalpadrumaH ..
<>(BvA0-Atma0-aka0-sew . ktvAvew .) viSvAsaH
<>SaNkArahitIBAvaH . x, asraBat . u, sramBitvA
<>srabDvA . Na, sramBate puttro mAtari . tAlavyAdi-
<>rayamityeke . iti durgAdAsaH ..
```
------------------------------------------
 Case 0570: srinBa  [page 5-466](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-466) 
```
skd:srinBa,41005:srinBa:n:

 <HI>srinBa, u hiMse . iti kavikalpadrumaH .. (BvA0-
<>para0-saka0-sew . ktvAvew .) rePayuktAdiH .
<>tftIyasvarI . u, srimBitA sribDvA . iti <>durgAdAsaH ..
```
------------------------------------------
 Case 0571: svargariH  [page 5-486](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-486) 
```
skd:svargariH,41134:svargariH:n:

 <HI>svargariH, puM, (svargasya pakze svaH giriH
<>parvvataH .) sumeruH . iti hema- <>candraH ..
```
------------------------------------------
 Case 0572: svargo  [page 5-486](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-486) 
```
skd:svargo,41136:svargo:n:

 <HI>svargo, [n] puM, (svargo'styasya Bogyatveneti . svarga +
<>iniH .) devatA . iti trikARqaSezaH .. (yaTA, <>kumAre . 2 . 45 .
<>“BuvanAlokanaprItiH svargiBirnAnuBUyate .
<>KilIBUte vimAnAnAM tadApAtaBayAt paTi ..”)
<>svargagAmini, tri . tallakzaRaM yaTA, --
  [and 5 more lines]
```
------------------------------------------
 Case 0573: svarRantatA  [page 5-488](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-488) 
```
skd:svarRantatA,41173:svarRantatA:n:

 <HI>svarRantatA, strI, (svarRavarRA latA .) jyotizmatI .
<>iti rAjanirGaRwaH .. (jyotizmatISabde'syA- <>vivaraRaM jYAtavyam ..)
```
------------------------------------------
 Case 0574: svarRapuzpI  [page 5-487](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-487) 
```
skd:svarRapuzpI,41165:svarRapuzpI:n:

 <HI>svarRapuzpI, strI, (svarRavat pItaM puzpaM yasyAH .
<>NIz .) AragvaDaH . iti SabdacandrikA ..
<>svarRaketakI . iti rAjanirGaRwaH ..
```
------------------------------------------
 Case 0575: svarvvEdyO  [page 5-488](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-488) 
```
skd:svarvvEdyO,41192:svarvvEdyO:n:

 <HI>svarvvEdyO, puM, (svaHsvargasya vEdyO .) aSvinI-
<>kumArO . tatparyyAyaH . aSvinIsutO 2 nAsatyO
<>3 aSvinO 4 dasrO 5 ASvineyO 6 . itya-
<>maraH . 1 . 1 . 54 .. dvitvaviSizwatvAdekavacanA-
<>BAvaH . iti svAmI . sarvva ete nityadvivaca-
  [and 3 more lines]
```
------------------------------------------
 Case 0576: svastibAcanaM  [page 5-489](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-489) 
```
skd:svastibAcanaM,41207:svastibAcanaM:n:

 <HI>svastibAcanaM, klI, (svasti maNgalasya vAcanam .)
<>mANgalyakarmmAramBakAlInavakzyamARamantroccAraRa-
<>pUrbbakataRqulavikiraRam . tanmantro yaTA . amuka
<>karmmaRi puRyAhaM Bavanto brubantu . ityukte
<>puRyAhamiti brAhmaREstrirukte amukakarmmaRi
  [and 19 more lines]
```
------------------------------------------
 Case 0577: svasnak  [page 5-473](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-473) 
```
skd:svasnak,41082:svasnak:n:

 <HI>svasnak, [j] tri, (svapiti tacColaH iti .
<>svapa + “svapitfzArnajiN .” 3 . 2 . 172 .
<>iti najiN .) nidASIlaH . ityamaraH . 3 .
<>1 . 33 ..) yaTA, BawwiH . 7 . 25 .
<>“ahaM svapnak pasAdena tava vandAruBiH saha ..”)
```
------------------------------------------
 Case 0578: svUrcCa  [page 5-495](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-495) 
```
skd:svUrcCa,41282:svUrcCa:n:

 <HI>svUrcCa, A vismftO . iti kavikalpadrumaH .. (BvA0-
<>para0-saka0-sew .) dantyAdiH hnasvI vakAra- [Pagẽ-495-c+ 52]
<>yuktaH . kvipi rACorlope svUH svurO svuraH .
<>A svUrcCitaM svUrRaM tena . svarcCati kaTAM
<>madyapaH . iti durgAdAsaH ..
```
------------------------------------------
 Case 0579: sveratA  [page 5-498](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-498) 
```
skd:sveratA,41297:sveratA:n:

 <HI>sveratA, strI, (svErasya BAvaH . svEra + tal .)
<>svEritA . ityamarawIkAyAM rAyamukuwaH . 3 . 2 . 2 ..
```
------------------------------------------
 Case 0580: svErinDnI  [page 5-498](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-498) 
```
skd:svErinDnI,41300:svErinDnI:n:

 <HI>svErinDnI, strI, paraveSmasTA svavaSA SilpakAriRI
<>nArI . iti SabdaratnAvalI .. paragfhasTA
<>svatantrA prasADanAnulepanAdiSilpakAriRIti
<>viSezaRatrayayuktA yA sA sErinDrI . svEraM
<>svAcCandyaM DaratIti svErinDrI nipAtanAt .
  [and 4 more lines]
```
------------------------------------------
 Case 0581: haRyA  [page 5-525](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-525) 
```
skd:haRyA,41575:haRyA:n:

 <HI>haRyA, strI, (halasya samUhaH . hala + “pASA-
<>diByo yaH .” 4 . 2 .. 49 . iti yaH .)
<>halasamUhaH . ityamaraH . 3 . 3 . 41 ..
```
------------------------------------------
 Case 0582: harirgfhaM  [page 5-514](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-514) 
```
skd:harirgfhaM,41449:harirgfhaM:n:

 <HI>harirgfhaM, klI, (harergfham .) harerAlayaH .
<>purIviSezaH . tatparpyAyaH . ekacakram 2 SumBa-
<>purI 3 . iti trikARqaSezaH ..
```
------------------------------------------
 Case 0583: harmma  [page 5-522](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-522) 
```
skd:harmma,41532:harmma:n:

 <HI>harmma, [n] klI, (harati glAnimiti . hf +
<>manin .) jfmBaRam . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0584: havanAyuH  [page 5-525](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-525) 
```
skd:havanAyuH,41583:havanAyuH:n:

 <HI>havanAyuH, [s] puM, (havanameva Ayuryasya .)
<>agniH . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0585: hasikaH  [page 5-526](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-526) 
```
skd:hasikaH,41608:hasikaH:n:

 <HI>hasikaH, tri, (haso hAso'styasteti . Wan .)
<>hAsyakarttA . iti kecit .. hasaSabdAt zRika- <>pratyayena nizpannaH ..
```
------------------------------------------
 Case 0586: hahAH  [page 5-531](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-531) 
```
skd:hahAH,41670:hahAH:n:

 <HI>hahAH, puM, hAhAnAmaganDarvvaH . iti SabdamAlA ..
```
------------------------------------------
 Case 0587: hitapraRIH  [page 5-537](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-537) 
```
skd:hitapraRIH,41791:hitapraRIH:n:

 <HI>hitapraRIH, puM, (hitaM praRayatIti . pra + nI +
<>kvip .) cAraH . iti SabdaratnAvalI ..
```
------------------------------------------
 Case 0588: huqat  [page 5-543](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-543) 
```
skd:huqat,41910:huqat:n:

 <HI>huqat, klI, vfzaSabdaH . iti kASIKaRqamiti <>kecit ..
```
------------------------------------------
 Case 0589: hU~  [page 5-544](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-544) 
```
skd:hU~,41927:hU~:n:

 <HI>hU~, vya, mantraviSezaH . tadudDAraparyyAyO yaTA, --
<>“hakAro vAmakarRAWyo nAdabinduviBUzitaH .
<>kUrccaM kroDa ugradarpo dIrGahUNkAra ucyate .
<>SabdaSca dIrGakavacaM tArApraRava ityapi ..” <>kiYca .
<>“SiKAvazaw ca kavacaM kroDo varmma hamityapi .
  [and 3 more lines]
```
------------------------------------------
 Case 0590: hemAdrijagaRaH  [page 5-549](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-549) 
```
skd:hemAdrijagaRaH,42054:hemAdrijagaRaH:n:

 <HI>hemAdrijagaRaH, puM, (hemAdrO joryyatIti . jF +
<>lyuH .) svarRakzIrI . iti ratnamAlA .. (asya
<>paryyAyavivaraRAdikaM svarRakzIrISabde jYAta- <>vyam ..)
```
------------------------------------------
 Case 0591: helAH  [page 5-550](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-550) 
```
skd:helAH,42067:helAH:n:

 <HI>helAH, strI, (hil + GaY . wAp .) strIRAM
<>SfNgAraBAvajakriyAviSezaH . ityamaraH . 1 . 7 . 32 ..
<>surate prOQecCA helA . yadAha .
<>“prOQecCA yAti rUQAnAM nArIRAM suratotsave .
<>SfNgAraSAstratattvajYErhelA sA parikIrttitA ..” <>iti . <>anye tu .
  [and 16 more lines]
```
------------------------------------------
 Case 0592: hEmazatI  [page 5-551](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-551) 
```
skd:hEmazatI,42088:hEmazatI:n:

 <HI>hEmazatI, strI, (himavato'patyaM strI . aR . NIp .)
<>pAvvatI . (yaTA, devIBAgavate . 12 . 8 . 57 . [Pagẽ-551-c+ 56]
<>“umABiDAnAM purato devIM hEmavatIM SivAm ..”)
<>harItakI . ityamaraH . 1 . 1 . 38 ; 2 . 4 . 59 ..
<>svarRakzIrI . (asyAH paryyAyo yaTA, --
  [and 14 more lines]
```
------------------------------------------
 Case 0593: hotFkAraH  [page 5-552](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-552) 
```
skd:hotFkAraH,42100:hotFkAraH:n:

 <HI>hotFkAraH, puM, hoturxkAraH . hoturmAtA . iti
<>mugDaboDavyAkaraRam ..
```
------------------------------------------
 Case 0594: hraqa  [page 5-555](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-555) 
```
skd:hraqa,42180:hraqa:n:

 <HI>hraqa, f Na gatO . iti kavikalpadrumaH ..
<>(BvA0-Atma0-saka0-sew .) f, aja-
<>hrUqat . ajuhroqat . Na, hrUqate . hroqate . <>iti durgAdAsaH ..
```
------------------------------------------
 Case 0595: hriRAyA  [page 5-554](http://www.sanskrit-lexicon.uni-koeln.de/scans/awork/apidev/servepdf.php?dict=skd&page=5-554) 
```
skd:hriRAyA,42164:hriRAyA:n:

 <HI>hriRAyA, strI, lajjA . ityamarawIkAyAM BarataH ..
<>asya rUpAntaram . hfRiyA hfRIyA ca ..
```
------------------------------------------

_Dr. Mārcis Gasūns_
