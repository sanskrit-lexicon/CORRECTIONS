Feb 25, 2016 -
  This revises the approach, by using the current pdhw2.txt.
  The variant hw0.py is not used.
  Rather, hwchk_iast.py is modified.
...
python hw0.py $PDTXT $PDHW2 > pdhw0_note.txt

Almost all the Headword records for PD have the headword written in 
...
The Devanagari in the current version of pd.txt is coded in slp1.
IAST is coded with the AS transliteration.
Thus, the headword (except for the 102 cases) is independently coded in two
forms in pd.txt.  The next program transcodes the IAST to SLP1, and
...
the case with the 11th example from pd-fuzzyalpha_not_cpd.txt,
where slp1 = akzatakrRI and AS = aks2ata-karn2i1.

python hwchk_iast.py pdhw0.txt hwchk_iast.txt
...
107620 records read from  pdhw0.txt
